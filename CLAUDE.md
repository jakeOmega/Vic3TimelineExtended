...
- **Mod state server** (the primary lookup tool, port 8950, ~60–110s startup):
  ...
  - Reload after mod-file edits: `curl -X POST http://localhost:8950/reload` (full, ~90 s) or `?engine_only=true` (skips ModState rebuild after re-launching the game with no mod-file changes). For fast iteration: `?mod_only=true&audits_only=true` (~25 s, reuses cached vanilla parse, skips file-rewriting generators — no working-tree side effects beyond `docs/engine/*_report.md`). Full flag table in `docs/guides/python_tools.md`. The reload response includes a `warnings` array if any post-load step surfaced findings (`modifier_visibility_audit`, `event_magnitude_audit`, `kill_character_audit`, …) **or crashed** (`{label, module, error, traceback_tail}`) — **always check it** before claiming a change is clean. It also carries `generators_wrote_files` (mod files the regenerators rewrote this run) with `reparsed_after_generators` (`false` ⇒ the one-shot re-parse failed, reload again), and `parse_failures` (files skipped as unparseable — also in `/status`).
...
- **Audits surface findings on every `POST /reload`** (`modifier_visibility_audit`, `event_magnitude_audit`, `kill_character_audit`, `loc_coverage_audit`, `concept_reference_audit`, `iterator_limit_audit`, `modifier_multiplier_var_audit`). Suppress an intentional flag with an inline `# REVIEWED YYYY-MM-DD: rationale` comment on the value line (or, for `loc_coverage_audit`, on the entity-opening `<name> = {` line; for `concept_reference_audit`, trailing on the loc value line; for `iterator_limit_audit`, on the `limit = {` or iterator-opener line; for `modifier_multiplier_var_audit`, on the `multiplier = …` or `remove_variable` line). Reports under `docs/engine/*_report.md`.
