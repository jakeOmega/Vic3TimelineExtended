...
- **Mod state server** (the primary lookup tool, port 8950, ~60–110s startup):
  ...
  - Reload after mod-file edits: `curl -X POST http://localhost:8950/reload` (full, ~90 s) or `?engine_only=true` (skips ModState rebuild after re-launching the game with no mod-file changes). For fast iteration: `?mod_only=true&audits_only=true` (~25 s, reuses cached vanilla parse, skips file-rewriting generators — no working-tree side effects beyond `docs/engine/*_report.md`). Full flag table in `docs/guides/python_tools.md`. The reload response includes a `warnings` array if any post-load step surfaced findings — **all 16 audits feed it**, not just the headline three — **or crashed** (`{label, module, error, traceback_tail}`) — **always check it** before claiming a change is clean. It also carries `generators_wrote_files` (mod files the regenerators rewrote this run) with `reparsed_after_generators` (`false` ⇒ the one-shot re-parse failed, reload again), and `parse_failures` (files skipped as unparseable — also in `/status`).
...
- **CI** (`.github/workflows/ci.yml`, GitHub Actions, Python 3.11 + 3.12): runs every game-independent check on every push and PR — `compileall`, the unittest suite, `ruff check --select F .`, `scripts/format_paradox_tabs.py --check`, `scripts/analysis/check_localization_files.py` (loc BOM / `l_english:` header / duplicate keys), `scripts/analysis/check_dds_dimensions.py` (block-compressed textures must be multiples of 4; known offenders in `scripts/analysis/dds_dimension_allowlist.txt`), and the eight offline audits (`duplicate_key`, `any_limit`, `loc_render`, `orphaned_event`, `attitude_key`, `kill_character`, `iterator_limit`, `modifier_multiplier_var`) in their `--strict` / `--check` exit-code mode. Nothing in CI needs a Victoria 3 install — dummy `VIC3_*` env vars satisfy `path_constants`' lazy resolution. Reproduce any failure locally by running that step's exact command. The tab check skips the three generator-owned files whose generator re-emits unformatted text (`common/ideologies/modified.txt`, `common/interest_groups/00_*.txt`, `common/scripted_effects/extra_law_consistency_generated.txt`).
- Other one-shot CLIs (event scaffolding, balance solvers, deposit configs, image pipelines) live under `scripts/generators/`, `scripts/analysis/`, `scripts/image_pipeline/`. Most config-driven generators and audits auto-run on `POST /reload` via `POST_LOAD_GENERATORS` in `mod_state_server.py`, which is `POST_LOAD_REGENERATORS + POST_LOAD_AUDITS`:
  ...
  - **Audits** (read-only; write only their reports under `docs/engine/`, mostly `*_report.md` but `kill_character_audit` writes `kill_character_audit.md`): `event_magnitude_audit`, `modifier_visibility_audit`, `kill_character_audit`, `loc_coverage_audit`, `concept_reference_audit`, `localization_accessor_audit`, `mod_structure_audit`, `loc_render_audit`, `any_limit_audit`, `iterator_limit_audit`, `modifier_multiplier_var_audit`, `pm_employment_audit`, `orphaned_event_audit`, `effect_trigger_validity_audit`, `duplicate_key_audit`, `attitude_key_audit`.
...
- **All 16 audits surface findings on every `POST /reload`** (roster above; `modifier_visibility_audit`, `event_magnitude_audit`, `kill_character_audit`, `loc_coverage_audit`, `concept_reference_audit`, `iterator_limit_audit`, `modifier_multiplier_var_audit` are the ones with a per-flag suppression comment). Suppress an intentional flag with an inline `# REVIEWED YYYY-MM-DD: rationale` comment on the value line (or, for `loc_coverage_audit`, on the entity-opening `<name> = {` line; for `concept_reference_audit`, trailing on the loc value line; for `iterator_limit_audit`, on the `limit = {` or iterator-opener line; for `modifier_multiplier_var_audit`, on the `multiplier = …` or `remove_variable` line). Reports under `docs/engine/*_report.md`.
