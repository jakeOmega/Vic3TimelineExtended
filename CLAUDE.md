...
- **Mod state server** (the primary lookup tool, port 8950, ~60–110s startup):
  ...
  - Reload after mod-file edits: `curl -X POST http://localhost:8950/reload` (full, ~90 s) or `?engine_only=true` (skips ModState rebuild after re-launching the game with no mod-file changes). For fast iteration: `?mod_only=true&audits_only=true` (~25 s, reuses cached vanilla parse, skips file-rewriting generators — no working-tree side effects beyond `docs/engine/*_report.md`). Full flag table in `docs/guides/python_tools.md`. The reload response includes a `warnings` array if any post-load step surfaced findings — **all 17 audits feed it**, not just the headline three — **or crashed** (`{label, module, error, traceback_tail}`) — **always check it** before claiming a change is clean. It also carries `generators_wrote_files` (mod files the regenerators rewrote this run) with `reparsed_after_generators` (`false` ⇒ the one-shot re-parse failed, reload again), and `parse_failures` (files skipped as unparseable — also in `/status`).
...
- Other one-shot CLIs (event scaffolding, balance solvers, deposit configs, image pipelines) live under `scripts/generators/`, `scripts/analysis/`, `scripts/image_pipeline/`. Most config-driven generators and audits auto-run on `POST /reload` via `POST_LOAD_GENERATORS` in `mod_state_server.py`, which is `POST_LOAD_REGENERATORS + POST_LOAD_AUDITS`:
  ...
  - **Audits** (read-only; write only their reports under `docs/engine/`, mostly `*_report.md` but `kill_character_audit` writes `kill_character_audit.md`): `event_magnitude_audit`, `modifier_visibility_audit`, `kill_character_audit`, `loc_coverage_audit`, `concept_reference_audit`, `localization_accessor_audit`, `mod_structure_audit`, `loc_render_audit`, `any_limit_audit`, `iterator_limit_audit`, `modifier_multiplier_var_audit`, `pm_employment_audit`, `orphaned_event_audit`, `effect_trigger_validity_audit`, `duplicate_key_audit`, `attitude_key_audit`, `event_image_audit`.
...
- **All 17 audits surface findings on every `POST /reload`** (roster above; `modifier_visibility_audit`, `event_magnitude_audit`, `kill_character_audit`, `loc_coverage_audit`, `concept_reference_audit`, `iterator_limit_audit`, `modifier_multiplier_var_audit`, `orphaned_event_audit`, `event_image_audit` are the ones with a per-flag suppression comment). Suppress an intentional flag with an inline `# REVIEWED YYYY-MM-DD: rationale` comment on the value line (or, for `loc_coverage_audit`, on the entity-opening `<name> = {` line; for `concept_reference_audit`, trailing on the loc value line; for `iterator_limit_audit`, on the `limit = {` or iterator-opener line; for `modifier_multiplier_var_audit`, on the `multiplier = …` or `remove_variable` line; for `orphaned_event_audit` and `event_image_audit`, on the event-opening `<id> = {` line). Reports under `docs/engine/*_report.md`.
...
- **Two docs are CRLF: `docs/systems/journal_entry_systems.md` and `docs/guides/gui_modding_guide.md`.** A Python rewrite (`open(...).write`) silently flips them to LF and turns a two-line edit into a 1,100-line diff; preserve line endings (`newline=""` on read/write, or read/write bytes) or edit with `sed`/`Edit`, and confirm with `file <path>` afterwards.
