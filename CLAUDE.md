...
- **Mod state server** (the primary lookup tool, port 8950, ~30 s startup; the parse itself is a few seconds since the linear-time parser):
  ...
  - Reload after mod-file edits: `curl -X POST http://localhost:8950/reload` (full, ~30 s) or `?engine_only=true` (skips ModState rebuild after re-launching the game with no mod-file changes). For fast iteration: `?mod_only=true&audits_only=true` (~13 s, reuses cached vanilla parse, skips file-rewriting generators — no working-tree side effects beyond `docs/engine/*_report.md`). Full flag table in `docs/guides/python_tools.md`. The reload response includes a `warnings` array if any post-load step surfaced findings — **all 19 audits feed it**, not just the headline three — **or crashed** (`{label, module, error, traceback_tail}`) — **always check it** before claiming a change is clean. It also carries `generators_wrote_files` (mod files the regenerators rewrote this run) with `reparsed_after_generators` (`false` ⇒ the one-shot re-parse failed, reload again), and `parse_failures` (files skipped as unparseable — also in `/status`).
...
- **All 19 audits surface findings on every `POST /reload`** (roster above; `modifier_visibility_audit`, `event_magnitude_audit`, `kill_character_audit`, `loc_coverage_audit`, `concept_reference_audit`, `iterator_limit_audit`, `modifier_multiplier_var_audit`, `orphaned_event_audit`, `event_image_audit`, `treaty_leverage_side_audit`, `event_context_audit` are the ones with a per-flag suppression comment). Suppress an intentional flag with an inline `# REVIEWED YYYY-MM-DD: rationale` comment on the value line (or, for `loc_coverage_audit`, on the entity-opening `<name> = {` line; for `concept_reference_audit`, trailing on the loc value line; for `iterator_limit_audit`, on the `limit = {` or iterator-opener line; for `modifier_multiplier_var_audit`, on the `multiplier = …` or `remove_variable` line; for `orphaned_event_audit` and `event_image_audit`, on the event-opening `<id> = {` line; for `treaty_leverage_side_audit`, on the `country_treaty_leverage_generation_add` line or the article-opening `<name> = {` line; `event_context_audit` alone takes a **check-tagged** comment anywhere inside the event — `# REVIEWED YYYY-MM-DD (<check>): rationale` — because an opening-line comment would also blind three other audits; a tag that no longer matches a flag fails `--strict`). Reports under `docs/engine/*_report.md`.
