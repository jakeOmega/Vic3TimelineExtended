...
- **Mod state server** (the primary lookup tool, port 8950, ~30 s startup; the parse itself is a few seconds since the linear-time parser):
  ...
  - Reload after mod-file edits: `curl -X POST http://localhost:8950/reload` (full, ~30 s) or `?engine_only=true` (skips ModState rebuild after re-launching the game with no mod-file changes). For fast iteration: `?mod_only=true&audits_only=true` (~13 s, reuses cached vanilla parse, skips file-rewriting generators — no working-tree side effects beyond `docs/engine/*_report.md`). Full flag table in `docs/guides/python_tools.md`. The reload response includes a `warnings` array if any post-load step surfaced findings — **all 21 audits feed it**, not just the headline three — **or crashed** (`{label, module, error, traceback_tail}`) — **always check it** before claiming a change is clean. It also carries `generators_wrote_files` (mod files the regenerators rewrote this run) with `reparsed_after_generators` (`false` ⇒ the one-shot re-parse failed, reload again), and `parse_failures` (files skipped as unparseable — also in `/status`).
