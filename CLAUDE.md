...
- Other one-shot CLIs (event scaffolding, balance solvers, deposit configs, image pipelines) live under `scripts/generators/`, `scripts/analysis/`, `scripts/image_pipeline/`. Most config-driven generators and audits auto-run on `POST /reload` via `POST_LOAD_GENERATORS` in `mod_state_server.py`, which is `POST_LOAD_REGENERATORS + POST_LOAD_AUDITS`:
  ...
  - **Audits** (read-only; write only `docs/engine/*_report.md`): `event_magnitude_audit`, `modifier_visibility_audit`, `kill_character_audit`, `loc_coverage_audit`, `concept_reference_audit`, `localization_accessor_audit`, `mod_structure_audit`, `loc_render_audit`, `any_limit_audit`, `iterator_limit_audit`, `modifier_multiplier_var_audit`, `pm_employment_audit`, `orphaned_event_audit`, `effect_trigger_validity_audit`, `duplicate_key_audit`, `attitude_key_audit`.
