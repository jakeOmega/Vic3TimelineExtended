...
| `map_data/state_regions/*.txt` (17 files) | `resources.py` | `deposits_config.json` + `state_trait_config.json` + vanilla `game/map_data/state_regions/` | `resources.py` injects two kinds of edits per state block: (1) `resource = { ... }` blocks driven by deposit scores in `deposits_config.json`; (2) state-trait references appended to the state's `traits = { ... }` line, driven by `state_trait_config.json` (trait *definitions* live in `common/state_traits/te_mining_traits.txt`; throughput modifiers must be registered in `common/modifier_type_definitions/mod_entity_modifier_types.txt`). After a vanilla map change, update both configs to point old/removed states at successors and re-run. Files do **not** carry the AUTO-GENERATED header — verify by reading `resources.py` before editing. |
...
| `common/scripted_effects/company_building_cleanup_effects.txt` | `scripts/generators/gen_company_building_cleanup.py` | `common/buildings/company_buildings.txt` + `common/company_types/*.txt` | One `if = { limit = { has_building = X NOT = { owner = { has_company = Y } } } remove_building = X }` block per company building, plus `remove_disbanded_company_buildings_effect` (called from `on_company_disbanded`). Idempotent — `regenerate` only writes when content drifts. Re-runs after every full `/reload` so adding a new company building in upstream files refreshes the cleanup automatically. |
| `localization/english/te_un_button_effects_l_english.yml` (all `UN_*_EFFECTS` keys) | `gen_un_button_descs.py` | `common/scripted_buttons/un_buttons.txt` (each button's applied/removed static modifiers + `un_authority` deltas) + the modifier definitions in `common/static_modifiers/extra_modifiers.txt` | Generates one `UN_*_EFFECTS` summary per UN button enumerating its modifier values and `un_authority` change; the buttons' hand-written `*_DESC` keys splice these in via `$..._EFFECTS$`. Runs **before** organize_loc.py. The file carries no in-file `AUTO-GENERATED` marker, so the `git grep -l "AUTO-GENERATED"` quick-check will not flag it — treat this row as the ownership record and edit the generator's inputs, never the `.yml` directly. |
...
| `gfx/interface/icons/goods_icons/prestige/*.dds` AND `texture =` lines in `common/prestige_goods/extra_prestige_goods.txt` | `scripts/image_pipeline/gen_prestige_icons.py` | Reads each prestige good's source icon (mod gfx if present, else vanilla `goods_icons/`), applies a uniform "shiny halo" treatment (warm radial glow + gentle gold tint + 4 corner sparkles), and rewrites the texture line to point at the new file. Skip-when-output-exists by default; pass `--force` to refresh after a vanilla icon update. Pipeline output is dedup'd by source filename, so multiple prestige goods sharing a `base_good` share the same prestige icon. |
| `gfx/map/fleet_entities/02_extra_fleet_entities.txt` | `scripts/generators/gen_fleet_entities.py` | One fleet_entity block per mod-added `ship_type` (currently 19, listed in the script's `SHIPS` table). Each block is a verbatim clone of a vanilla template from `~/src/vic3/game/gfx/map/fleet_entities/01_fleet_entities.txt` with only the top-level key + inner `ship_type =` line re-keyed — every `entity = "..."` reference is preserved so new ship_types render with vanilla 3D assets. Re-run after adding/removing a mod ship_type or after a vanilla bump that touches `01_fleet_entities.txt`. See GitHub issue #120. |
