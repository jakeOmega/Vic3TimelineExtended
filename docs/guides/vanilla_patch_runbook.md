...
Then grep **all** of `common/ events/ gui/ localization/` for every removed name. `effect_trigger_validity_audit` now covers the script blocks of `common/diplomatic_actions/`, `script_values/`, `journal_entries/`, `laws/`, `scripted_buttons/`, `decisions/`, `diplomatic_plays/`, `treaty_articles/` and `power_bloc_principles/` as well as events/scripted effects/triggers/on_actions (#295 — the gap that hid 1.14's `has_war_exhaustion` in `nuke.txt`), but the remaining `common/` subdirs and all of `gui/` + `localization/` are still hand-grep territory. After re-bootstrapping the catalog (§ 4), `git diff docs/engine/effect_trigger_valid_keys.txt | grep '^-'` also lists removed *vanilla script values* — grep the mod for those too. GUI 3-way merges, `REPLACE:` re-bases and loc-drift checks still need the vanilla clone (§ 5).
