...
| `silent_variable_audit` | `docs/engine/silent_variable_report.md` — options of visible mod events that write a variable the UI displays (read by mod loc or GUI through `Var('x')` / `GetVariable('x')` / `GetGlobalVariable('x')`, or through a `ScriptValue('sv')` whose script value reads `var:x`) outside any tooltip. `change_variable` / `set_variable` render nothing, so the option reads as doing less than it does — `nuclear_incident.40`'s "fix the procedure" showed only its expense, not the +3 command reliability. Descends into control flow, scope switches and the mod's scripted effects (with `$PARAM$` substituted). A write is covered inside `custom_tooltip = { … }` / `custom_description` / `hidden_effect`, or in (or under) a block that carries its own `custom_tooltip` line — lenient, so a sibling tooltip about something else also passes. Console-only `te_debug_*` files are skipped. Suppression is check-tagged, anywhere inside the option: `# REVIEWED YYYY-MM-DD (silent_variable): rationale`; `event_context_audit` ignores that tag (`FOREIGN_CHECKS`). |
| `prev_scope_audit` | `docs/engine/prev_scope_report.md` — a `prev` inside a scope change that was itself entered from a scope the script names outright (`root`, `scope:x`, `c:TAG`, …), inside some further scope. `prev` is the scope before the *most recent* scope change, so there it is that named scope, not the iterator it is almost always meant to be — `every_power_bloc_member = { root = { power_bloc ?= { add_leverage = { target = prev } } } }` gave a bloc leverage in its own leader once per member (`un_vote.2`, 2026-09-25). A `prev` directly inside the named scope is the ordinary use and is not flagged. Pure file scan over `common/` and `events/`; scope links come from `docs/engine/event_targets_summary.txt`. Suppress with `# REVIEWED YYYY-MM-DD: rationale` on the `prev` line or the named scope's opener line. |
| `container_timed_variable_audit` | `docs/engine/container_timed_variable_report.md` — a `set_variable` with `days`/`weeks`/`months`/`years` on a script container. The engine never expires a timed variable there, so `has_variable` stays true for good; every UN resolution's `un_res_cooldown` kept its topic on cooldown forever that way (2026-09-25). Flags a timed set whose own scope uses a container-only command (`has_tag`, `add_tag`, `remove_tag`, `destroy_container`, …) or is a container iterator / `on_created`, and one whose variable name is read or written in such a scope anywhere in `common/` or `events/`. Scopes are split as in `prev_scope_audit`. Fix with a month count counted down from a monthly pulse. Suppress with `# REVIEWED YYYY-MM-DD: rationale` on the `set_variable` line or its duration line. |
| `je_immediate_reset_audit` | `docs/engine/je_immediate_reset_report.md` — a write in a mod journal entry's `immediate` (or `immediate_all_involved`) that is not idempotent and not guarded by `NOT = { has_variable = <same variable> }`: `set_variable` to anything but `yes`, `change_variable`, `remove_variable`, a list or map clear. `immediate` runs again on every re-activation and, after the variable merge, on every entry a revolution's winner inherits (and on the fresh record of a `can_revolution_inherit = no` one), so such a write silently resets what the player built up — `je_banking_cycle` put an inherited panic back to 50 (2026-09-25 saves). Follows `if`/`else`/`random_list`/scope changes and the mod's scripted effects (with `$PARAM$` substituted), reporting a helper's write at its own line with the call chain. Each finding carries the entry's `can_revolution_inherit` / `can_deactivate`. Listed but not failing: progress-bar inputs (read by the entry's `current_value`). These are follow-up candidates to guard, and each listing says whether `goal_add_value` pins the goal against them. Also listed: pulse refreshes (inside a scripted effect that the entry's own pulse runs every time it fires, with the same parameters) and writes guarded only by another variable's absence (warnings). Suppress with `# REVIEWED YYYY-MM-DD: rationale` on any line of the write, on the innermost `if`/`else_if`/`else`/`random_list`/`random` opener, or on a call line leading to it. |
| `script_loc_reference_audit` | `docs/engine/script_loc_reference_report.md` — loc keys named in script that no vanilla or mod loc file defines: a `desc = "KEY"` labelling a script-value breakdown line (movement radicalism, party attraction, treaty AI acceptance, power bloc cohesion), `custom_tooltip = KEY`, an event `title`/`flavor`, a JE `custom_*_header`, and so on — `LOC_REFERENCE_FIELDS`, each of which resolves at every vanilla use bar one vanilla bug. The engine prints the raw key in the tooltip and logs nothing. `text` counts only inside `custom_tooltip = { }` (inside `custom_description` it names a `trigger_localization` entry); values with `$` are skipped. Complements `loc_coverage_audit`, which only asks whether each mod *entity* has its conventional keys and so never sees a key only script names. Suppress trailing on the `field = KEY` line. |


**Offline exit-code mode (what CI runs).** Fifteen of these audits are pure file scans — they never build a `ModState`, so they run with nothing but the dummy `VIC3_*` paths and are wired into `.github/workflows/ci.yml`. Each prints its report to stdout and signals findings through its exit status:
...
| `python3 container_timed_variable_audit.py --strict` | any unexempted flag exists |
| `python3 je_immediate_reset_audit.py --strict` | any unexempted `unguarded` flag exists (progress-bar inputs, pulse refreshes and other-variable guards are listed but never fail) |

"Unexempted" always means "without an inline `# REVIEWED YYYY-MM-DD: rationale` comment in the suppression position that audit documents". The other nine audits stay out of CI. Six need a live `ModState` (vanilla data, loc index). Two do not and run fine with CI's dummy `VIC3_*` paths — handy from a sparse worktree: `python3 effect_trigger_validity_audit.py` (**always exits 0** — read the `Flags (unreviewed)` line, e.g. `| grep -iE 'unreviewed|unresolved'`) and `python3 localization_accessor_audit.py --report` (exits 1 when `unreviewed > 0`). A third, `python3 script_loc_reference_audit.py --strict`, needs no `ModState` either (it reads vanilla keys from the committed `vanilla_parsed/` snapshot plus the mod's own loc) but is not in CI yet: it exits 1 while the findings it opened with are still unfixed. Run standalone, the eighteen offline audit CLIs (the fifteen in CI plus these three) print to stdout and leave `docs/engine/` alone — only the post-load chain writes the reports.
