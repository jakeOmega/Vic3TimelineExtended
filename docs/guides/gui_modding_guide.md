...
**Bars that share the width.** An `hbox` of fixed width whose items are `size = { 0 100% }` + `layoutpolicy_horizontal = expanding` (+ a `maximumsize` cap) divides the width between the *visible* items — vanilla `levels_progressbar` (`gui/shared/progressbars.gui`, used by `country_panel.gui`). With `ignoreinvisible = yes` on the hbox, hiding out-of-range samples widens the remaining bars instead of leaving empty slots, which is how a 1 / 5 / 20-year range selector works without three separate layouts.
