...
**Bars that share the width.** An `hbox` of fixed width whose items are `size = { 0 <height> }` + `layoutpolicy_horizontal = expanding` (optionally with a `maximumsize` cap) divides the width between the *visible* items — vanilla `levels_progressbar` (`gui/shared/progressbars.gui`, used by `country_panel.gui`). With `ignoreinvisible = yes` on the hbox, hiding out-of-range samples widens the remaining bars instead of leaving empty slots, which is how a 1 / 5 / 20-year range selector works without three separate layouts.

**History charts must also handle partial stores.** Choosing 480px only avoids the expanding-hbox remainder for full 12 / 60 / 240-month ranges. A partial store can contain any count: 121 expanding items in 480px produce 120 columns of 3px and a final column of 120px. The shared history chart therefore uses a right-anchored horizontal `flowcontainer` with fixed monthly widths of 40 / 8 / 2px for the 1 / 5 / 20-year ranges. A partial history leaves space on the left; no month absorbs unused plot width. Keep these widths in sync with the range lengths and the 480px inner plot.
...
19. **A formatted country name already carries its flag.** In tooltip text, `[….GetCountry.GetName]` and `[SCOPE.sCountry('x').GetName]` render the flag as well as the name, so a `GetFlagTextIcon` in front of them shows the flag twice. That is why vanilla always pairs `GetFlagTextIcon` with `GetNameNoFormatting`. The rule holds both for lines printed inside a country scope change and for lines that reference a country through a saved scope. The UN chamber and the UN ledger log both shipped doubled flags on the saved-scope form before this was seen in game (2026-09-25). This entry's first version said the saved-scope form needed the explicit flag; that was wrong. Use either `GetName` alone or `GetFlagTextIcon` + `GetNameNoFormatting`.
...
22. **A saved scope reaching a scripted GUI's `is_shown` is thinly evidenced — weigh it against the failure direction, and check `is_shown` not `is_valid` before citing precedent.** `saved_scopes` is documented as giving "event targets for in triggers / effects" and `is_shown` is a trigger, but the evidence is one-sided. **Vanilla:** of the two sguis declaring `saved_scopes`, neither `is_shown` reads one. **Strategic Reserve is not evidence either way, and is easy to misread as such:** `strategic_reserve_widget.gui:141,151,161` really do pass `AddScope('dir', …)` into `IsShown`, but `st_res_adjust_<good>_sgui.is_shown` is only `has_journal_entry` + `st_res_<good>_unlocked_trigger` — the `trigger_if = { limit = { scope:dir = 0 } … }` a few lines below it lives in **`is_valid`**. So st_res proves the call is harmless and that `is_valid` / `Execute` / the tooltips receive the scope; it proves nothing about `is_shown`. **One in-repo instance does read it:** `un_chamber_vote_sgui.is_shown` (`common/scripted_guis/un_chamber_sguis.txt`) gates its veto control on `trigger_if = { limit = { exists = scope:op scope:op = 2 } … }`, driven from `un_chamber_widget.gui:232,244,256`, and ships on main. Note that it hedges with `exists = scope:op`, and that its failure direction is **permissive** — if the scope never arrived the limit would simply fail, the `trigger_if` would be skipped, and the veto button would show for every member on every topic. That is a bug you could ship without noticing, so treat it as weak positive evidence rather than proof.

    **The decision rule is the failure direction, not the odds.** Where a missing scope shows a control too often, using it in `is_shown` is a reasonable risk. Where it would show **nothing** — the on/off pair of a widget whose vanilla button grid is hidden behind `is_ai = yes`, so there is no fallback surface — don't take it: answer "which half of the pair applies?" with a separate, scope-free read-only scripted GUI (`is_shown = { <trigger> }`, `is_valid = { always = no }`, `effect = { }`), and keep the op saved scope for `is_valid` / `Execute` / the tooltips, which are proven. Set the scope-free handler as the row's `datacontext`, wrap each control in a `widget` whose `visible` is `[ScriptedGui.IsShown(…)]` / `[Not(ScriptedGui.IsShown(…))]`, and let the buttons carry their own action-handler `datacontext` inside the blocks; a row instance is still ~10 lines. `gui/journal_entry_widgets/global_warming_widget.gui` plus the `gw_active_<policy>_sgui` family is the worked example.
...
25. **A temporary scope saved while a script-built list prints survives into the next entry.** In an `ExecuteTooltip` builder that iterates entries, `var:x = { save_temporary_scope_as = x_disp }` on one entry leaves `x_disp` set when the next entry prints, so a line that reads `SCOPE.sState('x_disp')` for an entry *without* `var:x` names the previous entry's state — no error, just the wrong name. Save the scope only when the entry has the variable, and print a line that reads it only from a branch that has checked for it; the plain fallback line must not read it. The UN ledger log (`un_authority_log_reason_on` in `un_authority_display_effects.txt`) is the worked form, and `test_un_ledger_reason_codes.py` checks it; the mission register (`un_chamber_mission_entry`) guards its host and state the same way.

26. **Headings inside a collapsible section, and a section nested in another.** A section that has grown several groups of lines reads better with a heading over each: a `#title` textbox with a `divider_clean` under it, one GUI row per heading, each group its own scripted GUI (gotcha #18 rules out script-built headings). A heading over a group that can be empty carries the group's `visible`, from an `is_shown`-only scripted GUI that reads nothing but the root (gotcha #22). A collapsible section *inside* another goes inside the parent's panel as a smaller `section_header_button` (fontsize medium, ~30 high, narrower) in a `flowcontainer` with `margin_left`; placed between the parent's header and its panel at full width, it reads as the next section. `un_chamber_widget.gui` has all three (`un_chamber_subheader`, `un_chamber_subsection_header` for the archive's "Voting Details").
