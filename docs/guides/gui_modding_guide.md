...
**Pick that hbox width so every bar count divides it.** Shared widths are integer pixels, and the remainder does not vanish: the layout gives it to the last item. With few bars nobody notices, but a history chart splitting 484px 240 ways gave 239 bars 2px and the newest one 6px — three times as wide as its neighbour, and the first thing the eye lands on. 480px divides 240, 60 and 12 exactly, so no range leaves a remainder at all. Check `plot_width % bar_count == 0` for *every* count the widget can show before picking the number (`te_history_chart.gui`).
