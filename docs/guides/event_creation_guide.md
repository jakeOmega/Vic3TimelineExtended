...
**There is no `is_triggered_only` in Victoria 3.** It is a CK3/Imperator keyword; vanilla Vic3 uses it in zero event files, and the engine silently ignores it. A Vic3 event with neither `mean_time_to_happen` nor a `random_events` registration is reachable only through `trigger_event`, so nothing needs to declare it. (`effect_trigger_validity_audit` flags the keyword as an unresolved helper call if you write it anyway. `orphaned_event_audit`'s docstring still says it keys on `is_triggered_only`; the code actually keys on the absence of `mean_time_to_happen`.)

## Available Event Videos

**Mod events overwhelmingly use a still texture, not a video** — 416 mod-authored `.dds` pictures under `gfx/event_pictures/` against 13 uses of a vanilla video across all of `events/`. Prefer reusing an existing picture; the full inventory is `git ls-files 'gfx/event_pictures/*'` and each one's subject is recorded as a generation prompt in `scripts/image_pipeline/event_image_prompts.py` (grep that instead of trying to eyeball a `.dds`). New pictures come from `scripts/image_pipeline/generate_event_images.py`, which needs a local FLUX install — not available on every machine.

```
event_image = { texture = "gfx/event_pictures/<name>.dds" }
```

Vanilla videos (`gfx/event_pictures/*.bk2`), usable as `event_image = { video = "<name>" }` — verified against the 1.14.3 install, which carries 74 (`placeholder_event scene` and `public_assasination_test` are engine test assets, excluded):

`africa_animism`, `africa_city_center`, `africa_construction_colony`, `africa_desert_expedition`, `africa_diplomats_negotiating`, `africa_leader_arguing`, `africa_prosperous_farm`, `africa_public_protest`, `africa_soldiers_breaking_protest`, `asia_buddhism`, `asia_confucianism_shinto`, `asia_dead_cattle_poor_harvest`, `asia_factory_accident`, `asia_farmers_market`, `asia_hinduism_sikhism`, `asia_politician_parliament_motion`, `asia_poor_people_moving`, `asia_sepoy_mutiny`, `asia_union_leader`, `asia_westerners_arriving`, `europenorthamerica_american_civil_war`, `europenorthamerica_antarctica`, `europenorthamerica_art_gallery`, `europenorthamerica_before_the_battle`, `europenorthamerica_capitalists_meeting`, `europenorthamerica_gold_prospectors`, `europenorthamerica_judaism`, `europenorthamerica_london_center`, `europenorthamerica_native_american`, `europenorthamerica_opium_smoker`, `europenorthamerica_political_extremism`, `europenorthamerica_rich_and_poor`, `europenorthamerica_russian_serfs`, `europenorthamerica_springtime_of_nations`, `europenorthamerica_sufferage`, `middleeast_battlefield_trenches`, `middleeast_courtroom_upheaval`, `middleeast_engineer_blueprint`, `middleeast_islam`, `middleeast_jungle_expedition`, `middleeast_middleclass_cafe`, `middleeast_oil_derricks`, `middleeast_police_breaking_door`, `middleeast_upperclass_party`, `southamerica_aristocrats`, `southamerica_child_labor`, `southamerica_christianity`, `southamerica_election`, `southamerica_factory_opening`, `southamerica_public_figure_assassination`, `southamerica_slave_chains`, `southamerica_slaves_night`, `southamerica_war_civilians`, `unspecific_airPlane`, `unspecific_airship`, `unspecific_armored_train`, `unspecific_automobile`, `unspecific_devastation`, `unspecific_factory_closed`, `unspecific_fire`, `unspecific_gears_pistons`, `unspecific_iceberg`, `unspecific_military_parade`, `unspecific_naval_battle`, `unspecific_politicians_arguing`, `unspecific_ruler_speaking_to_people`, `unspecific_sick_in_hospital`, `unspecific_signed_contract`, `unspecific_steam_ship`, `unspecific_temperance_movement`, `unspecific_trains`, `unspecific_vandalized_storefront`, `unspecific_whaling`, `unspecific_world_fair`

`event_image` accepts **multiple blocks, each with its own `trigger`** — the first match wins, and an untriggered block at the end is the fallback. Vanilla uses this for religion-gated art (see the seven-way ladder in `ig_petitions.txt`, mirrored by `monument_events.4`); it is the only clean way to picture something whose look depends on game state.

**Every visible event needs one.** The default event window has no fallback art, and the engine emits no warning — an event with no `event_image` renders a magenta missing-texture blob. The only supported alternative is a `gui_window` layout with no image panel plus `left_icon` / `right_icon` portraits (vanilla's `event_window_1char_tabloid`). `event_image_audit` enforces this; see `docs/engine/event_image_report.md`.
...
- **A variable change is invisible unless you say it.** `change_variable` / `set_variable` render nothing in the option tooltip, so an option that moves a number the UI shows (a journal entry's command reliability, crew strain, credibility) reads as doing less than it does. `nuclear_incident.40`'s "fix the procedure" showed a 60-day expense and nothing else; its +3 command reliability was silent. Wrap the write and name the change in the words the UI uses: `custom_tooltip = { text = nd_tt_reliability_up_3 change_variable = { … } clamp_variable = { … } }` → "#v Command reliability#! rises by #G 3#! …". If the variable drifts back (reliability returns a quarter of the way to its target each month), say so rather than imply permanence. `silent_variable_audit` (`docs/engine/silent_variable_report.md`, `--strict` in CI) flags a displayed variable written outside any tooltip.
...
- **Cooldowns become variables.** An event `cooldown` on the victim event becomes a timed variable, because the precursor's pick can read a variable but not a cooldown. Set it in the precursor's `immediate` (as the decolonization chains do), so it holds from the moment the actor is asked, whatever they answer. Setting it in the victim event's `immediate` lets two actors pick the same victim inside the delay. "Decline" sets a short variable of its own, so the precursor doesn't re-ask next month.
- **Decline is the default.** Events auto-resolve to `default_option` at `duration`, so a human who lets a precursor lapse must not end up taking the aggressive act. Keep the AI weights on the act.
