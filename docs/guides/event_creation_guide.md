...
- **Grant the modifier IN the option, not in a separate broadcast effect.** The engine renders `add_modifier` inside an event option as a tooltip beneath that option — that is the *only* place a player sees what an event does to them. If a scripted effect/on-action applies the modifier (e.g. `every_country { add_modifier = ... }`) and then merely `trigger_event`s a notification whose options carry no effects, the event reads as a no-op: the bonus is invisible unless the player manually opens their modifier list. Put the `add_modifier` in the option(s); both the actor who triggered the chain and any notified bystanders should see their gain in the tooltip. (Bug pattern fixed in the agricultural-diffusion events — modifiers had been broadcast in `agdiff_handle_tech_acquired_effect` before the event fired.) Only reach for the broadcast-then-`show_as_tooltip` split when application must be guaranteed *before* the option resolves and the tooltip is a genuine forward-looking preview (see `events/un_vote_events.txt` — there the modifier is applied *later*, so the preview is honest; previewing a modifier already applied is misleading).
