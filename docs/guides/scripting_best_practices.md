...
- **Reaching the loser, and telling which side won.** `te_cw_parent` is the shared pointer: `on_revolution_start` stores it on the rebels (`scope:target`) as ROOT, and the original clears its own copy at every `on_revolution_start` and `on_secession_start`, because a former rebel winner still carries one and the merge would keep that stale value if it later won as the loyalists. Read it only as `var:te_cw_parent ?= { NOT = { this = root } is_country_alive = no }`, which holds exactly on a rebel win: a loyalist winner inherits the rebels' pointer, which names itself, and the annexed loser has no states. Secessions never set it, because a seceder never continues the original. After a rebel win it is **retired at the winner's next monthly pulse**, not in the hook, so every reader of `on_civil_war_won` sees it whatever order the hook's files run in; the same hook re-points any other uprising against the annexed loser at the winner, its continuation. Until that pulse it still names the dead loser, and with two revolutions at once a second war can end inside the window (the first winner beating the second as the loyalists), so a reader that must act **once per loser** keeps its own marker of the object it acted on and refuses it — its own, never a shared "spent" flag, which would make readers depend on each other's order (`te_mon_cw_bank_taken`; without it the second win added the loser's gold again). Contract: the foot of `common/on_actions/te_monetary_on_actions.txt`. Reading the loser *through* it is the untested step above. A reader that needs only "did the rebels win", and must not depend on the order of on_actions inside one hook (`te_cw_parent` is dropped at `on_civil_war_won` when it names the winner), can keep its own self-pointer set on ROOT at the start hooks: `var:X = root` at `on_civil_war_won` means the loyalists won (`je_civil_rights`). A loyalist winner should then drop state it inherited from the rebels only, such as mirrors of policies it never chose.
