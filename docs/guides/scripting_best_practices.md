...
- **`immediate` runs again on every activation.** A JE that can deactivate and re-activate re-runs its `immediate`, so any `set_variable` there silently resets state the player accumulated — `je_nuclear_program` used to delete a country's entire nuclear arsenal when it regained great-power standing. Guard the variables that must survive with `NOT = { has_variable = X }`. But do **not** guard a variable that feeds `current_value`: the progress bar's goal is computed once at activation as `current_value + goal_add_value` (vanilla `journal_entries.md:392`), so carrying leftover progress in gives a non-zero `GetBaselineValue` and an inflated goal, and the bar reads negative the next time the value wraps.
