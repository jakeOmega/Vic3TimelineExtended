...
- **`on_civil_war_won` runs with ROOT = the winner, after the merge.** Vanilla's `anti_monarchist_revolution`, set on the original at `on_revolution_start`, reached the winner and was consumed there. Whether the loser can still be read through a stored scope variable at that moment is **not yet tested**.
