...
- **State held only in modifiers.** Anything a system records solely as a modifier — a toggled policy, a membership, a one-off reward — is gone. Mirror it in a country variable and rebuild the modifier from the variable.
- **A tracker variable without its modifier.** A system that records "modifier X is applied" in a country variable (`*_applied`, `*_mod_on`, a band index) and swaps from that record will, on a winner that lacked the variable, inherit the loser's "applied" with no modifier behind it — on the country or on an inherited entry — and never re-add it. Recompute statelessly each pulse (the nuclear domestic stance does), reset the tracker in `immediate` (`je_nuclear_program` zeroes `nd_*_mod_on`), or key a first-run sweep on the modifiers actually being absent rather than on the tracker being missing.
- **An unguarded `immediate`.** It runs again on every inherited entry: guard every set that must survive with `NOT = { has_variable = X }` — except a variable feeding `current_value` (the "`immediate` runs again on every activation" bullet under § "A Deactivated Journal Entry Can Only Speak Through `status_desc`").
- **A completed entry.** Record completion in a country variable and require its absence in `possible` (`je_colonial_empire`'s `colonial_empire_completed`).
...
- **Per-regime choices** (policy settings, doctrines, pledges) pass to the new regime wholesale, are replaced by the rebels', or are lost, depending on where they live. Decide which per system.
...
- **`immediate` runs again on every activation.** A JE that can deactivate and re-activate re-runs its `immediate`, and so does every entry a revolution's winner inherits (§ "What a Civil War's Winner Inherits"), so any `set_variable` there silently resets state the player accumulated — `je_nuclear_program` used to delete a country's entire nuclear arsenal when it regained great-power standing. Guard the variables that must survive with `NOT = { has_variable = X }`. But do **not** guard a variable that feeds `current_value`: the progress bar's goal is computed once at activation as `current_value + goal_add_value` (vanilla `journal_entries.md:392`), so carrying leftover progress in gives a non-zero `GetBaselineValue` and an inflated goal, and the bar reads negative the next time the value wraps.
...
**Worked case — swapping one member of a banded modifier family.** `banking_cycle_apply_stance_band` has to keep exactly one of `banking_stance_band_1..5` on `je_banking_cycle`. It decides entirely from two variables: the band the monthly update computed (`te_mon_stance_band`) and a stored `te_mon_stance_band_applied` recording which one is physically on the entry. The remove branch keys on the *stored* value, the add branch on the *new* one, and nothing reads `has_modifier` anywhere. Two details generalise: (a) run the whole swap only when the two differ, so an unchanged month costs nothing; (b) the stored tracker can disagree with the entry it sits beside, so a first-run sweep that removes every member of the family is cheap insurance — but key it on what can actually go wrong. A revolution's winner inherits the entry **with no modifiers** and the loser's tracker (when it had none of its own), so the tracker claims a band that is not there (§ "What a Civil War's Winner Inherits"). This case's sweep fires only when the tracker is *missing*, which a revolution never produces, and the saves show the band staying off for good: `docs/audits/civil_war_inheritance_audit.md` F7, not yet fixed. The sweep was written for the reverse premise (modifiers move, variables don't), which the saves disproved.
