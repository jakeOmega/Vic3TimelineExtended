...
- **Switch the PM with a mode group.** Add a PM group of two effect-less mode PMs, ratcheted one way (`market` unlocks on itself; `direct` unlocks on either). Gate each real PM family on its mode through cross-group `unlocking_production_methods`, and flip every building with `activate_production_method` when the rule is off. That call only needs the target PM to be *available*, which the one-way ratchet guarantees. It never needs to bypass `unlocking_*`, which nothing proves it can. A building the player flips by hand in the other setting cannot be ratcheted back, so replace it by script (`remove_building` + `create_building`).
