...
- **A downsizeable government building the AI depends on needs a guard against the AI.** In direct games play-testing saw the AI remove every Construction Site it had, and a country without a site may have no construction to build one with. Script cannot steer the AI's downsizing, and `downsizeable` is a plain flag (no trigger), so taking it away takes it from players too. Undo the AI's removals after the fact instead. Record each state's level (and PM) in state variables every week, for players as well, so that a player's own downsizing is never undone. Put back what an AI country lost, with exceptions where shedding is legitimate (`in_default`). Add a small top-up modifier so that a country with none still has something to build with (`te_direct_construction_keep_ai_sites`, `te_direct_construction_update_floor`). For a top-up, back out the modifier's own last contribution (stored in the persistent variable its `multiplier` reads) rather than granting a flat bonus that cuts off when the first level is built. To raise an existing building without firing its workers, use `create_building` with the target level (the engine documents max semantics), then check the level and rebuild exactly if it overshot, since the semantics are disputed ("Building Lifecycle Effect Gotchas").
