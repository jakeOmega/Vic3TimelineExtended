...
Verify it against a real save rather than by eye: `python3 scripts/analysis/check_save_history_order.py` unpacks the newest `.v3` and prints `ascending=` per store. A non-ironman save is a 24-byte header plus a zip whose `gamestate` is Clausewitz binary, and mod variable names survive in it as plain strings, so any script-held list can be read back the same way.
