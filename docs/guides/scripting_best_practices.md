...
- **A mod file that INJECTs into a vanilla script value must sort after the vanilla file.** `INJECT` errors when the target entry does not exist yet (1.12 `inject_types` digest) and files load alphabetically across the merged tree — hence the `zz_` prefix. Every other mod INJECT file (`modified.txt`, `extra_modifiers.txt`, `extra_laws.txt`, …) already happens to sort after its target.
