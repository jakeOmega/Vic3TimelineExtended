...
`every_*` / `random_*` / `ordered_*` / `any_*` take `limit = { }` as a property of the iterator, not as a sequential statement. `every_scope_state = { do_a = yes  limit = { has_building = X }  do_b = yes }` runs **both** `do_a` and `do_b` only in states that have X — `do_a` is not "before the filter". Found 2026-09-12 in `remove_invalid_buildings` (`extra_on_actions.txt`), where the generated company-building sweep was silently gated by an unrelated space-program limit. Put `limit` first by convention, and split into two iterations when two child effects need different filters. Candidate parse-time audit: flag any iterator whose `limit` is not its first child.
