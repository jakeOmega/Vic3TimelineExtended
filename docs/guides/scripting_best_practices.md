...
- **The global `on_monthly_pulse` has no root, so a shared effect it reaches must not read `ROOT`.** Vanilla's `00_code_on_actions.txt` says so ("No root scope set"). A scripted effect written for a button or an event, where ROOT is the acting country, loses that country when a global pulse calls it inside `some_scope = { … }`: `value = ROOT` writes nothing useful, and `this = ROOT` compares against nothing. Save the actor from the current scope instead (`save_scope_as`, `value = THIS`). `un_charter_invitations` does this for the UN's refounding path (`un_conference_monthly_update`); `un_found_organisation`'s `un_hq_country = ROOT` still has the problem there (#430 PR notes).
