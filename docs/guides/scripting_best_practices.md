...
### OPEN: does a `modifier = { }` inside an `INJECT:` **sum** with vanilla's, or overwrite it?

**Unresolved for ranks and laws as of 2026-09-19; owner-reported as *summing* for technologies. Nothing below is a recorded probe — do not cite it as one.** The mod has shipped on the *summing* assumption since April (`common/laws/sol_expectations_vanilla_injections.txt`, e.g. `law_industry_banned`: `state_expected_sol_mult = 0.1` against vanilla's −0.1), and monetary policy phase 1 added fourteen more blocks that depend on it, across three files: `common/country_ranks/te_monetary_rank_injections.txt` (eight ranks), `common/laws/te_monetary_law_injections.txt` (`law_laissez_faire`) and `common/technology/technologies/te_monetary_tech_injections.txt` (the five vanilla finance techs). The mod owner states that a tech's `modifier = { }` INJECT sums with vanilla's in this mod, which is what the tech file was written on (PR #335, 2026-09-19); that is an owner report rather than a probe, and it does not carry to ranks or laws. If INJECT is last-wins instead, each family is wrong *twice*: the cancel adds to vanilla's value instead of zeroing it, **and** the injected block wipes every other key in the entity's vanilla `modifier = { }`.
...
Four tooltip reads settle it. They need **no new code** — run them on any current build (`monetary_policy_design.md` §17 checks 1–4):
