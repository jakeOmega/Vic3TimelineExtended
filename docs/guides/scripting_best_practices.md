...
The `state_` / `building_` / `interest_group_` prefix is the modifier's **mask** (what `/engine-docs/origin/<name>` reports as `"mask": "state"`), not a restriction on who may carry it. A static modifier applied to a country with `add_modifier` may contain `state_*` keys, and they apply across every state that country owns — you do **not** need to iterate `every_scope_state`. This is how vanilla laws already work (`law_women_in_the_fields` carries `state_lower_strata_standard_of_living_add = -4`), and it holds for event modifiers too: `modifier_donghak_heaven_on_earth` (`common/static_modifiers/07_sphere_of_influence_4_modifiers.txt`) mixes `country_legitimacy_base_add` with `state_lower_strata_standard_of_living_add` and is added in a `country_event` (`events/soi_events/00_ep1_korea_events.txt`). So when reaching for a per-stratum or per-building effect from country scope, check the mask and use the masked key rather than building an `every_scope_state` iterator around it. Only this direction is verified — the reverse (a *country-masked* key inside a state-scoped modifier) is untested here. Getting the mask right is necessary but not sufficient: the key still has to be one the engine reads continuously (see the next section).

## Standard of Living Is an INTEGER — Fractional SoL Modifiers Are Inert

Pop standard of living is a whole-number level (it tracks the pop's wealth level; vanilla `NUM_WEALTH_LEVELS = 99`). A `*_standard_of_living_add` below 1 therefore does **nothing on its own** — it only matters if it happens to sum with other live sources across a whole point. Sizing one of these in tenths, or as a small unit value behind `add_modifier { multiplier = … }`, produces a modifier that renders in the tooltip and changes no outcome. Every vanilla grant of `state_<strata>_standard_of_living_add` but two is a whole number, which fits. Size SoL effects in whole points, or use a different channel.

The channel that *does* work at fractional magnitudes is **expectations**. `state_<strata>_expected_sol_add` is consumed continuously — vanilla's own tax modifiers ship `-0.5` / `+1.5` on it, and `RADICALS_MONTHLY_FROM_LOW_SOL` scales by the raw delta between SoL and expected SoL — so a tenth of a point of expectation is a tenth of a point of delta. In this mod that key is reached through `country_sol_expectations_<strata>_offset_add`, summed in `sol_expectations_<strata>_shift_value` (`common/script_values/extra_script_values.txt`) and applied as the multiplier on the unit modifier `sol_expectations_<strata>_strata_shift` (`common/scripted_effects/sol_expectations_effects.txt`).

**But size an expectations offset more conservatively than the SoL change you were reaching for.** An SoL change gets priced into expectations over time; an expectations offset never does. It is a standing ±X against whatever expectations would otherwise be, for as long as the modifier is on the country — which for a condition-gated modifier can be decades. For reference, a single strong labour law carries about +1 of headwind (`law_worker_protections`, `sol_expectations_vanilla_injections.txt`), so a compensating offset whose realistic maximum is a few tenths is already a substantial, permanent lever.
...
- **Solution:** Move state-scoped modifier updates that use complex script values to a **state pulse** — `on_yearly_pulse_state` or `on_monthly_pulse_state`; both guarantee a clean state scope chain. `migration_crowding` uses the yearly one; `tourism` uses the monthly one (`tourism_on_action`), because its multipliers read `city_size_rank` (recomputed every `on_monthly_pulse`) and live building levels, so a yearly refresh would run up to 11 months stale. Pick the cadence from how fast the multiplier's inputs actually move, and give each modifier exactly **one** refresh site.
...
- **In treaty-article `on_entry_into_force`/`on_break`/`on_withdrawal` effects, `ROOT` is the `treaty_article`, NOT a country** (engine `event_scopes.log`: the on-action expects scope `none`; the hook runs in `treaty_article` scope). So country-scope idioms copied from elsewhere silently break — e.g. `every_country = { limit = { market_capital = ROOT.market_capital } }` lifted from a scripted button (where `ROOT` is the acting country) becomes an invalid-object read in an article and the limit matches nobody (or, if the engine drops the term, everybody). Enter the country explicitly first — `scope:article_options = { source_country = { ... } }` — then reference it as `prev` inside a nested `every_*`, or `scope:source_country` / a saved scope. This bit `109_enforce_emissions_reduction.txt`.
...
- **Variables backing `add_modifier { multiplier = ... }` must persist beyond the call site.** The engine re-evaluates the `multiplier` expression on subsequent ticks (decay/tooltip), not just at add time. Removing the variable right after `add_modifier` makes every later evaluation read `var:X` as `'none'` and emit `Value of wrong type ... Got value of type 'none'` from `jomini_scriptvalue.cpp` per re-eval. **Fix:** never `remove_variable` a name that's referenced by a still-active modifier's multiplier — leave the variable set; it can be safely overwritten by the next `set_variable` call. Vanilla pattern: `c:FRA.var:PRC_money_transfer` in `01_paris_commune.txt` is set at JE start, used in `add_modifier { multiplier = ... }`, and never removed. Mod example: `population_transfer_effect` originally removed `pop_transfer_count` after the disruption modifier was added; reverted, and the runtime errors disappeared. Second instance (fixed 2026-09-12): `update_intel_sharing_defense` in `treaty_article_effects.txt` applied a permanent shield modifier with `multiplier = var:_best_boost` and then `remove_variable = _best_boost` — the multiplier now reads the persistent `intel_shield_mult` copy instead. Third instance (fixed 2026-09-13): `te_construction_market_apply_per_site_modifier` — caught by `modifier_multiplier_var_audit`. Enforced by `modifier_multiplier_var_audit.py` on every `POST /reload` (`docs/engine/modifier_multiplier_var_report.md`).
...
- **Working pattern: country-variable handoff.** Compute the building-scope value at country scope via `prev`, write it to a country variable on `owner`, then have `add_modifier` read `owner.var:X`. The variable lookup works from the outer-scope evaluation context. Clear the variable at the **start** of the effect only, so a stale value can't leak in from a previous tick or a partially-run iteration — and **never at the end**: per the "Variables backing `add_modifier { multiplier = … }` must persist beyond the call site" bullet above, the engine re-evaluates the stored multiplier expression on later ticks, so a trailing `remove_variable` makes every re-evaluation read `'none'`. There is nothing to clean up — the next call overwrites the variable. Mod example: `te_construction_market_apply_per_site_modifier` in `common/scripted_effects/te_construction_market_update_effects.txt`.
  ```
  te_apply_per_site_modifier = {
      ...
      # No trailing remove_variable: the multiplier is re-evaluated each
      # tick and must still find te_cm_capacity_mult set.
  }
  ```
...
- **Two objects, one tag.** During a revolution the rebels are a second country object with the same country definition (vanilla tests `is_revolutionary = yes` with `country_definition = c:JAP.country_definition`). The rebel starts with **no mod variables** except what the mod's own `on_revolution_start` hooks give it.
- **The winner annexes the loser** (the effect `annex_as_civil_war` names "the inheritance effects of a victorious side"). The revolutionary object kept the tag. The loyalist object was still in the save just after the win — every variable, list and modifier intact — and gone two weeks later.
- **Variables are merged, and the winner's own values take precedence.** Every variable only the loser held arrived with its exact value — the whole nuclear arsenal and posture. Wherever both held one, the winner kept its own in every case that had not simply ticked over in the interval (91 of 102 differing values). So the loser's value is **discarded**, not added: the loyalists' 111M `te_bank_gold` vanished because the rebels already had 2.8M of their own.
- **Variable lists are not inherited at all**, even when the winner has no list of that name (the loser's 240-sample `te_hist` list). Script containers parented to the loser die with it.
- **No modifier is inherited — country or journal entry.** None of the loser's mod static modifiers reached the winner (`nuclear_power`, `declared_bankruptcy`, `un_dues_modifier`, …).
- **Journal entries are inherited by default** (`can_revolution_inherit` unset behaves as `yes`: vanilla `je_strike` was). An inherited entry is a new active record with the loser's start date and progress-bar values, but **an empty modifier list** (`je_banking_cycle` 10 → 0, `je_united_nations` 12 → 0). Its `immediate` **runs again, after the variable merge**: `has_variable`-guarded sets keep the inherited value (`iw_tradecraft`), unguarded ones reset it (`finance_cycle_value` 6.6 → 50).
- **An inherited entry keeps its native progress bar's baseline and goal.** They are copied from the loser's record, not evaluated again: the winner's `je_global_warming` kept baseline 0.1 and goal 4.0 while the anomaly stood at 1.18 (the record's i64 fixed-point ×1e5 fields `0x4f11` / `0x4f15`, beside the start date `0x4f0a`). So a `has_variable` guard on a `current_value` variable is safe on inheritance. Only a **re-activation** of a record evaluates the goal again.
- **With `can_revolution_inherit = no`** the winner gets a fresh inactive record, which auto-activates within two weeks if its `possible` holds, running `immediate` then (`je_nuclear_program` zeroed a warhead bar at 97.5 %).
- **Completed journal entries are forgotten.** The winner gets fresh records for them, and one whose `possible` still holds starts over (`je_civil_rights` did, at 30 %).
- **Third countries' references to the loser dangle.** Variables pointing at the dead object were still there two weeks later; every read must be `?=` / `exists`-guarded.
- **`on_civil_war_won` runs with ROOT = the winner, after the merge.** Vanilla's `anti_monarchist_revolution`, set on the original at `on_revolution_start`, reached the winner and was consumed there. Whether the loser can still be read through a stored scope variable at that moment is **not yet tested**; since 2026-09-26 `te_civil_war_resolve_sides` (`te_civil_war_effects.txt`) writes the answer to `debug.log` as `TE_CIVIL_WAR:` lines the first time a civil war ends. The monetary repair (#462) adds its own `TE_CW_PROBE monetary` lines, which show whether the loser's variables can be read and copied.
- **Not yet observed:** a loyalist win (presumably the same rules with the original as the winner), secession, releases, interest-group and state modifiers.

What this breaks, and the fixes:
The mod's rule (owner ruling, 2026-09-26): **a revolution's winner continues the nation.** It keeps everything the loser had — memberships, rewards, stocks, running programmes and the player's policy choices — so the fixes below restore the loser's state rather than start the new regime afresh. The first round is #460–#465 (audit F1–F7).

- **State held only in modifiers.** Anything a system records solely as a modifier — a toggled policy, a membership, a one-off reward — is gone. Mirror it in a country variable and rebuild the modifier from the variable: a country modifier from `on_civil_war_won`; a journal-entry modifier from the entry's own `immediate` and a self-heal in its pulse, because the order of `on_civil_war_won` against the inherited entry's `immediate` is unknown. Mod examples: the UN's membership, seat and programmes (#461), the civil-rights policy buttons (`cr_policy_*`), the space-race choice and probe modifiers (`sr_sync_*`).
- **A timed or decaying reward.** A running timed modifier is lost with the rest, and re-adding it with its original `days` hands the winner a fresh one — worse still when the key is a permanent title (`is_world_first_<tech>`) whose reward ran out decades ago. Record when it was granted (a month index beside the title, or a months-since counter), and at `on_civil_war_won` re-add only the time left. For an `is_decaying` modifier also pass `multiplier = <fraction left>`, so it resumes at the strength it had reached. Give `days` / `months` a **literal**, branching over a ladder of fixed durations if needed: no `add_modifier` in vanilla or this mod takes a computed duration, the engine docs give the field no signature, and an ignored one would make the modifier permanent. The engine offers no read of a modifier's remaining time. Examples: `agdiff_restore_first_mover_prestige`, `cr_rebuild_outcome_modifier`, `te_mon_cw_add_timed`.
- **A bonus the rebels should share, but only if the nation had it.** Copy it from ROOT (the original) at `on_revolution_start` / `on_secession_start` with `root = { has_modifier = X }`, not from a global "this exists in the world" flag. Otherwise a nation the grant deliberately skipped gains it by having a civil war. Mod example: `agdiff_copy_diffusion_from_root`.
- **A backstop for wars that began before the fix.** When a start-hook fix makes an `on_civil_war_won` repair redundant for new wars, don't gate the repair on a property the country can gain later (a country type, a law). Mark the rebels at the start hook and run the repair only for an unmarked winner. This works in any hook-file order: a rebel winner holds its own marker, and a loyalist inherits it in the merge. Take the side from the shared layer (`var:te_cw_rebels_won`) too, when only one side needs the repair. Remove the marker after reading it, guarded by `has_variable`. Don't clear a marker the original carries at the start hook: only a rebel still at war can hold one there. The marker answers "did these rebels get the fix?", which `te_cw_role` can't: a war begun after the shared hooks but before the fix has a role and no fix. Mod example: `agdiff_cw_copied` (`agdiff_on_uprising` / `agdiff_repair_after_civil_war`).
- **A tracker variable without its modifier.** A system that records "modifier X is applied" in a country variable (`*_applied`, `*_mod_on`, a band index) and swaps from that record will, on a winner that lacked the variable, inherit the loser's "applied" with no modifier behind it — on the country or on an inherited entry — and never re-add it. Recompute statelessly each pulse (the nuclear domestic stance does), reset the tracker in `immediate` (`je_nuclear_program` zeroes `nd_*_mod_on`; `je_banking_cycle` zeroes `te_mon_stance_band_applied`), or key a first-run sweep on the modifiers actually being absent rather than on the tracker being missing.
- **An unguarded `immediate`.** It runs again on every inherited entry: guard every set that must survive with `NOT = { has_variable = X }` (`je_banking_cycle`'s cycle variables, #465). A variable feeding `current_value` is safe to guard for inheritance, since the bar's baseline and goal are copied (above). If the entry can also re-activate, pin the goal with `goal_add_value = TARGET − current` (the space-race milestones); if its bar wraps, keep the reset for the state that can re-activate (colony-less solar colonization). `je_immediate_reset_audit` flags unguarded writes.
- **Resets a fresh run still needs.** Put them under a run-in-progress marker — `NOT = { has_variable = X_run_in_progress }`, then set it — and remove the marker in the end-of-run cleanup every end hook calls. An inherited entry carries the loser's marker, so it skips the reset (`je_civil_rights`'s `cr_run_in_progress`). Don't reset at the end instead if the outcome events read the counters after `on_complete` / `on_fail` fires them.
- **A completed entry.** Record completion in a country variable and require its absence in `possible` (`je_colonial_empire`'s `colonial_empire_completed`, `je_civil_rights`'s `civil_rights_resolved`). Set it at every terminal hook that grants something (`on_complete`, `on_fail`, `on_timeout`), not at `on_invalid`. Check `possible` too: an entry whose end condition is a persisting state (a technology, a law) doesn't just re-arm on the winner, it **ends again at once** and replays its outcome event (`je_human_augmentation`, `je_mental_health`, #463). In normal play the variable changes nothing: a finished record is never re-activated.
- **Reaching the loser, and telling which side won: the shared layer** (#467; `common/scripted_effects/te_civil_war_effects.txt`, hooks in `common/on_actions/te_civil_war_on_actions.txt`). Every system's repair is a call inside `te_civil_war_on_won`, between `te_civil_war_resolve_sides` and `te_civil_war_clear`, so no repair depends on the order in which on_action files run. `te_civil_war_resolve_sides` reads the side from the end-hook pair (`on_revolution_end`'s ROOT and `scope:target`, stored globally and consumed straight after), or failing that from the rebel's `te_cw_origin`. It leaves:
  - `var:te_cw_rebels_won`: 1 when the revolutionaries won a revolution; 0 for any secession outcome.
  - the winner's own `var:te_cw_role`: 1 original government, 2 uprising. Both sides set it at the outbreak, and the merge keeps the winner's own value. `te_civil_war_clear` strips it at every win, so the winner of a second, concurrent war may carry none and inherit the loser's.
  - `scope:te_cw_loser`, when it resolves.

  `te_civil_war_clear` removes them in the same tick. Don't grow private which-side-won pointers: #460–#465 were built with three in parallel with #467, and all were folded into the layer. A marker that answers a different question, such as "did these rebels get X at the outbreak", stays the system's own (`agdiff_cw_copied`). Reading the loser *through* `scope:te_cw_loser` is the untested step above. A repair that must act **once per loser** keeps its own marker of the object it acted on and refuses it — its own, never a shared "spent" flag, which would make repairs depend on each other's order — and removes it once that object no longer resolves (`te_mon_cw_bank_taken`: without it, a concurrent second war that names the same dead original would add its gold again).
- **Copy state, never trackers.** When the winner takes the loser's values, leave every `*_applied` / "is applied" bookkeeping variable as the winner's own. It describes the winner's own modifiers, so the system's normal swap moves them to the copied state on the next pulse. Copying the loser's tracker without the loser's modifier recreates the tracker-without-modifier bug above. A copied **stock** also needs an inheritance guard: after the merge a winner value that equals the loser's is one it inherited for lack of its own, and adding the loser's again would double it (`te_monetary_inherit_central_bank`).
- **Anything split between the two sides.** Because the winner's value takes precedence, a quantity handed to the rebels at the start (a share of an arsenal, a reserve) makes the loser's remainder vanish at the end, whichever side wins. Move it explicitly in `on_civil_war_won`, from a figure cached while both sides still exist; the merge will not add them. Two rules make that safe (nuclear custody, `nuclear_custody_effects.txt`, 2026-09-26). **Give both sides their own copy of the variable at the outbreak**, even as 0: a winner that lacks it receives the loser's value through the merge, and adding the cached figure on top counts it twice. **Cache the figure in a freestanding script container**, not on either country: `create_container` without a `parent` outlives any country, so it can be read after the loser is dead without depending on the dead object resolving. The same container serves every other way a country can end (annexation, formation), since a monthly pass can find records whose owner is no longer `is_country_alive` and hand them to whoever holds the owner's old capital region.
- **Per-regime choices** (policy settings, doctrines, pledges) pass to the new regime wholesale, are replaced by the rebels', or are lost, depending on where they live. Under the ruling above the loser's choices carry over: #462 copies the monetary mandate, delegation and rate target onto the winner. A choice that is a *law* is not copied: the rebels start with the nation's laws except the one their movement fought for, so the monetary regime, which is re-derived from the laws each pulse, follows the winner's laws. Systems not yet fixed (audit F8, F16) still behave in all three ways.

**Is a civil war still being fought? Ask the rebel country, not `any_civil_war`.** The `civil_war` scope `any_civil_war` / `random_civil_war` iterate is the civil war still *brewing* — a movement's `civil_war_progress` (vanilla's journal entries test `civil_war_progress >= 0.75`, `is_civil_war_type = revolution`) — not the war once it has broken out. The nuclear "Pull them back" lock tested `NOT = { any_civil_war = { always = yes } }` to mean "our civil war is over" and could lift the month after the outbreak (fixed 2026-09-26). Test for a live rebel instead: `any_country = { civil_war_origin_country ?= <us> OR = { is_revolutionary = yes is_secessionist = yes } }` (`nd_in_civil_war`, `nuclear_custody_triggers.txt`).
...
## `prev` Is One Scope Change Back — Re-Entering `root` in a Loop Makes It `root`

`prev` is the scope before the **most recent** scope change, and entering `root = { }`, `scope:x = { }` or `c:TAG = { }` is a scope change like any other. So inside an iterator, `every_power_bloc_member = { root = { power_bloc ?= { add_leverage = { target = prev value = 3 } } } }` does **not** target the member: `power_bloc ?= { }` was entered from `root`, so `prev` is `root`. The engine logs nothing — `prev` is a valid country — and the effect lands on the wrong country once per iteration. This shipped in `un_vote.2` and gave a bloc leader "Bloc gains 3 Leverage in <itself>" twenty times over. Name the iterated scope instead (`save_temporary_scope_as = member`, then `target = scope:member`), which also survives later edits to the nesting. `prev_scope_audit` (`--strict` in CI) flags a `prev` that can only mean a scope named two blocks up inside a further scope; on its first run it also found three irredentism gates where `ROOT = { any_primary_culture = { is_primary_culture_of = PREV } }` compared ROOT with itself.
...
- **A goal is only reachable through a play type.** `common/diplomatic_plays/` is where a play gets its starting `war_goal`; `settings = { skip_build_list }` additionally removes the goal from the "add a war goal" list. If you want a player to *start* a war over a custom goal, ship a `dp_*` play type for it (vanilla `dp_return_state`, `00_diplomatic_plays.txt:129`), otherwise the goal is only ever a secondary demand in somebody else's play. A play type's `possible` sees `scope:target_country`; its `selectable_in_lens` does **not** — and do not put country-state triggers there at all: with `has_variable = … / var:… ?= { has_tag = … }` in `selectable_in_lens`, the UN mandate play never appeared for a country that satisfied it, and deleting the block fixed it (play-tested 2026-09-18). Vanilla only ever writes `always = no` in it. Use it as an off-switch and gate in `possible`.
...
- **The `scope:journal_entry` saved scope *propagates* into events `trigger_event`-fired from a JE context** (immediate / `on_monthly_pulse`). So a `type = country_event` fired from a JE can call JE-scoped effects on it directly: `scope:journal_entry = { set_bar_progress = { name = X value = root.var:Y } }`. Vanilla precedent: `lands_of_anarchy.1` (fired from `06_morocco_lands_of_anarchy.txt` immediate) reads `scope:journal_entry.scripted_bar_progress(...)` in its options without re-saving the scope. Use this for resyncing a `set_bar_progress`-driven bar after an event option mutates the bar's backing variable — which is what `heir_education_events.1/.2/.3` do for `heir_education_progress_bar`, matching `je_heir_education`'s own `on_monthly_pulse`.
- **Correction (2026-09-18): `je:<je_key>` IS valid script**, and is the right tool when `scope:journal_entry` is not available — a decision's `possible`, a scripted button's `effect`, a JE's `immediate`, or a console-fired event. An earlier version of the bullet above claimed the token "only appears as a comment, never valid script"; that is wrong. It resolves to a journal_entry scope from country scope and takes both triggers and effects: `je:je_colonial_empire ?= { "scripted_bar_progress(colonial_stability_bar)" >= 65 }`, `je:je_world_war ?= { has_modifier = ww_home_front_strain_modifier }`, `je:je_colonial_empire = { add_modifier = { name = colonial_empire_crumbling_modifier } }`. In-repo: `common/decisions/extra_decisions.txt:251`, `je_colonial_empire.txt`'s phase-modifier ladder, `banking_policy_triggers.txt` (×60), `extra_script_values.txt`'s global-warming reads. **Guard it** — `?=`, or an outer `has_journal_entry` — because the accessor errors when the country does not have the entry (see the § "Lifecycle" note further down). Prefer `je:` over `scope:journal_entry` in any helper that several kinds of call site share, so the one helper stays valid everywhere.
- **A `scripted_progress_bar`'s value is readable only by trigger.** `scripted_bar_progress` is a *trigger* (`journal_entry` scope) with no script-value form, so script cannot put the bar's number into a variable. Consequences worth planning around: any widget or history chart that needs the bar as a *number* must get it from a script value the bar itself consumes (see § "Journal-Entry Widgets"), and a comparison ladder (`else_if` on `>= 100, >= 95, …`) is the only way to snapshot the value at all — `common/scripted_effects/colonial_empire_display_effects.txt` buckets it to the nearest 5 in 21 branches for exactly this reason.
...
## Keep once-per-tick state advances out of effects that buttons also call

A running average, a counter, a decay step — anything that must advance exactly once per pulse — must not live inside an evaluator that a scripted GUI or button also calls "so the panel reacts on the same click". Every click would advance the state again (five stepper clicks = five weeks of averaging). Split it: a `*_tick_*` effect the pulse calls (advance, then evaluate) and the bare evaluator for click paths, with the evaluator reading the stored state through a `has_variable`-guarded script value that falls back to the live value, so the first evaluation before any tick still works. In-repo example: the Strategic Reserve's price smoothing — `st_res_policy_tick_good_effect` (weekly pulse) vs `st_res_policy_evaluate_good_effect` (policy panel), both reading `st_res_<good>_price_signal`. Seed such an average from the live value on first run and `remove_variable` it on reset, never zero it — 0 usually means something ("at base price").
...
## Every Ship Type Needs `ai_ship_category` (since 1.14.4)

1.14.4's AI fleet rework added `common/ai_ship_categories/` (`battle_line`, `coastal_defense`, `screens`, `torpedo_craft`, `carriers`, `transports`), and vanilla's `ai_ship_categories.md` says every ship type must belong to exactly one. The field goes on the ship type, next to `ship_group`: `ai_ship_category = ai_ship_category_screens`. It is not a parse error when missing. The engine logs `ship_type.cpp:385: Ship Type is missing ai_ship_category! Ship Type: <key>` once per type in `error.log` and keeps loading. On 2026-09-23 all 19 mod ship types lacked it, and every 1.14.4 session crashed on the month tick at the same fault address. That made it the prime suspect, and the field was added to all 19. Choose the category by combat role, not by `ship_group`, which only drives construction, laws and presentation. Vanilla's `destroyer` is `ship_group_torpedo_craft` *and* `ai_ship_category_torpedo_craft`, but a ship whose job is screening belongs in `screens`. The mod's roles in the `extra_ship_types.txt` header map as follows: capital → `battle_line` (carriers → `carriers`), escort → `screens`, raider → `torpedo_craft`, troop → `transports`. The one deliberate exception is `swarm_coordination_vessel`, labelled "capital/support", which is `screens` because its 700 screening is the highest in the roster. Getting escorts right matters more than it looks. The mod's `is_obsolete` INJECTs are meant to retire vanilla's light cruiser and destroyer at `sonar`, which would leave the escorts as the AI's only current-generation screens. Whether the AI's build choice honours `is_obsolete` is unverified. When you add a ship type, grep `error.log` for `ship_type.cpp` after the next launch.
...
no-maintenance settings, which remove it).
...
10. **Event loc can't reach a container in a saved scope.** `TopScope` has `sCountry`/`sState`/… but no script-container accessor, so copy numbers the event text shows onto a ROOT country variable in `immediate` (`[ROOT.GetCountry.MakeScope.Var('x').GetValue|0]`). A GUI `datacontext = "[Scope.GetScriptContainer]"` can read the container directly.
11. **Use tags for status** (`add_tag = x_passed` / `remove_tag = x_voting`) rather than flag-valued variables. `has_tag` works in triggers, `tag = x` filters `every_container`, and GUI gets `ScriptContainer.HasTag('x')`.
...
14. **A saved container scope does not survive a helper that re-saves the same name.** Anything that runs `covert_ops_sync_all` (e.g. `covert_refresh_funding_state`, and so every priority step) overwrites a caller's `scope:iw_op`, because `covert_op_refresh_detection` does `save_scope_as = iw_op` per container. A caller that needs its container across such a call must hold it under a distinct saved-scope name and re-save it before each use: `scope:iw_debug_op = { save_scope_as = iw_op }` in `te_debug_covert_max_priority`. Found when the slice-3 console harness's second step moved a different operation than its first.
15. **A timed variable on a container never expires.** `set_variable = { name = x days = N }` is accepted on a container, and `has_variable = x` is true afterwards, but the engine never counts the time down: the save stores the remaining days on the variable, and on a container that number stays at N for good, while on a country it falls. Nothing is logged. Use a month count instead: `set_variable = { name = x_months value = <named script value> }`, count it down from a global monthly pulse, and `remove_variable` it at 0. Found in a 1996 save where every UN resolution back to No. 1 still showed 1825 days on `un_res_cooldown`, which kept every topic that had ever been voted on (a charter reform, sanctions, peacekeeping, …) on cooldown for good (`un_resolution_cooldowns_tick` in `common/scripted_effects/un_vote_effects.txt` is the fix). The same bug meant a mandate's 60-day binding grace (`un_mnd_grace_months` now) never ran out. Vanilla uses no containers, so it gives no example either way. `container_timed_variable_audit` catches this; it is `--strict` in CI.
...
Containers are also the way to store a *history* of a value. Vic3 variables hold one value and variable names cannot be built from an index at runtime, so "the last 240 monthly readings" has no variable-family expression that isn't 240 generated names. The pattern lives in `common/scripted_effects/te_history_effects.txt`; the parts worth reusing:
...
Treaty articles need four keys, all engine autokeys off the article name and none referenced from script: `<article_id>:0 "Display Name"` (the picker row), `<article_id>_desc` (the closing line of the article tooltip), `<article_id>_effects_desc` (the tooltip's bullet list) and `<article_id>_article_short_desc` (the one-line summary on the treaty draft, the only one where `[SOURCE_COUNTRY…]` / `[TARGET_COUNTRY…]` resolve). A missing one renders as the raw key, with nothing in the logs: `nuclear_disarmament` and `nuclear_program_pause` sat in the treaty picker under their raw names, and thirteen more articles had no short desc, until 2026-09-24. `loc_coverage_audit` has checked all four for mod-introduced articles since then.

### Diplomatic Action Notification Keys

A diplomatic action's notifications are localized through engine autokeys off the action name, and which ones render depends on the action's body. An action without `requires_approval = yes` notifies its target through `<action>_action_notification_name` / `_desc`, and a `pact` adds `_action_notification_break_name` / `_desc`. **`should_notify_third_parties = yes` also notifies every observer, through `<action>_action_notification_third_party_name` / `_desc`** (plus `_third_party_break_name` / `_desc` with a pact). An approval action uses `_proposal_notification_*`, `_proposal_accepted_*` and `_proposal_declined_*` instead, plus `_proposal_third_party_accepted_*` / `_declined_*` when it notifies third parties. A missing key shows up as a raw-key notification with nothing in the logs. `nd_nuclear_ultimatum_action` and `nd_repudiate_pledge_action` shipped that way until 2026-09-25. `loc_coverage_audit` has checked this family since then, and a test holds its rules against every localized vanilla action. The flag's default is undocumented and vanilla leaves it unset on actions with and without observer loc, so set it explicitly on new actions. The observer text reads best in third person with both parties named (`[INITIATOR_COUNTRY.GetName] has … [TARGET_COUNTRY.GetName]`), as vanilla's `rivalry` and `force_regime_change` do.

### Directed Treaty Articles: the SOURCE Concedes

A directed article is listed in its **source's** column of the treaty designer ("Their Articles" are what they give), so the party an article restricts is its source: put the restriction in `source_modifier`, have the demanding target pay `maintenance_paid_by = target_country`, and word `_article_short_desc` from `[SOURCE_COUNTRY…]`. This is vanilla's convention for enforceable concessions (`trade_privilege`, `foreign_investment_rights`) and the mod's (`forced_disarmament`, `imposed_currency_peg`, `debt_receivership`), and a war goal enforcing the article has the loser concede. `nuclear_disarmament` and `nuclear_program_pause` shipped inverted, on a note claiming war-goal enforcement needed `target_modifier`, so "Nuclear Program Freeze" under the USA's articles froze the *other* country's program. The follow-on events and the pause's AI score had assumed the source all along, which exposed the inversion. Fixed 2026-09-24. Mod articles also consistently treat ROOT in `visible` / `possible` as the source: the aid articles gate `visible` on the giver's bloc principle, and `suppress_subject_liberty`'s `possible` is `is_subject_of = scope:other_country`. That is inferred from the mod's own articles and not confirmed from the engine, so directional tests still belong in `can_ratify` behind an explicit `scope:source_country` (see **Lessons from monetary policy phase 5** below).
...
### Forming a Country Can Switch On Vanilla Content Meant for Someone Else

Forming a country adds every culture in the tag's `country_definitions` `cultures` list to its primary cultures, and `change_tag` leaves a country's journal entries as they were. Together they can put a mod formable in front of vanilla content written for a different kind of country. Vanilla's Turtle Island entry (`je_unite_the_nations`) shows to any North American country with a `heritage_group_indigenous_american` primary culture while TIS does not exist, and its "Proclaim Turtle Island" button is a bare `change_tag = TIS`. UNA's list carried Métis (`heritage_subarctic_indian`), so a UNA formed from the USA got the entry. The tag change deleted UNA, the same country formed UNA again, and the AI alternated between the two tags, stacking a `unification_prestige` modifier each time (seen in five consecutive 1.14.4 autosaves, 2026-09-26). Before settling a formable's culture list, grep the vanilla clone for checks on each culture's heritage group (`git grep -n "heritage_group_<x>" common events`). To keep a vanilla entry off a tag, gate its `is_shown_when_inactive` *and* add an `invalid`: an active entry without `can_deactivate` stays active after its show triggers go false. Worked example: `common/journal_entries/te_vanilla_turtle_island.txt`.
...
- **`effect_trigger_validity_audit` now scans `common/diplomatic_actions/`, `common/script_values/` and the other script-bearing entity dirs (#295).** The removed `has_war_exhaustion` survived the 1.14 migration inside `nuke.txt` because the audit's scan roots stopped at events + scripted helpers + on-actions; `SCAN_ROOTS` covers 16 directories now, so a removed trigger there flags on the next `POST /reload`. A directory *not* in `SCAN_ROOTS` still needs the manual grep on a vanilla bump.
...
- **Per-actor writes to a shared world value need weighting.** The conversion made every member's flat ±N event delta land on one number. A micro-state's gesture then moved world UN authority exactly as much as a superpower's, and a hundred-odd members' event choices swamped the systematic drift. Weight each actor's contribution by its size, or route acts through decaying stocks that the single monthly updater reads. UN redesign phase 1 does both: `un_actor_weight` is the actor's share of world prestige, and the ledgers are in `un_authority_effects.txt`.
...
**`#…#!` formatting codes are NOT required to balance within a single loc value.** Vic3 routinely **splits a formatting run across concatenated loc fragments** — one key opens `#R` (e.g. `..._prefix_up_red:0 "#R +"`), a sibling key supplies the `#!` reset — and tolerates defensive `#!#!` double-resets. A vanilla sweep finds 2341 values with "unbalanced" `#…#!` out of 1.15M. So **do not lint `#…#!` for balance**: the engine handles the fragment concatenation and ignores surplus `#!`. The render-breaking loc bug is bracket-style markup (`[b]…[/b]`, `[/i]`, any `[/x]`), which `loc_render_audit` flags; `#…#!` imbalance is not a bug. (Established when scoping #134 — the balance check was implemented, measured against vanilla, and dropped.)

**Common-word concepts are easy to *mis*-link to the wrong meaning (homonym misuse).** Concepts link only via the two explicit forms above — there is **no** plain-text auto-linking (vanilla concepts define no aliases) — so a misuse only happens when an author deliberately wraps a word whose Vic3 concept means something else. Examples caught (May 2026): "genetic `[concept_screening]`" / "psychological screening" linked the **naval ship-screening** concept; `[concept_bolster]` used as the plain verb "to bolster" linked the **political-movement Bolster action**; `[concept_dominion]` in a religion name linked the **subject-type Dominion**; "patriotic/speculative `[concept_fervor]`" linked the **cultural national-spirit stat**; adjectival "radical law/movement" `[concept_radical]` linked the **Radical pop type**. The engine renders these silently (the ref is valid, just semantically wrong) — `concept_reference_audit.py` won't catch them since it only checks existence. **Rule:** before wrapping a common English word in `[concept_X]`, confirm the concept's `_desc` actually matches the sentence's meaning; if not, use plain text. Full inventory + findings: `docs/audits/concept_usage_audit.md` (regen via `scripts/analysis/concept_usage_inventory.py` + `render_concept_usage_audit.py`).
...
### Invalid effects / functions that no-op or cascade (2026-05-24 log-triage batch)

Four more "looks valid, isn't" cases, each confirmed against engine-docs + vanilla:

- **`add_authority = <n>` is not an effect.** Authority is a **capacity** (a flat reservation pool — every active decree/suppression/monopoly reserves a chunk), **not a stockpileable resource**, so there is nothing to add to instantaneously. The only way to change authority is the `country_authority_add` modifier, which raises/lowers the *cap*. Use a timed static modifier: the mod's pair is `te_authority_drain` (`country_authority_add = -100`) / `te_authority_boost` (`+100`), applied `add_modifier = { name = te_authority_drain days = normal_modifier_time }`. `add_authority` reports `Unknown effect add_authority` (inconsistent_effect_scope). The modifier is simply the correct mechanism — not a workaround — since an instant authority delta is conceptually meaningless.
- **`negate(X)` is not a script-value function.** `add_treasury = negate(sv_x)` parse-errors (`Cannot read [negate] as a script value`) and **cascades** into "Unknown effect (" / "Unknown effect )" / "Unexpected token: <next-event-id>" on following lines — looks like 4 bugs, is 1. Use `{ value = sv_x multiply = -1 }`.
- **`has_role = <type>` is wrong; use `has_role_of_type = <type>`.** `has_role` expects a specific `character_role_*` key (vanilla: `has_role = character_role_emperor_of_japan`); passing a role *type* like `general` makes it `PostValidate ... returned false` (a soft validity warning, not Unknown-trigger). For "is this character a general," vanilla on_actions use `has_role_of_type = general`.
- **`any_scope_treaty` requires a country scope.** It iterates in-force treaties binding the *scoped country*, so calling it bare inside a treaty article's `can_ratify` (where the ambient scope is the treaty) throws `Inconsistent trigger scopes (treaty vs. country)`. Vanilla always wraps it in a country scope (`scope:first_country = { any_scope_treaty = { ... } }`, `00_alliance.txt`). In a mod article, wrap in `scope:source_country ?= { any_scope_treaty = { binds = scope:other_country ... } }`.
...
## Vanilla 1.13.9 Load-Time Validation Wave (Behavior Changes, Not Just Logging)

1.13.9 added validations that turn previously-silent script into refused effects or new warnings. Found via the first post-1.13.9 log triage (2026-07-03); date any "new" error class against pre-patch `error.N.log` generations before diagnosing (`grep -c <signature> error.*.log` — 0 hits pre-patch = new validation, not new breakage):

- **`create_building` now validates the state owner's building-unlock tech** and REFUSES ("\<country\> must have invented \<tech\>", `jomini_script_system.cpp:247`). Pre-1.13.9 the building was created anyway. Any history/script seeding of buildings into tech-less countries must grant the prerequisite tech first (see `build_effects.txt` for the pattern).
- **Active laws are validated against `unlocking_technologies` at load** ("Country X is not permitted to retain law Y", `country_law_manager.cpp:464`). Warning-only — the law is kept. Fires for deliberate historical seeds and for first-in-group auto-assignment (below).
- **`set_institution_investment_level` validates the institution is active** ("invalid institution type") — surfaced 18 vanilla-history schools errors; investigation tracked in issue #226.

## First Law in a Lawgroup File Is the Unseeded Default

For any country history doesn't give a law in some group, the engine auto-assigns the **first law listed in the group's file order** — regardless of `unlocking_technologies`. Mod lawgroups ordered by progressiveness/menu position (most-authoritarian first) therefore hand every 1836 country the tech-gated late-era law at init. This mod's convention: keep menu ordering, and seed the intended 1836 baselines in `common/history/extra_history.txt` GLOBAL (`every_country = { activate_law = ... }`, "executed last among all history"). Final state is correct; 1.13.9's retention validation logs a transient warning per country per gated-first group (registered as known noise, `mod_known_noise.md`). When adding a NEW lawgroup: either order the tech-free baseline first, or add it to the GLOBAL seeding block — otherwise unseeded countries sit on the gated law until the history block runs.
...
## Event `ai_chance` Uses MTTH Syntax — a Named Script Value in a `modifier`'s `add` Is Unproven
...
A trap for anything hooking per-resolution consequences into the UN request topics. `un_vote.2`'s aid and peacekeeping branches do `every_country = { limit = { has_modifier = un_humanitarian_aid_modifier } … }` — that is *whoever currently carries the marker*, which includes donors from earlier resolutions (the marker decays over `long_modifier_time`) and, for `un_peacekeeping_contributor_modifier`, every country running the standing peacekeeping programme. Awarding anything there pays the same country again on every future request.

The per-country, per-resolution site is `un_vote.3` instead: `un_vote.2` fires it once per member, an event runs exactly one option, and the two options that reach `un_vote_apply_resolution_compliance` are mutually exclusive by their own triggers. That is also where pledges are **charged**. `un_vote.2` used to levy the costs itself, but it runs *before* `un_vote.3` pledges anyone, so a fresh pledger paid nothing until some later request carried. Pre-fix saves can therefore still hold a peacekeeping marker with no cost modifier, so any "is this programme actually running" test must require the cost modifier as well as the marker (see `un_standing_program_active_*` in `common/scripted_triggers/un_standing_triggers.txt`).

**Don't apply a GDP-scaled journal-entry modifier from inside an `every_country` loop in an event.** The old `un_vote.2` peacekeeping levy did exactly that, and it failed in two ways. Its multiplier was a script value reading `gdp`, which returns `'none'` in journal-entry scope. The `root.var:X` fix doesn't help inside such a loop either, because `root` is the event's recipient, not the iterated country. Charge the cost where ROOT *is* the paying country: a scripted button, the country's own journal-entry pulse, or a country event such as `un_vote.3`.

## Diplomatic Obligations Can Be Created and Removed From Script

`set_owes_obligation_to = { country = <country scope/tag> setting = yes|no }`, **scoped to the country that owes** — so `c:GRE = { set_owes_obligation_to = { country = c:RUS setting = yes } }` means Greece owes Russia. Vanilla uses it at `game/common/history/diplomacy/00_favors.txt:3` (the historical favours), `game/common/decisions/france_savoy.txt:97` (granting one), and `game/common/diplomatic_actions/02_obligation_actions.txt:27` (`redeem_obligation` clears it with `setting = no`). The matching triggers are `owes_obligation_to = <country>` and `is_owed_obligation_by = <country>`; both are booleans, so a country owes at most one obligation to any other country, and `setting = yes` over an existing one is a silent no-op.

That makes an obligation the natural engine-native currency for "you owe me a favour" in a mod deal: the AI already values it, the diplomacy UI already shows it, and the recipient already has a vanilla action to cash it in. Two cautions. The effect's own documentation entry in `effects.log` misnames the key in its example body (`set_owes_obligation = { … }`); the real key is the heading, `set_owes_obligation_to`. And a separate, *engine-side* obligation mechanic exists on diplomatic actions — `can_use_obligations = yes` lets the proposer spend a favour the target already owes to force a deal through, with `use_obligation_chance` steering the AI. If your deal *grants* an obligation, set `can_use_obligations = no`, or a proposer can cash in one favour to buy another.

## A Diplomatic Action Is the Right Vehicle for "Pick Any Country, and Let Them Answer"

Scripted buttons cannot prompt for a free choice of target, which normally forces the "deterministically pick the best candidate" pattern. A diplomatic action solves three problems at once, and needs **no GUI or category registration** — dropping a file in `common/diplomatic_actions/` is the whole of it (the mod's nine covert-warfare actions are the in-repo precedent).

- **`requires_approval = yes`** gives a *human* target the engine's own accept/decline request. No popup event of your own, and a mod deal that a player on the other side can refuse.
- **`ai = { accept_score = { … } }`** is itemised: each `add` carries a `desc = "LOC_KEY"`, and the engine renders the whole list as the acceptance tooltip. That is a "why would they say yes" display that can never drift from the numbers the AI actually uses. `uses_random_approval = yes` turns a positive score into a percentage chance instead of an automatic yes.
- **Omitting the `pact` block** means no pact is created — a one-shot action. Template: `game/common/diplomatic_actions/03_violate_sovereignty.txt`.

**Scopes flip between blocks, and it is the main way to get this wrong.** In `selectable` there is no target at all. In `potential` / `possible` / `accept_effect` / `decline_effect` / `ai.will_propose`, ROOT is the initiator and the target is `scope:target_country`. In `ai.accept_score`, ROOT is the **AI being asked** and the proposer is `scope:actor` (`diplomatic_action.md`: "the AI country is always root"); relations there are read as `"relations(scope:actor)" >= relations_threshold:cordial`, and the `desc` loc lines address the evaluating country as `[SCOPE.GetRootScope.GetCountry…]` and the proposer as `[SCOPE.sCountry('actor')…]`.

`diplomatic_action.md`'s syntax block is slightly stale: it documents the execution effect as `effect = {}`, but every vanilla action and every mod action uses `accept_effect = {}` (plus `decline_effect` for approval-requiring ones), and the AI's obligation keys are `use_obligation_chance` / `owe_obligation_chance` rather than the `use_favor_chance` it lists.

**`effect_trigger_validity_audit` has no schema for `common/diplomatic_actions`,** so legitimate fields there (`can_use_obligations`, `decline_effect`, …) are flagged as unresolved helper calls. Cross-check the field against vanilla by hand and suppress with an inline `# REVIEWED YYYY-MM-DD:` on the line, the same as the diplomatic-play fields in `te_un_mandate_play.txt`.

## `grep` Silently Returns Nothing on the Modding-Digests `effects.log`
...
- **A number the engine rolls against cannot be shown if it lives in a write-just-before-use proxy.** The space race set `sr_base_risk` from a `$RISK$` parameter one statement before `random = { chance = sr_failure_weight }`, so between pulses the variable held whatever the last milestone had written and the GUI had nothing truthful to read — the player was choosing between Safe and Ambitious without ever being shown the risk. Put the literal in a per-entity script value, roll `chance = <sv>_$ENTITY$` (placeholder substitution works on script-value *names*, not just literals), and have the widget read the same value. Where the roll is conditional, give the display a thin wrapper rather than a second formula: the space race's `sr_risk_shown_<m>` is `sr_risk_pct_<m>` forced to zero while the cooldown suppresses the roll, so the panel can never claim a risk the pulse will not take.
- **Keep customizable-localization target keys as plain text.** The branch keys a `GetCustom` family resolves to are the one place in this mod where no scope expression has ever been used (the only `[...]` in any of the Strategic Reserve's 27 target keys is a game concept). Custom loc is also evaluated for countries that have opened nothing, which is why every branch needs a `has_variable` guard. Put the numbers in the **outer** widget loc key instead, where `[JournalEntry.GetCountry.MakeScope.ScriptValue(...)]` is confirmed working, and let the custom loc pick the words. In the space race that turned what would have been ~45 per-milestone branch keys into 5 shared ones.
- **Building a display state can be cheaper than displaying the raw variable.** A guarded script value (`value = 0`, `if = { limit = { has_variable = x } add = var:x }`) is three lines and makes a widget read safe in every state, including an old save that predates the variable. Use one wherever the widget touches a variable that a completion or cleanup path can remove — the alternative is a "Failed to fetch variable" line per frame for whichever month the variable is missing.
...
Two more from the nuclear programme panel (2026-09-18):

- **A `random_valid = no` customizable-localization block evaluates every branch's trigger until one matches, so an unguarded `var:` read in an early branch fires for every country that reaches the block.** On a JE with `is_shown_when_inactive` that includes countries whose entry never activated and which have none of the system's variables, i.e. a "Failed to fetch variable" line per branch per frame — the same trap as `.gui` gotcha #14, one layer further in. Pair every variable read with `has_variable` in the same trigger (`AND = { has_variable = X  var:X = N }`); scope-typed globals use `global_var:x ?= { … }`.
- **A branch that describes a *stopped* system cannot read the variable the stopped pulse was writing.** When the condition that blocks a system also deactivates its journal entry, the derived status variable freezes at its last value, so those branches have to test the live modifier (or the shared `possible` trigger) instead, and must come first in the block. That is not a re-derivation of display state — it is the only correct source, because the single derivation site is no longer running.

## Moving a Loc Key From `status_desc` Into a Widget Changes Its Accessor Root

A journal entry's `status_desc` renders with `ROOT` = the journal entry, so its keys read `[ROOT.ScriptValue('x')]` / `[ROOT.GetCountry.GetModifier…]`. A custom widget has no such root: the same reading must become `[JournalEntry.GetCountry.MakeScope.ScriptValue('x')]` (gotcha #20 confirms `.MakeScope.ScriptValue`, `.MakeScope.Var` and `.GetCustom` in-game). Neither form errors visibly in the wrong context — the value simply renders blank — so every key a JE-widget refactor lifts out of `status_desc` needs its root rewritten, and **a key still reachable from both surfaces must carry no country accessor at all**; split it in two if it does. Customizable-localization targets and `$KEY$`-nested keys inherit their caller's context, so they belong in that no-accessor class by default, while an absolute accessor such as `GetPlayer` is safe anywhere (which is what the covert funding tooltips use, since both the hidden JE button and the widget render them). Moving 16 `je_iw_*` keys this way, the check that catches a mistake is *which surfaces can reach this key*, not whether the line looks right — audit reachability, not spelling.

## Project a Threshold Into an Enum Variable Rather Than Comparing In `.gui`

`.gui` can compare a script variable against a literal (`GreaterThanOrEqualTo_CFixedPoint(…, '(CFixedPoint)12')`), and that is how the covert-operations rows originally decided an operation's phase — which put the system's 6- and 12-month phase boundaries in a second, silent place. The fix generalises: have script write the *classification* into its own variable (`iw_phase` = 1/2/3, plus `iw_phase_months_left` for the countdown) at the one site that already recomputes the underlying number, and let the `.gui` test identity with `EqualTo_CFixedPoint` (35 vanilla uses) instead of re-deriving the band. The threshold then exists only in the scripted trigger that defines it. One consequence to plan for: a projected variable is *new*, so containers or countries restored from an older save lack it until the next pulse, and a `.gui` read of a missing variable logs "Failed to fetch variable" every frame — gate the new lines on an `is_shown`-only scripted GUI that asks script whether the variable has landed yet.

## Loc in a JE's `status_desc` / `reason` Is Re-Evaluated Every Frame — Never Put a Sweep There

A `[… .ScriptValue('x')]` in a journal entry's description-family loc is evaluated on **every frame the panel is open**, not once per tick. That makes an iterating script value in one of those strings a per-frame world sweep. `je_global_warming_reason` had fourteen such reads, three of them world-scale: `global_greenhouse_gas_emissions_script_value_display` (an `every_state` over the whole map), `market_greenhouse_gas_emissions_script_value` (three `every_scope_building` passes across a market) and eight separate `gw_countries_with_*` values, each an `every_country` with a journal-entry `has_modifier` lookup — roughly 1,200 modifier lookups plus two global iterations per frame.

The fix is a cadence split, not a cache: **snapshot exactly what iterates, and read everything cheap live.** Write the iterating figures into variables from a pulse that is *already* evaluating them (the yearly `global_warming_update_on_action` was already computing the market figure to accumulate the global total, so storing it there costs nothing — set the variable first and let the accumulation read the variable, and the sweep is still evaluated once). Collapse N counting sweeps into one iteration that increments N counters. Then expose each stored value through a guarded O(1) reader (`value = 0` + `if = { limit = { has_variable = x } add = var:x }`) — unguarded reads are the "Failed to fetch variable" spam that gotcha #14 in the GUI guide describes, because a JE with `is_shown_when_inactive` is evaluated in the scope of countries that never ran a pulse. Keep live only what is both cheap and must react to a click: a `modifier:` read or a handful of `has_modifier` tests are fine, and they are *more* correct live, since `add_modifier` is invisible inside the effect block that applied it and a variable written from a button tail would lag a tick.
...
Two consequences worth knowing before designing such a panel:

- **Loc roots differ by context, so a shared customizable-localization block must be accessor-free.** A key reached from `status_desc`/`desc` resolves the country as `ROOT.GetCountry`; a key reached from a `.gui` `text =`/`tooltip =` resolves it as `JournalEntry.GetCountry` (gotcha #20). No root is right for both, so a block called from both contexts must carry no country accessor at all — put the numbers in the calling key. Keeping each block to exactly one context, and saying so in its header, is the cheaper discipline.
- **`immediate` runs again on every activation.** A JE that can deactivate and re-activate re-runs its `immediate`, and so does every entry a revolution's winner inherits (§ "What a Civil War's Winner Inherits"), so any `set_variable` there silently resets state the player accumulated — `je_nuclear_program` used to delete a country's entire nuclear arsenal when it regained great-power standing. Guard the variables that must survive with `NOT = { has_variable = X }`. But a variable that feeds `current_value` needs care on **re-activation**: the progress bar's goal is computed at activation as `current_value + goal_add_value` (vanilla `journal_entries.md:392`), so carrying leftover progress into a re-activated record gives a non-zero `GetBaselineValue` and an inflated goal, and the bar reads negative the next time the value wraps. Pin the goal with `goal_add_value = TARGET − current` where the target is fixed, or keep the reset where the bar wraps. An inherited entry is not affected: it keeps the loser's baseline and goal (§ "What a Civil War's Winner Inherits").

`je_immediate_reset_audit` checks this on every `POST /reload` and in CI (`--strict`). It flags a write in a mod journal entry's `immediate`, directly or through the mod's scripted effects, that is not idempotent (`set_variable` to anything but `yes`, `change_variable`, `remove_variable`, a list clear) and does not sit under `limit = { NOT = { has_variable = <that variable> } }`. A positive `has_variable` test is no guard, since "remove it if it is there" still destroys it. Three kinds are listed without failing. The first is a reset of a variable that feeds `current_value`: these are follow-up candidates to guard, with the goal pinned where the record can be evaluated again (above), and each listing says whether `goal_add_value` already subtracts that variable. The second is a write inside a helper the entry's own pulse runs every time. The third is a write that only runs while another variable is missing. A deliberate reset, such as the tracker of a modifier the new record no longer carries (`je_nuclear_program`'s `nd_*_mod_on`), takes `# REVIEWED YYYY-MM-DD: why` on the write, on its `if` line or on the call line.

## A Modifier Applied in `je:` Scope Cannot Be Removed in Country Scope
...
**Worked case — swapping one member of a banded modifier family.** `banking_cycle_apply_stance_band` has to keep exactly one of `banking_stance_band_1..5` on `je_banking_cycle`. It decides entirely from two variables: the band the monthly update computed (`te_mon_stance_band`) and a stored `te_mon_stance_band_applied` recording which one is physically on the entry. The remove branch keys on the *stored* value, the add branch on the *new* one, and nothing reads `has_modifier` anywhere. Two details generalise: (a) run the whole swap only when the two differ, so an unchanged month costs nothing; (b) the stored tracker can disagree with the entry it sits beside, so reset it wherever the entry is known to be empty, and have the reset value clear the whole family. A revolution's winner inherits the entry **with no modifiers** and the loser's tracker (when it had none of its own), so the tracker claimed a band that was not there (§ "What a Civil War's Winner Inherits"). The original first-run sweep fired only when the tracker was *missing*, which a revolution never produces, and the band stayed off for good (`docs/audits/civil_war_inheritance_audit.md` F7). The fix (#465): the entry's `immediate` runs exactly on a first activation and on an inherited copy. It sets the tracker to 0, unguarded, and a tracker of 0 makes the swap remove all five before adding. Clearing on 0, rather than removing nothing, keeps the reset harmless even if `immediate` ever re-ran on a record that kept its modifiers. Removing and re-adding one modifier in the same block is safe: the phase modifiers do it every month. Saves already hit got a one-time migration keyed on a marker variable (`banking_stance_band_healed`), because their `immediate` had already run.
...
## An IG Law Stance Placed on a Baseline Ideology Vanishes in the Countries That Swap It Out

An interest group's stance on a law comes from the ideologies it holds, and `common/interest_groups/00_*.txt` reassigns those per country: `00_landowners.txt` alone removes `ideology_paternalistic` and adds one of eight flavoured replacements (junker, papal, scholar, republican, magnatial, japan, carlist, moderantist) depending on the tag, and swaps `ideology_hierarchic` for `ideology_japan_hierarchic` in Japan. `00_rural_folk.txt` swaps `ideology_agrarian` for `ideology_agrarian_jeffersonian` in the USA; `00_intelligentsia.txt` swaps `ideology_liberal` for `ideology_liberal_modern`.

So a stance added to a baseline ideology alone is missing in exactly the countries with the most flavour — and it fails silently, because the swap is ordinary script and the stance is simply absent rather than wrong. When adding a law-group stance in `ideology_modifications.py`, grep `add_ideology =` in each affected IG file and decide *per variant* whether the stance carries over: `ideology_agrarian_jeffersonian` inherits an agrarian stance (it is the same constituency), but `ideology_modernizer` — the Bhadralok's replacement for `ideology_reactionary` — should not inherit a reactionary one. Note also that `apply_ideologies.py` **silently skips** a `modifications` key that names no vanilla ideology, so a typo costs nothing and produces nothing; confirm each new key exists in the vanilla `common/ideologies/` files first.

Two IGs can also cancel each other's stance inside one interest group, since an IG holds several ideologies at once. Check the net per law across an IG's whole `ideologies` block before concluding a new stance will be felt.

## One `# REVIEWED` Comment Suppresses Every Audit That Reads That Line

The suppression comment is keyed to a *line*, not to an audit. On an event's opening line, `te_debug_monetary.6 = { # REVIEWED 2026-09-20: console-only test event …` is read by `orphaned_event_audit` (which is why it is there) **and** by `loc_coverage_audit`, which also takes its exemption from the entity-opening `<name> = {` line. Since 2026-09-21 it is also read by `event_image_audit`, which takes its exemption from the same line — so that one comment can hide an orphan, a missing loc key and a missing event image at once. So a console-only or otherwise deliberately-unfired event silently stops reporting missing loc keys the moment it is marked reviewed for orphanhood — a genuinely missing `.z` option string then shows up only as a line in the `## Reviewed Exemptions` section of `docs/engine/loc_coverage_report.md`, never in the `/reload` warnings array.

`event_context_audit` (2026-09-25) avoids this by design: its suppression is **check-tagged** — `# REVIEWED YYYY-MM-DD (<check>): rationale`, with the check name between the date and the colon, anywhere in the event block — so the other audits' regex, which wants the colon straight after the date, never reads it, and one tag hides one check. New audits should take the same shape — and add their check name to `FOREIGN_CHECKS` in `event_context_audit.py`, which reads every tagged comment inside an event and otherwise reports another audit's tag as a misspelled check (failing `--strict`). `silent_variable_audit` (2026-09-25) is the first: its `(silent_variable)` tag sits inside an event option.

Caught in the phase-2 monetary harness: `te_debug_monetary.6.z` was never written, and the reload was clean. When an entity carries a `# REVIEWED` comment on its opening line, read the audit *reports* rather than trusting the warning count — and for a new event, check its loc keys by hand (`grep '<namespace>\.<n>\.' localization/english/`) before calling it done.

## `activate_law` From Script Fires `on_law_activated` — and With It This Mod's Law-Consistency Sweep

Vanilla's own comment on the hook (`00_code_on_actions.txt:5008-5022`) says it plainly: `on_law_activated` runs "when the law is activated for **any** reason… script can activate laws directly without going through the enactment process, thus bypassing… `on_law_enactment_ended`". So a script `activate_law` skips every `on_law_enactment_pass` hook (in this mod, `te_banking_law_change_cleanup` and its siblings) but runs all five `on_law_activated` hooks in `extra_on_actions.txt:449-457` — including **`te_fix_inconsistent_laws_from_law_scope`** (defined in `extra_on_actions.txt`), which calls the generated dispatcher `te_fix_inconsistent_laws` and through it the **37** per-lawgroup helpers in `extra_law_consistency_generated.txt`.

The consequence is that a cheat, a test harness or a scripted grant **cannot assume the law it just activated is the law the country ends up with**. The sweep bounces any law whose `disallowing_laws` / `unlocking_laws` are violated to the closest-progressiveness valid alternative, and it runs *between* your `activate_law` lines, not after the block. Phase 2's monetary harness hit exactly this: `activate_law = law_national_bank` on a `law_traditionalism` country was bounced to `law_no_national_bank`, which then bounced `law_fiat_currency` to `law_commodity_money`, and the "pin" silently produced a country with no policy dial at all. Two rules that came out of the fix:

- **Order the activations so the blocking law goes first**, then re-check. Settle the economic system before the bank, the bank before the currency.
- **Verify at the end, with a trigger, not a `custom_tooltip`.** An option's preview tooltip is evaluated against the *pre-click* state, so it cannot report whether the activation held. Re-fire the event and branch its title with `first_valid` instead.
...
The failure is silent and enormous when the multiplier is GDP-scaled. `sv_money_flow_event_small` is `gdp × 0.0001` — of order 10,000 for a great power — so adding a `country_inflation_pressure_add = 0.01` (1pp) field to a minting modifier applied that way would have produced thousands of percentage points of inflation pressure. The fix is a second, **unscaled** static modifier granted beside the first at every application site (`monpol_currency_stability_pressure` / `monpol_currency_devaluation_pressure` in `law_enactment_modifiers.txt` are the worked example), with a comment on the companion saying *never apply this with `multiplier =`*. Before adding a field to an existing static modifier, `git grep` its name and check how it is applied — this is the same shape as the existing rule under **Expense Scaling with GDP** ("when a modifier has BOTH expenses and gameplay effects, split into two"), generalised.

## A Repeated Key in a Generator's Python Dict Silently Drops the Earlier Entry — `ruff` F601 Is the Guard
...
## `remove_list_variable` Does Not Preserve List Order — It Swaps the Last Element Into the Hole

A variable list rendered by a GUI `datamodel = "[...GetList('x')]"` is drawn in the order the engine stores it, and `GetList` is the only accessor: `data_types_script.txt` has no sorted or reversed variant, and the GUI cannot reorder a datamodel. So for any list whose *order* is meaningful on screen, the stored order has to be right.

Appending keeps it. Removing from anywhere but the end does not. Measured against a 1865 save of the banking history store (240 monthly samples per country, one evicted per month once at cap): every store that had ever evicted held its samples as `[the newest k months][the oldest 240 - k]` — exactly one break, both halves internally chronological, `k` = the number of evictions since that list first filled. Every store still under the cap, which has never evicted, was perfectly chronological. That signature is "move the last element into the removed slot and shrink", and it means one eviction per month silently rotates the list by one more each month. On the 20-year chart it put the most recent years at the *left* edge; the 1-year and 5-year views survive only while `k` exceeds their window, because their filter selects by month index but cannot move the bars.

The fix is to rebuild the list in the intended order after every mid-list removal — `common/scripted_effects/te_history_effects.txt` `te_history_sort_samples`. Three things that rebuild needs:

- **Guard every `clear_variable_list` on the scratch list with `has_variable_list`** — it does not exist before the first rebuild, and the clear at the end may remove it again each time.
- **Build the sorted copy into a scratch list first, and clear the real one only once the copy is known complete.** Clear-then-refill loses the whole store if the iteration ever comes back short, and the containers it pointed at become unreachable *and* undestroyable — they are saved forever.
- **`min = 0  max = <cap>  check_range_bounds = no`, not a bare `order_by`.** What a bare `order_by` with no `position` iterates is undocumented and untested here: of 304 `ordered_*` iterators in vanilla 282 set `position`, and the handful that set neither only `save_scope_as` a single result — so nothing in vanilla relies on a bare `order_by` to walk a whole list, while `min`/`max` with an effect body does have precedent (`events/japan_events/ep2_iwakura_events.txt`). `check_range_bounds = no` keeps a shorter-than-`max` list from erroring.
- **Sort on the way out as well as the way in**, so the result does not depend on what order the untouched iterators hand elements back in.

Verify it against a real save rather than by eye: `python3 scripts/analysis/check_save_history_order.py` unpacks the newest `.v3` and prints `ascending=` per store. Confirmed in-engine on a 1868 save: every live store came back `ascending=True` one game month after loading the fix. Note the rebuild-on-removal shape only heals a list that is still being written to — a list that stopped changing while scrambled stays scrambled, so if that state is reachable by the player you need a second heal path. A non-ironman save is a 24-byte header plus a zip whose `gamestate` is Clausewitz binary, and mod variable names survive in it as plain strings, so any script-held list can be read back the same way.

## A Visible Event With No `event_image` Renders a Magenta Blob, Silently

The default event window has an image panel and no fallback art. An event that renders in it without an `event_image` block draws the engine's missing-texture placeholder — a flat magenta shape where the picture should be — and the engine says **nothing**: no `error.log` line, no `debug.log` line, no parse warning. Unlike a missing `icon`, which at least leaves a `VFSOpen Error: … not found` line in `debug.log`, a missing image block is completely invisible until you watch the event fire in a real game. `monument_events.3`–`.10` shipped that way for eight days and were found by a player screenshot.

Vanilla treats this as non-negotiable rather than stylistic. Of its 2,185 visible events (1.14.3), 153 carry no `event_image` — and every one of those supplies art some other way: a `gui_window` whose layout has no image panel, plus `left_icon` / `right_icon` character portraits (`acceptance_events.1` → `event_window_1char_tabloid`). The only event in the entire base game with neither is `test.120` in `test_events.txt`.

`event_image` takes **multiple blocks, each with its own `trigger`**; the first match wins and an untriggered block at the end is the fallback. That is the only clean way to picture something whose look depends on game state — vanilla's `ig_petitions.txt` uses a seven-way religion ladder for the `law_state_religion` event, mirrored by `monument_events.4`. Note the fallback block must be genuinely untriggered: a ladder whose every branch has a `trigger` renders the blob for any state that matches none of them, which is the same bug with a narrower blast radius.

`event_image_audit` enforces the rule at parse time (`docs/engine/event_image_report.md`, `--strict` in CI). It checks that art is *declared*, not that it resolves — a mistyped `texture` path renders the identical blob, so verify new paths against `git ls-files 'gfx/event_pictures/*'` for `.dds` and the vanilla install for `.bk2` videos. Inventory and the `texture`-vs-`video` tradeoff: `docs/guides/event_creation_guide.md` § Available Event Videos.

## Lessons from monetary policy phase 5 (cross-country state, treaty articles)

- **In a script value, `min =` is a FLOOR and `max =` is a CEILING.** `max(a, b)` is therefore written `value = a  min = b`, and `min(a, b)` is `value = a  max = b` — the opposite of what the words suggest. Caught in `te_mon_credibility_c` (imported credibility = max(own, 0.8 × anchor's)), where the first draft's `max = { … }` silently *capped* every anchored country's credibility at the import. Nothing logs; the number is just wrong.
- **Two scripted triggers that ask each other a question across a scope change recurse.** "A is anchored if its anchor has a dial" + "B has a dial if it is not anchored" is a call chain as long as the chain of anchors, and unbounded on a cycle in saved data. Keep one of the pair a **stored-state read** (`te_mon_is_anchored`: kind > 0 and the variable exists) and enforce the invariant from one effect that runs first in the pulse (step 1c). Same rule for effects: `te_mon_effect_union_exit` = `_exit_costs` + the discovery, and the discovery calls only the `_costs` leaf.
- **Copy a foreign country's figures into local variables once, then read the copies.** `var:te_mon_anchor = { save_scope_as = x }` followed by `scope:x.var:te_policy_rate` / `scope:x.<script_value>` in a `set_variable` works and avoids nested `prev`. Everything downstream — script values, customizable localization, the `.gui` — then reads a plain local `var:`, which also keeps an absent scope variable out of GUI reads entirely.
...
A game rule reaches script through `has_game_rule`, reaches production methods through its `disable_<pm>` / `force_<pm>` flags ("Locking Production Methods" above), and reaches countries through `apply_modifier = <player|ai|all>:<static_modifier>` in a setting (vanilla `game_rules.md`): a static modifier on every country of that category for the whole game, with no script and nothing to refresh. Prefer it to a scripted pulse for any flat per-setting bonus. Defines and building groups have no rule gate at all. `free_market_construction_rule` shows how far that goes (`docs/systems/mod_systems.md`, "Free Market Construction off"):
...
- **A downsizeable government building the AI depends on needs a guard against the AI.** In direct games play-testing saw the AI remove every Construction Site it had, and a country without a site may have no construction to build one with. Script cannot steer the AI's downsizing, and `downsizeable` is a plain flag (no trigger), so taking it away takes it from players too. Undo the AI's removals after the fact instead. Record each state's level (and PM) in state variables every week, for players as well, so that a player's own downsizing is never undone. Put back what an AI country lost, with exceptions where shedding is legitimate (`in_default`) (`te_direct_construction_keep_ai_sites`). Give the rule setting a small flat base through `apply_modifier` (above) so that a country with none still has something to build with (`te_direct_construction_base_modifier`). To raise an existing building without firing its workers, use `create_building` with the target level (the engine documents max semantics), then check the level and rebuild exactly if it overshot, since the semantics are disputed ("Building Lifecycle Effect Gotchas").
...
## Parameters Are Text: Never Pass `PREV` or a Bare `var:` Into a Scripted Trigger or Effect

`$ENEMY$` is substituted as text before parsing, so the argument is re-read wherever the placeholder sits — including inside the helper's own scope changes. `nd_core_threatened_by = { ENEMY = var:nd_crisis_opponent }` expands to `any_scope_state = { is_target_of_wargoal = var:nd_crisis_opponent }`, which reads the **state's** variable; `ENEMY = PREV` means a different scope at every nesting level (inside `source_country = { … }` it is the treaty article). Neither fails loudly. Save a temporary scope and pass that: `var:nd_crisis_opponent ?= { save_temporary_scope_as = nd_def_opp }` then `ENEMY = scope:nd_def_opp`. `save_temporary_scope_as` works inside triggers (vanilla `guarantee_independence`'s `possible` does it). The nuclear-deterrence helpers do this throughout (`common/scripted_triggers/nuclear_deterrence_triggers.txt` header).
...
- **Hand a dispatcher's parameter to its event as a saved-scope marker.** `save_scope_as = sr_setback_$MILESTONE$` before `trigger_event` lets the event and every option ask `exists = scope:sr_setback_probe` — which milestone failed — where "any running milestone" or a flag two journal entries can raise in one month cannot (`sr_fire_ambitious_failure_event`, #429). The marker lives for the script run, so never dispatch twice in one run: each journal entry's pulse is its own run, a console helper looping over milestones is not. Related: journal entries that fail for one shared cause each run `on_fail` on their own update (`JOURNAL_ENTRY_UPDATE_ACTIVE` = 4 days), so dedupe the report with a pending variable and a longer delay (`sr_report_program_loss`).
- **The global `on_monthly_pulse` has no root, so a shared effect it reaches must not read `ROOT`.** Vanilla's `00_code_on_actions.txt` says so ("No root scope set"). A scripted effect written for a button or an event, where ROOT is the acting country, loses that country when a global pulse calls it inside `some_scope = { … }`: `value = ROOT` writes nothing useful, and `this = ROOT` compares against nothing. Save the actor from the current scope instead (`save_scope_as`, `value = THIS`). `un_found_organisation` and `un_charter_invitations` do this for the UN's refounding path (`un_conference_monthly_update`). There, `un_hq_country = ROOT` would leave the new UN with no headquarters host until the next month's upkeep picked one; this follows from vanilla's comment and was not seen in game.

## A Second Path to an Action Must Share the Action's Gates

- **Repeat a diplomatic action's gates on every other path to the same outcome, through one scripted trigger.** The nuclear crisis deadline's "carry out the threat" (`nuclear_crisis.4.g`, `.7.a`, `.20.b`) dispatched a strike through the same effect as `nuke.txt`, but checked only doctrine and pledges. The Rules of War gate lived inline in the diplomatic action's `possible`, so a country under Limited War could strike through a crisis. `nd_war_law_permits_strategic_strike` now serves both paths.
- **A trigger that reads ROOT answers for ROOT, whatever scope you call it from.** `is_losing_war_against` compares `ROOT`'s won-battles share. The crisis pressure formula called it in the *target's* scope from the issuer's weekly pulse, so "the target is losing to us" fired when the issuer was losing battles. When a helper must work from a non-ROOT scope, write a twin that saves `this` as a temporary scope and reads that (`nd_is_losing_war_to`).
