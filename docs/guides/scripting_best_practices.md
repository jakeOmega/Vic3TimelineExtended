...
The per-country, per-resolution site is `un_vote.3` instead: `un_vote.2` fires it once per member, an event runs exactly one option, and the two options that reach `un_vote_apply_resolution_compliance` are mutually exclusive by their own triggers. Note the ordering consequence too — `un_vote.2`'s levy runs *before* `un_vote.3` pledges anyone, so a fresh pledger carries the contributor marker with **no** cost modifier until the next request carries. Any "is this programme actually running" test must therefore require the cost modifier as well as the marker (see `un_standing_program_active_*` in `common/scripted_triggers/un_standing_triggers.txt`), or a free marker will read as a live commitment.
