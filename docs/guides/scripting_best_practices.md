...
**`set_variable`'s lifetime field takes a *value*, not a boolean — and `days` / `months` / `years` are alternatives to `value`, not companions to it.** The fields are `name` plus **either** `value = <number-or-script-value>` (a stored number) **or** one of `days = N` / `months = N` / `years = N` (a timed flag that auto-clears after that span). All three time units are valid: the vanilla election idiom is `set_variable = { name = election_event_cooldown months = election_event_cooldown_months }` — 36 sites in `modern_election_events.txt` / `repeatable_events.txt` use it, with `election_event_cooldown_months` a *vanilla script value* (it is in `docs/engine/effect_trigger_valid_keys.txt`) — and `religious_revival_events.txt` uses `set_variable = { name = rre_recent_cooldown years = 5 }`. **Do not "fix" these into `days = N`.**

What actually parse-errors is mixing the two forms or passing a boolean: `set_variable = { name = X value = 6 months = yes }` fails with `Named value not found: yes: yes` (the parser sees a `value = 6` pair plus an orphan `months = yes`). Pick one form. If your system already decrements the variable itself in an `on_action` monthly pulse (e.g. `space_race_on_actions.txt`'s `change_variable subtract = 1` on `sr_failure_cooldown`), drop the time field entirely and store the months-count in `value` — don't double up engine cleanup and manual decrement.
