...
**Event timing at game start is not proven.** Vanilla ships no `trigger_event` from `on_game_started` to copy (its own block is empty and `on_game_started_after_lobby` uses only direct effects). The mod's working precedent for fanning an event out to every country at game start is a **GLOBAL history file** — `GLOBAL = { every_country = { trigger_event = { id = … } } }` in `common/history/te_construction_market_global.txt`, no delay, effect visible on day one. Vanilla itself uses `trigger_event` with an explicit `days = N` from history files (`common/history/global/00_global.txt`). If a no-delay `on_game_started` dispatch turns out not to deliver, move the fan-out to a GLOBAL history file; a self-healing marker bounds the damage to the first monthly pulse either way.
...
### CONFIRMED (nested `modifier = { }`): an `INJECT:` block **sums** with vanilla's

**CONFIRMED IN-GAME 2026-09-19 (owner, PR #335) for nested `modifier = { }` blocks on country RANKS and on TECHNOLOGIES: they SUM with vanilla's.** The read: a great power's budget-panel interest tooltip no longer lists the rank's −50%, because the mod's `+0.5` cancel-INJECT nets it to zero — and the five vanilla finance techs' −2% is likewise cancelled. That settles §17 check 4 outright and the rank half of check 1, and with them the thirteen cancel-INJECT blocks monetary policy phase 1 added in `common/country_ranks/te_monetary_rank_injections.txt` (eight ranks) and `common/technology/technologies/te_monetary_tech_injections.txt` (the five finance techs). **LAWS were not separately read** — `common/laws/te_monetary_law_injections.txt` (`law_laissez_faire`, the other half of check 1) and check 3's `law_industry_banned` are the same nested-block case on a third entity type, so they are very likely the same, but that is inference, not a read. Confirm laissez-faire's −25% on the same tooltip when convenient. It also retro-justifies the *summing* assumption the mod has shipped on since April (`common/laws/sol_expectations_vanilla_injections.txt`, e.g. `law_industry_banned`: `state_expected_sol_mult = 0.1` against vanilla's −0.1).

**STILL UNVERIFIED: a FLAT KEY inside a static modifier** (§17 check 2). `INJECT:base_values` demonstrably works — the mod has shipped bureaucracy, authority and innovation contributions through it since April, so whole-block last-wins is ruled out — but *same-key* semantics there are a separate question from the nested-block case above, and `common/static_modifiers` is not even on the digest's INJECT-capable type list. Two live instances ride on it: `state_expected_sol_from_literacy = -5` against vanilla's `+5`, and (added 2026-09-19) `country_loan_interest_rate_add = -0.2` against vanilla's `+0.2`, both in the mod's `INJECT:base_values` block. The interest one has the cleanest day-one tell on a fresh 1836 game: **no "Base Value" line in the budget interest tooltip and Britain near 3.4% / Russia near 12% ⇒ the keys sum; every country reading 0.0% ⇒ last wins.** Record the answer here when it is read.

What is known: the 1.12 digest's INJECT-capable type list (`~/src/Modding-Digests/1.12.0/inject_types.md`) includes `common/country_ranks`, `common/laws`, `common/technology`, `common/institutions`, `common/company_types` and `common/production_methods`, so the open question is **sum vs last-wins**, not "is the INJECT a no-op". Note also that merge semantics are documented to vary by entity type, which is why the tech probe is separate from the rank and law ones.

The four tooltip reads that were set up to settle this, kept for reference — 1, 3 and 4 are now answered, 2 is the open one (`monetary_policy_design.md` §17 checks 1–4):

1. **A great power's budget-panel interest tooltip.** Does it still list the rank's −50% *and* laissez-faire's −25%? If yes, separate `modifier = { }` blocks merge.
2. **The breakdown of `state_expected_sol_from_literacy`.** The mod injects −5 against vanilla's +5 (`extra_modifiers.txt:17`). Three outcomes: **0 ⇒ values sum; −5 ⇒ last wins; +5 ⇒ the inject is ignored.** *Caveat:* this one is a flat key inside a **static modifier**, and `common/static_modifiers` is not on the digest's INJECT-capable list even though `INJECT:base_values` demonstrably works. Ranks, laws and techs merge a *nested* `modifier = { }` block — do not let this result stand in for them.
3. **Expected SoL under `law_industry_banned`** — does the same key net to zero across the two blocks?
4. **A technology probe.** `common/technology/technologies/modified.txt` INJECTs `modifier = { }` into vanilla techs (`intensive_agriculture`, `nationalism`, …): does vanilla's own modifier still show in the tech tooltip?

**Record any further result here, in this section, in the same session it is obtained** — it is the most reusable engine fact this area can produce, and three separate systems are currently guessing. Fallback if summing fails: `REPLACE:` the entity and restate its full vanilla modifier block — for **techs** that means restating each of the five minus the interest line, or (equivalently) going back to compensating in script and touching no vanilla entity. The monetary cancels are isolated in one file each precisely so that is a one-file fix. **The tech probe has a free tell**: the five vanilla finance techs carry `country_minting_mult = 0.1` in the same block as the interest line the mod cancels, so if minting still shows `+10%` on the tech tooltip the INJECT summed, and if it is gone the INJECT is last-wins.
