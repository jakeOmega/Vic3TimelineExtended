...
| Complied with a binding decision we voted against | `events/un_vote_events.txt:2857` (`un_vote.3` option B) | 2 | Same once-per-member-per-resolution argument. Gated on `un_resolution_is_binding` — yielding on a recommendation costs nothing, so it earns nothing. |
...
**Early withdrawal is charged from the pulse, not from the withdraw buttons.** One site then covers every exit path — the four withdraw buttons, `un_stop_championing_button`, a pledge revoked by `un_vote.2` when a request resolution fails, and anything added later — treats the AI exactly like the player, and needs no edit to the button effect blocks that `gen_un_button_descs.py` generates tooltips from. The trade-off: a withdraw button's own tooltip does not name the standing cost. The chamber's "How standing works" panel does.
...
`un_vote_proposer_standing_lean` (in `un_standing_values.txt`) is **one shared script value**, range −5 … +5, added by a single `modifier` block to `un_vote.1` option A's `ai_chance` at `events/un_vote_events.txt:476` — against a base of 40, next to the existing alliance (+20), bloc (+15) and rivalry (−30) terms. It is a lean, never a guarantee. Option B is untouched: raising A already lowers B's share of the roll, and a symmetric penalty would double a deliberately modest nudge. Nothing else in any `ai_chance` was changed.

The value guards twice — `exists = scope:un_vote_proposer` (the proposer may be annexed mid-vote) and `has_variable = un_standing` on it (a founder's first month, or a pre-feature save).

> **Caveat worth verifying in-game.** `ai_chance` in events uses MTTH syntax, and no vanilla or mod precedent was found for a **named script value** as a `modifier` block's `add` value at that level. The failure mode is benign — the lean simply does not apply and the AI votes exactly as it did before — but it would be logged. Grep `debug.log` for `un_vote_proposer_standing_lean` on the first load. If it is rejected, replace the single block with four tier-keyed blocks with literal adds (`un_standing_tier_exemplary` → +5, `_respected` → +2, `_poor` → −2, `_disgraced` → −5 on `scope:un_vote_proposer`), which needs no other change.
...
- **The AI vote hook** is at `events/un_vote_events.txt:476`, marked `<-- LOBBYING STAGE: ADD YOURS HERE`. Add a **second** `modifier` block with your own shared script value; do not fold lobbying into `un_vote_proposer_standing_lean`. The two must stay separately tunable and separately attributable, and read the caveat above before assuming either one takes effect.
