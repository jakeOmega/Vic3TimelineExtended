...
### Debug Harness
`event te_debug_covert.1` sets funding to any of the six levels; `event te_debug_covert.2` seeds four operations at months 5 / 11 / 14 / 14 (industrial espionage, influence campaign, military espionage, destabilization), forces a detection, cuts funding to 0, ages everything by a year, re-derives the rows' display state, and (2026-09, covert slice 3) raises one randomly picked running operation to maximum priority by calling the shipping step effect twice, without the stepper's click — the pick is held as `scope:iw_debug_op` and re-saved as `scope:iw_op` before each step, because the step's sync overwrites `scope:iw_op`. Files: `events/te_debug_covert_events.txt`, `common/scripted_effects/te_debug_covert_effects.txt`.
