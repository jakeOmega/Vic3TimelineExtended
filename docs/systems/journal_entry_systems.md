...
A revolution's winner inherits the loser's variables but no modifiers (`docs/guides/scripting_best_practices.md` § "What a Civil War's Winner Inherits"). The repair is `cr_repair_after_civil_war`, called from the shared `te_civil_war_on_won` (#467, `common/on_actions/te_civil_war_on_actions.txt`). ROOT = THIS = the winner, and it runs between `te_civil_war_resolve_sides` and `te_civil_war_clear`. Which side won comes from that layer:
- **`te_cw_rebels_won = 1`** (the revolutionaries won a revolution): `cr_rebuild_after_civil_war` runs. Every add is guarded by the modifier being absent.
- **The winner's `te_cw_role = 1`** (the original government won): it keeps its own modifiers, and `cr_drop_rebel_run_state` drops the policy mirrors and run marker it inherited from any struggle the rebels ran themselves. Otherwise the pulse's self-heal would turn those mirrors into policies.
...
- **Button policies.** Each button sets or removes a `cr_policy_<path>` mirror with its modifier. Only the buttons and the entry's cleanup may add or remove these modifiers; the test pins this. For a run in progress (`cr_run_in_progress`, with the entry active), the rebuild re-adds the six modifiers from the mirrors, plus `cr_cooptation_expired` when cooptation has run 12 months. The JE pulse syncs both directions too, as a safety net, and it counts a month for a mirror as well as for a modifier. The monthly country on_action cleans up a run marker left with no active entry.
- **Old saves.** The same monthly on_action sets `civil_rights_resolved` for a country running a resolution-only outcome modifier without it. The age is unknown, so it assumes 60 months. That guess only sets the strength and length of a rebuild: a migrated modifier older than 60 months runs out first, and its record goes with it. Not recognisable: `.240` option B (its modifier is shared with `.16`), `.230`–`.232`, and rewards that have already expired. A save that is mid-run gets its marker and mirrors from the JE pulse.
...
- `common/on_actions/civil_rights_on_actions.txt` — monthly: outcome-age counter, lapse check, save migration, orphan sweep (the civil-war repair hangs off `te_civil_war_on_won`)
...
### Founding Process
The UN must be actively founded by a Great Power with Intergovernmental Organizations tech via the `un_found_button`. Founding costs prestige and bureaucracy (`un_founding_cost_modifier`). The founder becomes the first member, hosts the HQ, and gains a permanent Security Council seat + `un_founding_member_modifier`. Sets `un_founded` global variable. After founding, any country that conducts its own foreign policy can join (no tech requirement): an independent country, or a dominion, protectorate or tributary (`un_membership_eligible`, `common/scripted_triggers/un_membership_triggers.txt`).

**Membership and representation** ([`un_redesign_design.md` §0.8](un_redesign_design.md#08-subjects-diplomatic-autonomy-and-suspended-representation)):
- **Suspension.** A member that stops conducting its own foreign policy (made a puppet, vassal, colony, personal union, crown land or chartered company, or decentralized) keeps its membership (`un_member_modifier`), but its representation is suspended (`un_representation_suspended`, computed live). It casts no ballot and tables nothing. The docket offers it nothing, it cannot be lobbied, it is not asked to comply or ratify, and it is not counted in the two-thirds. It holds no permanent seat, does not host the HQ and sends no contingent at will. It runs no UN programme and takes no stance: the six start buttons refuse it, and the monthly check ends any it ran. The check sits beside every Article 19 check; `test_un_membership.py` keeps the two together.
- **The overlord carries the seat.** A suspended member is never assessed dues. A represented direct overlord is assessed on its GDP (`un_dues_assessed_gdp`). While that overlord pays, the subject draws `un_member_privileges_modifier` and the authority-scaled benefits (`un_member_draws_benefits`). `un_member_modifier` itself has no effects.
- **Restoration and notices.** Representation is restored the moment the member conducts its own foreign policy again. `un_representation_monthly_update` posts one notice either way and strips a suspended member's permanent seat. A restored member gets one chance (`un_events.36`) to ratify the conventions passed while it was suspended. It was not asked to ratify them then; a snapshot taken at suspension tells those conventions apart from ones it refused before.
- **The General Assembly line.** It reads "N of M eligible nations are represented", then "K more member states have suspended representation because they lack diplomatic autonomy".
- **Non-members.** Pariah modifiers and the Supranational outsider case apply only to eligible non-members.
- **Annexation** passes nothing to the annexer.
...
- **Authority (redesign phase 1):** authority no longer drifts toward 50, and individual acts no longer add flat amounts to it.
  ...
  - **Display.** The last 12 entries are logged for display (`un_ledger_log`). The **Why UN Authority Is Moving** widget (`gui/journal_entry_widgets/un_authority_widget.gui`, container 3) shows, each under its own heading, the target, the pillars and the viewer's weight; the tier and charter; the champions, underminers and outsiders; and the recent entries — then a help text and a history chart of authority and its target.
  - **What an entry says.** Each entry names what was done and to whom. Every response to a UN event has its own reason code (`1NNk`, the k-th response to `un_events.NN`: 1041 is `un_events.4`'s full peacekeeping force, 1043 its refusal), and an entry can carry a **subject**, the country or state the act was about: `un_ledger_actor_entry_on`, `un_ledger_actor_entry_unweighted_on`, `un_ledger_entry_on` and `un_authority_actor_shock_on` take `SUBJECT = <scope>`. A state prints through `SCOPE.sState(...).GetName`, so it can be hovered ("sent a full peacekeeping force to Attica, where order had broken down"). It prints by name alone: a state's owner would be read when the log is drawn, not when the entry was booked, and a struck state or a mission's state often changes hands; the hover shows the owner. The subject travels as the saved scope `un_led_subject` and is cleared once the entry is booked, since `un_vote.2` books a dozen entries in one option. Resolution entries (codes 1–4, 12–14) need no subject: `un_ledger_log_add` notes the resolution's number, target and a reference for its topic. The old codes (1–20, 1NN) keep their lines for entries already in a save. `test_un_ledger_reason_codes.py` fails on a booked code with no line, and on a subject line that could be printed for an entry without a subject (the display's temporary scope would still hold the previous entry's).
...
- **Dues, teeth and regimes (redesign phase 5):** rulings and the in-game checklist are in `un_redesign_design.md` §0.5.
  ...
  - **Joining at will (§0.7).** Any major-power member not under sanctions may send a contingent to an active mission it does not host and is not at war with (`un_mission_volunteer_eligible` + `un_mission_accepts_volunteer`), and bring it home again. A player does it from the mission's own row in **Missions in the Field** (one row per display slot `un_msn_slot`, 0–7, through `un_chamber_mission_join_sgui` / `un_chamber_mission_withdraw_sgui`); the AI from the journal entry's monthly pulse (`un_mission_ai_monthly`, scored by `un_mission_ai_join_score`, capped at 2 missions, 3 for a great power). A volunteer is listed in `un_msn_volunteers` and pays `un_mission_service_cost` (a quarter of a percent of GDP a year per mission, refreshed by `un_mission_service_refresh`), except that the peacekeeping programme pays for its members' peacekeeping and stabilisation contingents. Bringing a contingent home costs standing 2 (reason 35) and 10 relations with the host, bars it from that mission (`un_msn_withdrawn`), and fails a peacekeeping or stabilisation mission it leaves empty. The docket's appeals and the resolutions' pledges are unchanged.
...
| `un_topic_human_rights` | Event 3 / chamber | Universal Declaration of Human Rights |
...
| `un_topic_heritage` | Event 9 / chamber | Cultural heritage program (UNESCO) |
| `un_topic_decolonization` | Event 12 / chamber | Anti-colonial declaration |
| `un_topic_npt` | Event 14 / chamber | Nuclear Non-Proliferation Treaty (IAEA) |
| `un_topic_pandemic` | Event 16 / chamber | Global pandemic response (WHO) |
| `un_topic_climate` | Event 17 / chamber | Climate accord (UNEP) |
| `un_topic_refugee` | Event 18 / chamber | International refugee resolution (UNHCR) |
| `un_topic_space` | Event 19 / chamber | Space cooperation (UNOOSA) |
| `un_topic_law_of_sea` | Event 21 / chamber | Convention on the Law of the Sea (ITLOS) |
| `un_topic_physical_protection` | Event 23 / chamber | Convention on the Physical Protection of Nuclear Material (`un_agency_cppnm`; nuclear_crisis_design.md §0.10 step 5) |
| `un_topic_icc` | Event 22 / chamber | International Criminal Court |
...
- **Cooldowns:** a closed resolution carries `un_res_cooldown_months` (60, or 120 for expulsion), which `un_resolution_cooldowns_tick` counts down from the global monthly pulse and removes at 0; `un_resolution_topic_on_cooldown = { TOPIC = x }` checks `un_resolution_history` for it. History eviction skips entries still on cooldown. It is a month count because a timed variable on a container never expires (`scripting_best_practices.md`, script containers rule 15); saves from before the fix carry a frozen timed `un_res_cooldown` that nothing reads, so on those saves every topic comes off cooldown at once. When a charter reform's cooldown lifts on a reform already ripe, the major-power members get the ripe notice then (`un_charter_reform_notify_ripe`).
...
| | 7 | Propose — Universal Declaration of Human Rights | `un_propose_human_rights_effect` |
| | 8 | Propose — International Criminal Court | `un_propose_icc_effect` |
| | 9 | Propose — Nuclear Non-Proliferation Treaty | `un_propose_npt_effect` |
| | 10 | Propose — Climate Accord | `un_propose_climate_effect` |
| | 11 | Propose — Global Pandemic Response | `un_propose_pandemic_effect` |
| | 12 | Propose — International Refugee Resolution | `un_propose_refugee_effect` |
| | 13 | Propose — Cultural Heritage Program | `un_propose_heritage_effect` |
| | 14 | Propose — Decolonization Resolution | `un_propose_decolonization_effect` |
| | 15 | Propose — International Space Cooperation | `un_propose_space_effect` |
| | 16 | Propose — Convention on the Law of the Sea | `un_propose_law_of_sea_effect` |
| | 17 | Propose — Convention on the Physical Protection of Nuclear Material (row hidden under the disabled nuclear weapons rule, `un_chamber_nuclear_rule_sgui`) | `un_propose_physical_protection_effect` |

Two display-only siblings share the same `op` contract: `un_chamber_propose_row_sgui` (op 0-17, the row's description) and `un_chamber_propose_intro_sgui` (no op). The climate accord's row is hidden under the disabled global warming rule by `un_chamber_climate_rule_sgui`, a root-only yes/no GUI (gotcha #22). **The op table is repeated in the widget, in the sgui file and in the row renderers — keep all three in step.** `test_un_chamber_propose_ops.py` fails if they drift.
...
- **Why the proposal list is eighteen GUI rows and not one script-built block.** Tooltip generation prints a scope's own `custom_tooltip` lines before the lines its nested country-scope blocks produce (found in the first play test — it is why `un_chamber_ballot_lines` now makes every entry self-describing instead of putting a header above each side). Structure that has to interleave therefore cannot come from ordering inside one effect. Each row is its own widget with its own `ExecuteTooltip` call, so its topic, rule, target preview and status stay together whatever the renderer does with order. The target-preview lines print at country scope rather than inside a country block. Like `je_un_chamber_proposer` / `_target` / `_vetoer`, they print the name with `GetName` alone, which already carries the flag; an explicit `GetFlagTextIcon` doubled it in game (`gui_modding_guide.md` gotcha 19).
- **Which topics are proposable.** All eighteen, listed above (charter reform joined in phase 2; the docket also offers it through `un_events.6`, under the same gates). The eleven conventions (ops 7-17: human rights, ICC, NPT, climate, pandemic, refugee, heritage, decolonization, space, law of the sea, and — since 2026-09-26 — the physical protection of nuclear material, offered only once warheads have gone missing) became proposable later; before that they reached the floor only when the docket offered them to a delegation (redesign phase 4, `un_docket_effects.txt`) through their proposer events in `events/un_events.txt`, and the chamber listed them under "Raised as Assembly business" with no control. The docket still offers them, a human delegation first, and each row says when.
- **The conventions share their gates and their terms with the docket.** Each has `un_propose_<key>_in_force` (its agency founded, or for decolonization its regime standing — the row says "in force" and the button stays disabled), `un_propose_<key>_qualifies` (the per-country conditions: technology, rank, not already a party, no recent refusal) and the docket's own `un_docket_topic_open_<key>` (not in force, not on cooldown, the authority threshold). `un_docket_may_propose_<key>` is now `un_docket_can_be_offered` plus `_qualifies`, so the docket and the chamber ask the same questions, and the docket's clauses carry the tooltips the Propose button shows. What tabling does — the IG reactions and the vote — is `un_propose_<key>_table`, which the proposer event's first option calls too. The ICC waits for the Declaration of Human Rights on our own motion, as the docket's Assembly business does. The refugee resolution names the country the displaced flee; its selector (`un_propose_refugee_select_target`) is shared with `un_events.18`'s `immediate` and previewed in the row, deterministic for a human and random for the AI, as condemnation's is.
- **Own motion costs; the docket's offer does not.** Tabling a convention from the chamber takes `un_ga_resolution_modifier` (5 years) and `un_request_cooldown` (10 years, shared with every other own-motion proposal) through `un_propose_own_motion_costs`, shown in the button's tooltip rather than folded into a custom line. Accepting the docket's offer stays free: the docket paces the floor. This is the charter-reform precedent (`un_events.6` free, `un_propose_reform_effect` not).
- **No scripted buttons for the conventions, deliberately.** The AI reaches them only through the docket. A button with an `ai_chance` would let it skip the docket's pacing (redesign §8.1, §8.3), so the Propose control lives in the chamber alone, which the AI never presses (`ai_is_valid = { always = no }`).
- **Adding a convention.** About thirty sites in twenty files list the conventions by hand: the propose triggers and effects; the docket's triggers, offer rows and Assembly business; the chamber's three op switches, widget row and row renderer; the topic and consequence lines; `un_vote.1` and `un_vote.2`; the treaty modifier and its preview; `un_apply_ratified_conventions`; the §0.8 snapshot, forget and missed trio and `un_events.36`; the regime layer; leaving and dissolution; the ledger's topic and reason cases; the AI's `un_lean_interests`; the §0.9 mirrors; and the loc. **`test_un_convention_registry.py` is the executable form of this list:** add the convention's `CONVENTIONS` row first, and the failures name every site still missing. A convention that exists only under a game rule (the climate accord) names the rule and its row-visibility GUI in its row, and the gate is checked at every entrance. Comments and docs that count the conventions in words are checked too.
...
- **Timing.** Only what the containers store: `un_res_months` (months in session / months it sat) and `un_res_seq` ("Resolution No. N"). A closed resolution still carrying `un_res_cooldown_months` gets a "cannot be tabled again for another N months" note.
...
- **Cost.** `ExecuteTooltip` re-renders every frame the widget is visible, so the archive sits behind a collapsed-by-default toggle, and its voting details behind a second one nested inside the archive's panel (`un_chamber_subsection_header`, set in from the panel's edge so it reads as part of the archive rather than as the next section).
- **Headings.** Inside each section, every group of lines sits under its own heading (`un_chamber_subheader`, `un_auth_subheader`): the top panel's standing / Security Council / business before the Assembly, the proposal list's censure / requests for help / charter / docket / Assembly business, and so on. They are GUI rows over one scripted GUI per group, because a script-built "heading, list, heading, list" prints both headings first (`gui_modding_guide.md` gotcha #18). A heading over a group that can be empty is hidden with it through a yes/no scripted GUI that reads only the root: `un_chamber_founded_sgui`, `un_chamber_member_sgui`, `un_chamber_in_session_sgui`, `un_chamber_mandate_case_sgui`.
...
**Cooldown divergence, deliberate.** `un_resolution_topic_on_cooldown` is topic-wide: one member's mandate vote would bar every other member for five years and leave the feature dead on a busy map. The mandate topic opts out and throttles per country instead. `un_resolution_archive` still stamps `un_res_cooldown_months` on the closed resolution (that is what keeps it in the history), but nothing consults it for this topic.
...
The chamber's ballot block (`un_chamber_ballot_lines`, called by both `un_chamber_active_voters_sgui` and `un_chamber_history_details_row`, one row per archived resolution, so it covers the resolution in session *and* the archive's voting details) gained `un_chamber_commitment_lines`: a **Commitments** sub-list, headed "undertakings, not votes", printing pending / delivered / broken as three disjoint groups with the same guarded flag-and-name accessor the ballot uses. The whole block is hidden unless a commitment was ever made on that resolution, which is also what keeps it invisible on every resolution in a pre-feature save.
...
- **Handlers:** `common/scripted_guis/nuclear_deterrence_sguis.txt` — two action handlers (`nd_posture_sgui`, `nd_crisis_action_sgui`) and fourteen display handlers: `nd_armed_sgui`, `nd_umbrella_sgui` (the umbrella line), `nd_in_crisis_sgui`, `nd_crisis_opponent_sgui`, `nd_last_crisis_sgui`, `nd_has_reputation_sgui`; `nd_crisis_we_issued_sgui`, `nd_crisis_we_are_target_sgui` and `nd_crisis_private_sgui`, which hide the crisis moves our side cannot make; `nd_crisis_pressure_breakdown_sgui`, `nd_crisis_danger_breakdown_sgui` and `nd_crisis_next_pressure_sgui`, which print script-built text from the figures `nd_crisis_refresh_figures` stores on both parties; and `nd_home_list_sgui` and `nd_home_footer_sgui`, the At Home list and its footer. All carry `ai_is_valid = { always = no }`: the AI sets its posture in `nd_ai_review_posture` and answers crises through the crisis events, which call the same effects.
...
Areas (posture): **Nuclear Posture** (open) — doctrine, readiness (with the step under way), authority, the nuclear umbrella (covered subjects, shown only while there are any), weekly upkeep, incident exposure band. **Doctrine** — five choice rows, each tooltip listing the doctrine's modifier through `GetStaticModifier(...).GetDesc`. **Readiness and Launch Authority** — four and four choice rows (Recessed to High Alert; Central Authorization to Automatic Retaliation); readiness tooltips show the weekly upkeep at that level. **Forces** — warheads (exact, with the world's estimate), survivability against its ceiling, reliability, strain, the chance a delegated sequence is halted, and the safeguards / hardening steppers. **At Home** — a legend, then a script-built list (`nd_home_list_sgui`, printed with `ExecuteTooltip` into an `nd_script_line`): each interest group with a view, its class and lean, the approval number its breakdown shows, and a second line with the terms behind it (or "not reviewed yet" before the first monthly review). A second script-built line below it (`nd_home_footer_sgui`) says "Reviewed monthly." and, while doctrine tenure is under six months, when opinions start: a closing line inside the list handler would print above the groups (gotcha #18). It is script-built rather than a `datamodel` list because a datamodel item's tooltip loses the `JournalEntry` context (gotcha #24) and no dynamic list has been proven in this mod's JE widgets (gotcha #15).
