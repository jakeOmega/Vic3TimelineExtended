...
- **`is_shown` must never read `scope:op`.** Vanilla declares `saved_scopes` beside `is_shown` exactly once (`je_meiji_restoration_get_faction_sgui`) and that `is_shown` does not read it, so the IsShown call path is unverified; a silently-false expression would leave a row with no control. The in-force question comes from the scope-free `gw_active_<policy>_sgui` handlers.
