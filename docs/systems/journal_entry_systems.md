...
- **Handlers:** `common/scripted_guis/nuclear_deterrence_sguis.txt` — two action handlers (`nd_posture_sgui`, `nd_crisis_action_sgui`) and eleven display handlers: `nd_armed_sgui`, `nd_in_crisis_sgui`, `nd_crisis_opponent_sgui`, `nd_last_crisis_sgui`, `nd_has_reputation_sgui`; `nd_crisis_we_issued_sgui`, `nd_crisis_we_are_target_sgui` and `nd_crisis_private_sgui`, which hide the crisis moves our side cannot make; and `nd_crisis_pressure_breakdown_sgui`, `nd_crisis_danger_breakdown_sgui` and `nd_crisis_next_pressure_sgui`, which print script-built text from the figures `nd_crisis_refresh_figures` stores on both parties. All carry `ai_is_valid = { always = no }`: the AI sets its posture in `nd_ai_review_posture` and answers crises through the crisis events, which call the same effects.
