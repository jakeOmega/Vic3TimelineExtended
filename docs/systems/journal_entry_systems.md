...
7. **Operation rows** — one per `iw_ops` container: type and target, phase by name with months until the next phase, a dormancy line, that operation's own detection risk with the IC-versus-defence arithmetic, a priority stepper and a line stating what the current level multiplies. Optionally a per-row **Stand down** control.
