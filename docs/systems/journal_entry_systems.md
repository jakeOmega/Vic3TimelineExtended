...
**How the AI chooses.** Every button's `ai_chance` reads `common/script_values/global_warming_ai_values.txt`. Each country has one climate will: temperature (20 per °C, capped at 70), the environment ministry (+15), laissez-faire (−20), an environmentalist leading a governing interest group (+8), industrialists in government (−8), the environmental movement (5 per 10% support, at most 10), standard of living against the world mean (±10; the mean is stored by `gw_refresh_global_counts_effect`), and a fossil-exporting market (−10). Each policy has one threshold on the will: adopt at or above it, and only with the authority it costs to spare; repeal only once the will is `gw_ai_repeal_band` (15) below it, or while authority is negative. Between the two, neither button scores. The thresholds, from cheapest to hardest, are adaptation 25, reforestation 30, renewables 35, transit 40, green building codes and emission standards 45, carbon tax 55 and divestment 70. The old weights read temperature alone and never repealed above ~0.5°C, so every AI ended up holding every policy; a 2008 save at 1.26°C had 132 of 136 countries on seven of eight. `test_gw_ai_policy_table.py` keeps each adopt/repeal pair on the same threshold. `event te_debug_gw.1`, option **l**, writes every country's will, part by part, to `debug.log` (`GW_AI_WILL`), which is how to check the thresholds against a save.
