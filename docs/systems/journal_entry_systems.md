...
> **Redesign in progress:** see [`un_redesign_design.md`](un_redesign_design.md). Phase 1 (the authority model), phase 2 (tiers, the charter, the crisis, dissolution and refounding) and phase 3 (the dossier and grounds, the itemised vote lean, AI voting in script, the recess) have shipped and are described under *Authority*, *The ladder, the ceiling and the floor* and *Grounds, votes and the recess* below. The docket and missions are still to come. Everything else in this section is the system as it stands.
...
- **Grounds, votes and the recess (redesign phase 3):** `common/script_values/un_dossier_values.txt` (the dossier, the thresholds, every term of the lean), `common/scripted_triggers/un_dossier_triggers.txt`, `common/scripted_effects/un_dossier_effects.txt` (every writer). Rulings and the in-game checklist: `un_redesign_design.md` §0.3.
  ...
  - **The lean.** Every ballot is decided by `un_vote_lean`, the sum of ten named terms (consensus, grounds, the target itself, ties to the proposer, ties to the target, glass houses, the proposer's standing, a pledge, the target's acceptance, the burden, interests). The AI no longer answers `un_vote.1`: thirty days after a resolution opens, `un_vote.5` sends every AI member the hidden `un_vote.4`, which writes its lean and casts through the same `un_vote_cast_*` effects a human uses — a veto for a permanent member on a binding topic at a lean of −30 or below (−50 after a veto of its own, −10 at Moribund), otherwise yes when lean plus noise (±20) is above 0. Human members get `un_vote.1` and see their own lean in the chamber, term by term. Each ballot's reasons are tallied on the resolution (`un_res_why_<side>_<term>`).
  - **The recess.** When a resolution closes, the floor is in recess for three months (`un_recess_months`): only human members may table business (`un_may_table_now`, in every propose trigger and every proposer event of the monthly roll). Human members are notified when it opens.
- **Other authority thresholds** (unchanged by phase 2, still read from `un_authority`):
  ...
  - **≥80:** Non-nuclear member powers face NPT disarmament pressure (`un_npt_disarmament_modifier`: `nuclear_disarmament = yes`) (requires IAEA). Reachable only after Charter Reform I.
