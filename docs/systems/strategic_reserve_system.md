...
- **Inventory rows:** one per unlocked good, five lines deep. Line 1: `@good!` icon and name, a fill bar driven by `st_res_<good>_fill_pct`, a Storing / Withdrawing / Idle / Blocked label, and decrease / stop / increase controls. Line 2: `stored / capacity`, the configured signed rate and the actual net weekly movement (`st_res_<good>_last_net`). Line 3: the annual decay rate (`st_res_<good>_annual_decay_rate`, §4.4) and this week's loss (`st_res_<good>_weekly_decay`). Lines 4–5: the reserve policy (below). The row tooltip breaks down stock, rate setting, active rate, net movement, the decay rate with its per-source breakdown, weekly decay, hub flow cap and hub staffing, then states the reason movement differs from the setting.
...
- **Collapsing a row** works the same way: clicking a good's name toggles `st_res_row_collapsed_<good>`, which hides lines 2–5 and the settings panel and leaves line 1 (bar, status, controls). Rows start expanded; the key is never saved and no script reads it. The name is an untextured `button`, so it still reads as plain text, and its tooltip (`st_res_row_collapse_tooltip`) replaces the row tooltip over the name only; the row tooltip's last line points players at it.
