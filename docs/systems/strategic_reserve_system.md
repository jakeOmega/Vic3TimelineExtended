...
`market_goods_pricier` and `market_goods_cheaper` are readable as script values in `market_goods` scope — vanilla itself does it in `common/treaty_articles/13_goods_transfer.txt` (`value = market_goods_cheaper`). What the engine docs do **not** settle is whether each is clamped at zero or is the signed mirror of the other. `st_res_<good>_price_rel` is therefore built as
