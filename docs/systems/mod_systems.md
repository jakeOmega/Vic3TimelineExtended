...
|---|---|---|
| Construction Sector | Sells the construction good; ordinary heavy industry | Idle: `te_direct_construction_sector_idle_modifier` (`building_construction_sector_throughput_add = -1` × the sector's country-level throughput factor, `var:te_dc_throughput_cancel`) stops its goods; `te_direct_construction_modifier` requires subsidies (`country_building_construction_sector_require_subsidies_bool`), so the government pays its wages as vanilla's government-funded sector's are; `te_direct_construction_no_output_modifier` (`goods_output_construction_mult = -1` × `var:te_dc_output_cancel` = 1 + every other country-level source, e.g. the nuclear crash program's −5%) makes the construction-good output exactly zero, barracks included |
| Site capacity (`te_construction_market_total_use_target`) | Government + private purchase | `var:te_dc_capacity` from `te_direct_construction_refresh_capacity`: staffed levels (level × `te_direct_construction_building_staffing` = max(`occupancy` read as a value, the sites' 10%-step staircase)) summed per sector tier, × tier points (`TE_DC_POINTS_*`: 1 / 2 / 3.5 / 5 / 6 / 10 / 16, the market PMs' output, applied once after the sum), × the throughput factor, + 0.2 per staffed level of Engineering & Logistics barracks. Same buildings, same construction as under the market |
...
**Site mode group.** The site has a mode group, `pmg_te_construction_market_site_mode` (defined once, in `extra_pm_groups.txt` — `13_construction.txt` loads first, so a copy there loses; an earlier draft of the rule put the direct tiers in exactly such a copy, where the engine never saw them). Its two PMs have no effects and no gates; each family in the base group is gated on its mode through cross-group `unlocking_production_methods`: the market base PM on `pm_te_construction_mode_market`, the direct tiers on `pm_te_construction_mode_direct` plus their sector tier's tech. There is deliberately no self-referencing ratchet: the market base PM sits on every site in every game, and a self-referencing default is unverified in game. Script enforces the mode instead:
- direct games move sites with `activate_production_method`, country-wide every pulse (`te_direct_construction_sync_site_pms`) and per state straight after a site is placed (`te_direct_construction_sync_state_site_if_direct`), since a market-mode site's input is cancelled and its points would be free;
- market games replace any site moved to direct mode by hand (`te_construction_market_reset_direct_sites`), at the **end** of the pulse, after the sites are sized. Run earlier, the replacement would be sized while unstaffed: `te_construction_market_num_sites` skips it and `te_construction_market_employment_adjusted_per_site` gives it ×10, which it keeps once it hires — a sole 100-point site would then offer 1,000. Run last, it carries no capacity modifier until the next pulse, when it has hired (both hiring rates are 1.0) and is sized normally: at worst a week of reduced construction. Replacing beats switching the PMs back, which would charge the retooling tax (+1000%) on the market PM's construction-good input. `pm_te_construction_mode_direct_desc` warns the player.

**One refresh site.** `te_construction_market_on_weekly_pulse` branches on the rule, and the pulse is the only place that refreshes the direct modifiers, the capacity and the per-site / private-share modifiers (`te_direct_construction_on_weekly_pulse`: modifiers for every country, then capacity, site PMs and `te_construction_market_country_update_data` for non-decentralized ones). Setup and border changes only place sites and sync their PMs. Two reasons: the exact-cancel modifiers read their own last contribution back through `modifier:X + var`, which a second refresh on the same day could double-count, and sizing sites right after placing them would hit the staffing compensator (`te_construction_market_num_sites` counts only staffed sites; `te_construction_market_employment_adjusted_per_site` is ×10 below 10% staffing). So a direct game's first week — and a country released mid-week — runs without the direct modifiers until its first pulse, as a market game's first week runs without its private share. Files: `te_construction_market_direct_effects.txt`, `te_construction_market_direct_values.txt`, the new PMs in `te_construction_market_pms.txt`, the modifiers at the bottom of `te_construction_market_modifiers.txt`.
...
- *Staffing.* `occupancy` read as a script value has one earlier use in the repo (`extra_script_values.txt`); if it reads as 0, staffing falls back to the 10%-step staircase, which rounds down (a 99% staffed sector counts 0.9).
- *Unverified engine behaviour — the play-test list.* (1) Market game, new and pre-rule save: a newly placed site sits on Market Construction with the market PM, and the AI never picks Direct; with a non-empty queue, switch the only site to Direct and follow reset → rehire → next pulse — construction should dip for at most a week and never exceed the purchase. (2) Direct game: a newly placed site shows Direct Construction and the right tier straight away — the mode and the tier are activated in the same block, and the tier's availability depends on the mode. (3) Direct game: capacity in the panel matches the sectors, and the site's `country_construction_add` follows its throughput modifier as under the market.
...
| `events/te_debug_covert_events.txt`, `common/scripted_effects/te_debug_covert_effects.txt` | Console harness: `event te_debug_covert.1` / `.2` / `.3` (Tradecraft) / `.4` (slice-6 operations) |
...
| `common/scripted_effects/covert_warfare_effects.txt` | Operation containers (create/destroy/monthly sync), per-target networks (`covert_nets_sync`, `covert_net_create` / `_gain` / `_loss`), duration aging, phase-based effects, election confidence |
...
- **`covert_warfare.2` keyed infrastructure sabotage on its state modifier until 2026-09 (covert slice 6)**, so a burned sabotage operation never reached the defender; it now keys on the country-scope `covert_infra_sabotage_morale`.
