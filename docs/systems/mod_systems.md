...
- *Construction minimum* (`te_direct_construction_update_floor`, weekly, after the guard). `te_direct_construction_floor_modifier` (`country_construction_add = 1`, "Minimum Construction") tops construction up to `te_direct_construction_floor` (5 points a week) with `multiplier = var:te_dc_floor_points`. The multiplier is the floor less `modifier:country_construction_add`, plus the modifier's own previous contribution, and never below 0. A country with no sites (the AI in default, a player who downsized everything, a country before Urbanization) can therefore still build. Because it is a top-up, a country whose sites already give 5 gets nothing, and a new site's first levels replace the minimum instead of cutting it off. Its points carry no wages. Whether they draw on the sites' material purchases (the engine buys the camp's inputs per point spent) is unverified.
...
- *A minimum of construction.* Direct games top every country up to 5 construction points a week (above).
...
6. *Direct game, minimum.* A country without sites reads at least 5 construction per week in the panel, with Minimum Construction making up whatever its other sources (a base, barracks) leave below 5. Downsize every site as a player: within a week construction is back to at least 5, and new sites can be queued and built. A country with more than 5 from its sites shows no Minimum Construction, and the panel figure never counts the minimum twice.
