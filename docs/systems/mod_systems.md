...
- **Reserve Requirements** (`cb_reserve_requirements`, Prudential) — a leaning tool with no tech: bubble and momentum down, capitalists' pool efficiency −5%, and `country_inflation_pressure_add` −0.5pp (read by the §9.1 pressure sum, so it shows in the dashboard's Price Pressure breakdown). AI: the buffer's weights, +15 while the buffer is still locked.
