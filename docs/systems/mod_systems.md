...
- market games replace any site moved to direct mode by hand, at the start of the pulse so the replacement gets its capacity modifier in the same pulse (`te_construction_market_reset_direct_sites`). The replacement is unstaffed at that moment, so it is sized like any site placed just before a pulse (the staffing compensator's ×10 until it hires); replacing still beats switching it back, which would charge the retooling tax on a construction-good input. `pm_te_construction_mode_direct_desc` warns the player.
