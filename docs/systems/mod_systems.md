...
- **AI policy choice** is one table, `common/script_values/global_warming_ai_values.txt`. It gives each country a climate will and each policy a threshold on it. Adopt scores at or above the threshold, repeal only 15 below it, so each AI holds the policies its conditions support rather than all of them. The will is built from temperature, the environment ministry, laissez-faire, interest groups in government, the environmental movement, relative standard of living, and fossil exports. Details and thresholds: `journal_entry_systems.md` → **How the AI chooses**. Calibrate with `event te_debug_gw.1` option **l** (`GW_AI_WILL` lines in `debug.log`).
