...
**Phase 6 is planned, not built.** A review of the three 5a articles (`monetary_policy_design.md` §15B) found no leverage, no coercive variant, and three verified exploits — multi-provider swap-line stacking against a self-inflicted crisis, serial default farming against a guarantor whose odds never move, and uncapped per-pegger prestige on the anchor. The staged plan is §15C: **6a** hardening (the draw becomes a repayable, capped loan from one provider, drawn only in a financial crisis; a ward-side `te_mon_lolr_calls` counter that escalates the honour cost, tapers the relief, shifts `te_lolr.1`'s odds and lengthens the cooldown; one managed `te_mon_arrangement_prestige` term in step 9b replacing the articles' flat prestige lines; a yearly AI re-evaluation that withdraws from monetary-only treaties; `non_fulfillment = withdraw` on war / expulsion), **6b** `country_treaty_leverage_generation_add` 200 / 150 / 300, **6c** two hostile enforceable articles, `imposed_currency_peg` and `debt_receivership`. The owner decisions it needs are tabled in §15C.5; its checklist is P6-1…13. Do not treat any of it as shipped until a §0.10 exists.
