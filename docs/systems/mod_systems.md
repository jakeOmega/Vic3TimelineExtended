...
### History Charts
`gui/journal_entry_widgets/banking_history_widget.gui` adds a third custom widget (`custom_widget_container_2` - the policy list took `_3`): a collapsed-by-default **History** section with one column chart each for cycle value, momentum and bubble pressure — plus, since monetary phase 1, policy rate and rate paid on debt (see **Monetary Policy (phase 1)**), five in total — with 1 / 5 / 10-year ranges and dated policy and crash markers. The policy-rate chart is a `visible`-toggled **pair** rather than one instance: an unsigned 0–12 axis for everyone, and a −3–12 axis with a drawn 0% line for a digital currency, picked by the `banking_mon_is_digital` scripted GUI. Only one is ever drawn. The store, the chart type and the marker tooltips are shared infrastructure — see **History Store and Charts** below before touching any of it.
...
| `common/scripted_guis/te_monetary_sguis.txt` | `banking_mon_control_sgui` (the one interactive handler, ops 0–6) + six scope-free display handlers |
...
The **gold band stays scripted** and deliberately so: reference rate ±2 is a phase-1 *interim* rule standing in for §12's gold flows (§19 row 1), not a property of the law, and it goes when reserves start moving. The **regime ceiling** (25, or 15 on gold) stayed scripted too — it is the same number for both discretionary regimes, so it distinguishes nothing on a tooltip, and gold's 15 never binds because the ±2 band is always tighter. The band itself is printed live in the Rate Target tooltip from `te_mon_target_min` / `_max`, which remain the single source shared by the monthly clamp and the stepper's `is_valid`.
...
**Open market operations, rewired.** `banking_open_market_ops` no longer touches interest at all (the dial owns the rate). It is gated on the policy rate sitting at its regime **floor** — `te_mon_policy_rate_at_floor`, i.e. at or under `te_mon_floor_threshold`, which is `min( te_mon_target_min , 0 ) + 0.01`: `-2.99` under digital currency and `0.01` otherwise, the shipped §0 ruling, now derived rather than restated. The `max = 0` in that value is what keeps a gold standard's band floor (≈1pp) from counting as the floor. It is the balance-sheet tool of last resort rather than an everyday easing lever. A gold-standard country (clamped to the reference rate ±2 in phase 1) **cannot reach it until the world reference rate falls to 2.0** — `te_mon_target_min` is `ceiling(max(reference − 2, 0))`, which is 1 at the 1836 rate of 3.0 and still 1 at 2.5, but 0 once a great power holds both `macroeconomics` and `globalization`. The loc (`banking_omo_rate_floor_tt`, "zero under a gold standard") is the accurate wording. Its `country_bubble_pressure_monthly_add` went 0.8 → **1.5**: buying assets with the rate already at zero is how speculative excess builds. Its `ai_chance` was retuned around the new gate (panic 70, downturn 45, bubble-risk-high −30) and its old "don't act while the rate hike is on" weight is gone as unreachable.
