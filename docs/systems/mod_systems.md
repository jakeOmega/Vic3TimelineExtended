...
- *Private share*: `te_direct_construction_law_private_share` restores the economic-system laws' vanilla allocations (25 / 50 / 75 / 35 / 10 %), which `construction_system_law_injections.txt` cancels for the market; keep the two in step. Applied at country setup and refreshed weekly (`te_direct_construction_on_weekly_pulse`).
...
- *No downsizing.* `downsizeable = no` is shared with market games, where a downsized site would take the country's construction with it until script re-placed it. So direct-mode sites can't be downsized by the player or the AI.
...
**Play-test list.**
1. *Market game (the default), new game and a pre-rule save.* Day 1: the capital has a Construction Site on Basic Construction Market, and states that start construction get one. Construction Sites can't be queued (the build menu may list them as unavailable). No building offers No Maintenance or a `_direct` method. Construction behaves as before this change.
