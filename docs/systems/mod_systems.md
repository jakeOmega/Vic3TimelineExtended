...
| `common/scripted_guis/covert_warfare_sguis.txt` | Widget handlers: the funding stepper, the ladder tooltip builder, four `is_shown`-only questions, nine per-type stand-down handlers, the per-row priority stepper (`covert_priority_up_sgui` / `_down_sgui`, 2026-09 covert slice 3) |
...
| `events/te_debug_covert_events.txt`, `common/scripted_effects/te_debug_covert_effects.txt` | Console harness: `event te_debug_covert.1` / `.2` / `.3` (Tradecraft) |
...
- **Gain:** `covert_tradecraft_monthly`, called **only** from the JE pulse, right after `covert_ops_age_all_durations` (never from `covert_ops_sync_all` / `covert_refresh_funding_state`, which run on every stepper click). Each operation at establishing or better adds `covert_tradecraft_gain_per_op` (1), counting at most `covert_tradecraft_ops_counted` (4) — the count is `iw_tradecraft_ops`, shown in the tooltip — scaled by `covert_tradecraft_gain_scale` = (100 − x) / 50 clamped [0.1, 1], the UN-standing shape. **Decay:** `covert_tradecraft_decay` (0.25) a month while `covert_operations_active < 1`; an agency whose operations are all still preparatory neither gains nor decays.
...
- **Balance** (corrected from the spec's closed form, which assumed every operation-month earns): detection rolls from month 0 and a burned operation is relaunched preparatory, earning nothing for up to 6 months. Simulated long-run averages (600 months; head start 0 / 5) and median months to Established: at 10 % per-op detection one moderate op averages 31 / 70 (221 / 78 months to 40), two 38 / 72 (114 / 38), four 53 / 74 (51 / 18), one severe 14 / 45 (375 / 138); at 5 % one moderate op averages 79 / 85 (74 / 51 months); at 2 %, 93 / 95. So at 10 % (funding 1, no efficiency) an agency holds Established only with several operations or a strong network; at 5 % or less the curve behaves as the spec intended. This also means an AI at low funding reaches priority 3 far less often than before slice 5. Table: `covert_warfare_script_values.txt` § TRADECRAFT.
...
- **Unlocks** (one trigger each, so each gate is one `custom_tooltip`; none ever ends something already running): `covert_tradecraft_unlocks_priority_3` (Established) — the step *to* priority 3 in `covert_possible_priority_up`, so it binds the row stepper and `covert_ai_manage_priorities` alike; an operation already at 3 stays there and stepping down is never gated. `covert_tradecraft_unlocks_severe_ops` (Seasoned) — for slice 6's regime change and nuclear programme sabotage; nothing reads it yet. `covert_tradecraft_unlocks_extra_per_type` (Veteran) — +1 in `covert_ops_max_per_type`, which only gates launching.
