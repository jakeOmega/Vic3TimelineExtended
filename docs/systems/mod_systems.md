...
2. **`banking_cycle_advance_variables`** — momentum decay, modifier-driven changes, the monetary-stance push (`te_mon_stance_momentum_add` / `te_mon_stance_bubble_add`, −0.125 and −0.75 per pp of tight stance gap, gap clamped ±4 — never rendered as a number, per `monetary_policy_design.md` §8), investment pool, random mean-reversion nudge (scaled by `banking_random_nudge_down/up_value`), apply momentum to value, clamp all variables.
...
The policy-rate dial and the risk-premium stack that replaced the old *Raise Policy Rate* toggle. `docs/systems/monetary_policy_design.md` stays the authority for phases 2–4 (inflation, QE, world rate, gold flows, FX) and carries the **"Phase 1 as shipped"** section with the deviations list and the in-game verification checklist — phase 1 is implemented but **not yet verified in a running game**.

**Files.**

| File | Holds |
|---|---|
| `common/script_values/te_monetary_script_values.txt` | the **variable contract** (file header), `te_mon_era_base`, the regime range (`te_mon_target_min` / `_max`), `te_mon_access_premium`, `te_mon_vanilla_tech_offset`, `te_mon_scaled_debt_premium`, `te_mon_cycle_lean`, and the three stance-channel values |
...
| `common/scripted_guis/te_monetary_sguis.txt` | `banking_mon_control_sgui` (the one interactive handler, ops 0–6) + five scope-free display handlers |
| `common/customizable_localization/banking_dash_custom_loc.txt` | `te_mon_stance_band_name`, `te_mon_delegation_state`, `te_mon_mandate_name`, `te_mon_no_dial_reason` |
| `common/country_ranks/te_monetary_rank_injections.txt`, `common/laws/te_monetary_law_injections.txt` | the cancel-INJECTs |
| `common/static_modifiers/extra_modifiers.txt` | `te_monetary_base_offset`, `te_monetary_rate_paid`, `banking_stance_band_1..5` |
| `common/modifier_type_definitions/banking_cycle_modifier_types.txt` | `country_credit_standing_add`, `country_risk_premium_add`, and the one permitted `REPLACE:` — vanilla's `country_loan_interest_rate_add` *type definition* at `decimals = 1` |
| `gui/journal_entry_widgets/banking_dashboard_widget.gui`, `banking_history_widget.gui` | the dial block and the two new charts |
| `events/te_debug_monetary_events.txt` | console-only `te_debug_monetary.1` — the single place hidden state is printed, deliberately unreachable from script |
...
**The rate stack.** `rate_paid_pts = clamp( policy_rate + structural + cyclical , 0.5 , 60 )`, everything in percentage points (3.5 = 3.5%). The policy rate is the dial's target drifted toward at 1/3 pp a month (2/3 under digital currency); a country with no national bank has no dial and gets `reference_rate + 1` instead. The reference rate is `te_mon_era_base` — a **world** quantity, 3.0 / 2.5 / 2.0 by era, evaluated once a month into `global_var:te_mon_era_base_world` and read per country from there.

**Two premium tiers.** *Structural* (`country_credit_standing_add`) is what the country **is** — rank, capital-market access, laissez-faire, gold-standard and CBI credibility, the national-bank institution, the mod's finance techs — floored at 0.5pp (0.25pp under central bank independence), so standing earned past the floor buys nothing by design. *Cyclical* (`country_risk_premium_add`) is what is **happening** — cycle phase, crash interventions, banking-event outcomes, `declared_bankruptcy`, the debt-load term — added *after* the floor, so it always bites. Both are read back in country scope as `100 × modifier:X`; both are `script_only` and neither is granted from anywhere the monthly update does not read.

**Engine write path.** The update's last step removes and re-adds `te_monetary_base_offset` (`country_loan_interest_rate_add = -0.2`, cancelling vanilla's `base_values` flat rate) and `te_monetary_rate_paid` (`= 0.01`, applied with `multiplier = var:te_rate_paid_applied`) **in one effect block**. The invariant is that the offset is never present without the rate modifier — otherwise the country borrows at ≤ 0. `te_rate_paid_applied` is `te_rate_paid_pts` plus `te_mon_vanilla_tech_offset` (2.0pp per researched vanilla finance tech, which are compensated in script rather than INJECTed); every *displayed* number uses `te_rate_paid_pts`. A month whose change is under 0.05 skips the re-apply.

**Single owner, and it is not idempotent.** `te_monetary_monthly_update` is the only writer of `te_policy_rate`, `te_rate_paid_pts` and the two interest modifiers. It advances the drift by a third of a point and draws two random walks **per call**, so calling it twice in a month drifts twice as fast and ages the hidden state twice as quickly. Call it only from `common/on_actions/te_monetary_on_actions.txt` — `on_monthly_pulse_country`, `on_game_started`, `on_country_released_as_{independent,own_subject,overlord_subject,company_subject}`, `on_revolution_start`, `on_secession_start`, `on_country_formed` (plus `on_monthly_pulse` for the world reference rate and `on_yearly_pulse_country` for the growth term) — **never** from an event, a scripted button, a scripted GUI or a journal entry's pulse. Anything that needs the rate to reflect a change it just made writes the input and lets the next pulse pick it up. On the six release/uprising hooks ROOT is the *parent* (vanilla `00_code_on_actions.txt:3634`, `:5143`, `:5510`) and only `scope:target`, the new tag, is updated; the parent already pulses monthly, and a second call in the same month would drift it an extra third of a point and re-draw both walks. `on_country_formed` is the exception where ROOT *is* the new tag.

**One step has a second entry point.** Step 9, `te_monetary_apply_interest_modifiers`, is the only idempotent step — it recomputes the applied value, compares, and re-adds both modifiers in one block, without drifting the rate or drawing a walk. `on_acquired_technology` therefore calls **step 9 alone** (`te_monetary_on_acquired_technology`, registered in `extra_on_actions.txt`'s existing block). Without it a vanilla finance tech's −2pp lands instantly while `te_mon_vanilla_tech_offset` waits for the next pulse, so a country already paying under 2% spends up to a month with a non-positive `country_loan_interest_rate_add` sum — the state the invariant above exists to prevent. No other step of the update may be called in isolation.

**Hidden state.** `te_neutral_rate`, `te_neutral_error`, `te_neutral_walk` and `te_mon_stance_gap` are never printed in any tooltip, loc string or visible modifier. The player's only window is the stance **band name**, and the band is computed from the bank's *estimate* (`gap − error`), so the true neutral rate cannot be recovered by stepping the dial and watching the band flip. Consequence for script: `banking_stance_is_tight` / `_loose` read that hidden gap and are therefore **`ai_chance`-only** — putting either in a `possible` / `is_valid` / `custom_tooltip` would render the gap into a player tooltip. The eight surviving uses are all `ai_chance` weights in `timeline_extended_scripted_buttons.txt`.

**Never print `bubble_pressure` to a decimal, on any surface.** The stance reaches the banking cycle as `te_mon_stance_bubble_add` = `−0.75 × te_mon_stance_gap_clamped`, and — unlike momentum and cycle value, which `banking_cycle_advance_variables`' `random_list` nudges — bubble gets **no random term**. `banking_display_bubble_monthly_add` is exactly `modifier:country_bubble_pressure_monthly_add`, i.e. the modifier-driven part *without* the stance push, so Δ`bubble_pressure` minus the displayed monthly add is the stance push alone. Two readings a month apart therefore recover the hidden stance gap, and with it r\* — exactly wherever the gap is inside the ±4 clamp and bubble is off its own 0/100 bounds, which covers the entire stance-band range. Every surface today bands it (the dashboard band, title-only history rows, the graphical vanilla bar), which is what keeps the hidden-state rule above true; this is now load-bearing, not cosmetic. Any future tooltip, chart or debug surface that wants a bubble figure must band it, or give `bubble_pressure` a random term first.

**Cancel-INJECTs rest on an unverified engine assumption.** `te_monetary_rank_injections.txt` (eight ranks) and `te_monetary_law_injections.txt` (`law_laissez_faire`) cancel vanilla's `country_loan_interest_rate_mult` with its exact inverse and re-express the contribution on `country_credit_standing_add`. That only works if a `modifier = { }` inside an `INJECT:` **sums** with the vanilla entity's own block. If INJECT is last-wins instead, each file is wrong twice over — the cancel *adds* to vanilla's mult, and the injected block wipes the rest of the rank/law modifier. The checks that settle it need no code (`monetary_policy_design.md` §17 checks 1–4, and the OPEN question in `scripting_best_practices.md` § INJECT); both files are deliberately isolated so the fallback (restate the entity as a `REPLACE:`) is a one-file fix.
...
**Open market operations, rewired.** `banking_open_market_ops` no longer touches interest at all (the dial owns the rate). It is gated on the policy rate sitting at its regime **floor** — `te_mon_policy_rate_at_floor`, `<= -2.99` under digital currency and `<= 0.01` otherwise — so it is the balance-sheet tool of last resort rather than an everyday easing lever. A gold-standard country (clamped to the reference rate ±2 in phase 1) **cannot reach it until the world reference rate falls to 2.0** — `te_mon_target_min` is `ceiling(max(reference − 2, 0))`, which is 1 at the 1836 rate of 3.0 and still 1 at 2.5, but 0 once a great power holds both `macroeconomics` and `globalization`. The loc (`banking_omo_rate_floor_tt`, "zero under a gold standard") is the accurate wording. Its `country_bubble_pressure_monthly_add` went 0.8 → **1.5**: buying assets with the rate already at zero is how speculative excess builds. Its `ai_chance` was retuned around the new gate (panic 70, downturn 45, bubble-risk-high −30) and its old "don't act while the rate hike is on" weight is gone as unreachable.
...
**Toggle costs.** Each `cb_*` button has friction on toggle to discourage rapid cycling, calibrated to its real-world parallel: monetary tools (OMO, asset relief, directed credit, e-liquidity, fx support, export credit) cost treasury via `banking_*_activation_cost` script values scaled to GDP; political tools (countercyclical buffer, raise margin requirements, deposit guarantee) generate radicals/loyalists via `banking_small_radicals_value` and `banking_medium_radicals_value` (banking-specific equivalents of vanilla `small_radicals`/`medium_radicals`, defined in `extra_script_values.txt` to allow tuning banking-button friction independently of global event balance); FX manipulation (devaluation, capital controls) costs infamy and -2/-3 relations with great-power trade partners. Moral suasion is the lone exception — speeches have no real-world toggle cost.
...
| `te_monetary_on_actions.txt` | Monetary policy: world reference rate, the monthly country update, the yearly growth snapshot, and the country-creation hooks (see **Monetary Policy (phase 1)**) | Mixed |
