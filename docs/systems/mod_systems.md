...
**6b — leverage, one line each.** `country_treaty_leverage_generation_add` on the strong side: **200** on `currency_peg`'s `target_modifier` (the anchor is the target), **150** on `swap_line`'s and **300** on `lender_of_last_resort`'s `source_modifier`. Flat, not scaled, and that is safe for two engine reasons: the modifier generates leverage only while the dominant country **leads a power bloc**, and leverage is generated **per pair** out of the target's own pool — so an anchor with ten peggers has leverage on ten countries, which is orbit-building, not a stack on one.
