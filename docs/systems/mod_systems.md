...
- Uses 3 `scripted_progress_bar` instances and 68 scripted buttons (34 `cb_*` — 17 market tools, each an enable/disable pair — 18 `ce_*`, 16 `cw_*`). `test_banking_tool_roster.py` checks every `cb_*` tool is registered everywhere it has to be (see **Policy tools added 2026-09-23** below).
