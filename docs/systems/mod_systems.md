...
- **Lookup rule:** every "network for target X" lookup is a `random_in_list` under an `any_in_list` guard on the same target, with its own saved-scope name (`iw_create_net`, `iw_det_net`, `iw_burn_net`, `iw_tick_net`). These run inside loops, and an unmatched lookup would otherwise leave the previous iteration's network in the saved scope.
