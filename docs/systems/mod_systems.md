...
**Step 10, `te_monetary_update_gold`.** For `te_mon_has_gold_flows` countries (on gold, with a dial): `gap = clamp(policy − world, ±5)`, `flow = gap × 0.002 × gdp` a month, applied with **`add_treasury`** — never a scaled expense modifier, which would register an outflow as deficit spending in both the fiscal-stimulus reader and the §9.1 deficit term. Inflows are **hot money** (`te_gold_hot_money`): they stop in debt, at the engine's reserve limit, and once the borrowed balance itself reaches one reserve limit; when the gap is ≤ 0 the balance leaves first, at twice the ordinary speed and never slower than a 0.5pp gap would give. **The balance pays the policy rate** (owner decision I; `te_gold_carry`, a twelfth of the rate a month, by `add_treasury`, or capitalised into the balance when there is no treasury to pay it from) — the historical reason nobody raised funds this way, and what makes hot money dearer than ordinary borrowing. Under pressure (`scaled_gold_reserves < 0.1` or any debt) outflows stop and **`te_peg_confidence`** moves instead (−3 per pp of negative gap, −2 in a recession, −2 at `scaled_debt ≥ 0.5`, +2 otherwise; a hot-money exit that cannot be paid counts double); out of pressure it heals +1 a month. At ≤ 20 it dispatches `te_peg.1` behind a 24-month cooldown it sets itself — the step 6c shape. No outflow exceeds `gold_reserves`, so this step can empty a treasury and never creates debt — with one deliberate exception: a country that stops being on gold **altogether** pays its whole hot-money balance out at once (a suspension *freezes* it instead). The flow reaches inflation a month later as `te_mon_pressure_gold_flow`, 0.5pp per 1% of GDP a year — the price–specie mechanism that makes pulling gold in self-correcting. Step 10 also ticks the three clocks (crisis cooldown, Defend, suspension) after everything that reads them.

**Peg defence is `world + 0.5 × shortfall`, rounded up.** `te_mon_peg_shortfall_pp` is 2 with an empty vault or any debt, falling to 0 at half the reserve limit. The branch in step 2 carries its **own** hysteresis — move whenever the target is below the formula, take the next integer up, come down only when more than 1.25 above — and then hands the shared round-to-nearest rule the target it has just settled, because a peg defender parked 0.75 *under* the world rate bleeds gold by construction.

**The crisis, `te_peg.1`.** *Defend* (default): `te_peg_defend_months = 12`, which `te_mon_target_min` turns into a floor of `ceiling(world + 4)` for manual and delegated targets alike; confidence +40. *Suspend*: the 60-month counter, `te_mon_peg_suspension` (+2pp cyclical premium, 1825 days) and `te_mon_peg_credibility_lost` (cancels the gold law's −1pp credit standing, 3650 days); convertibility resumes at confidence 50. *Devalue*: confidence 50, a one-off `add_treasury` of 15% of the reserve limit, `te_mon_peg_devalued` (±15% export / import advantage, decaying over five years), `te_mon_peg_devaluation_pressure` (+3pp of inflation pressure decaying over two — pressure, not expectations, because gold re-pins expectations at 0 every pulse), and the old Devalue button's +1 infamy / −3 great-power relations. Like `te_inflation.1` it writes inputs and state, never the rate, and **must never call the monthly update**. It is the variable contract's phase-3 documented outside writer (`te_peg_confidence` and the two clocks).

**Nothing in phase 3 is hidden state.** The world rate, the gap, the flow, the hot-money balance and confidence are all built from displayed figures and carry no r\*, so the dashboard prints them exactly: a **World Rate** row under the reference row, and — for a gold-law country with a dial, suspended or not — **Gap to World Rate**, **Gold Flow** (the only place the flow is visible; `add_treasury` never reaches the budget ledger) and **Peg Confidence** with a word from `te_mon_peg_state`. `te_debug_monetary.8` prints the globals and stages §19 row 3's exit criteria: its option a adds 2pp to the world rate through `global_var:te_world_rate_debug_offset`, the one debug hook in production code.
...
| `te_monetary_on_actions.txt` | Monetary policy: world reference rate and (phase 3) the world rate, the monthly country update, the yearly growth snapshot, and the country-creation hooks (see **Monetary Policy (phase 1)**) | Mixed |
