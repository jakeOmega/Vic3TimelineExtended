...
- **AI policy choice** is one table, `common/script_values/global_warming_ai_values.txt`. It gives each country a climate will per policy and each policy a threshold on it. Adopt scores at or above the threshold, repeal only 15 below it, so each AI holds the policies its conditions support rather than all of them. Every will is a shared core (temperature, the environment ministry, an environmentalist in government) plus the same five signals under per-policy weights: laissez-faire, industrialists in government, the environmental movement, relative standard of living, and fossil exports. So wealth delays Climate Adaptation but speeds a carbon tax, and exporters resist divestment hardest. The weight table is in the file's header, and a test checks it against the code. Details and thresholds: `journal_entry_systems.md` → **How the AI chooses**. Calibrate with `event te_debug_gw.1` option **l** (`GW_AI_WILL` lines in `debug.log`).
...
**Civil wars, annexation and custody** (`nuclear_custody_effects.txt`, since 2026-09-26; `nuclear_crisis_design.md` §0.10). Every arsenal has a **ledger record**: a freestanding script container holding its count, warhead progress, capital region, posture and at-war flag, refreshed weekly by the entry. A container outlives its country, so when an arsenal's owner stops being alive — a civil war either way, a crushed secession, a war, diplomatic or formable annexation, any scripted `annex` — `nd_custody_reconcile` hands the arsenal to whoever holds its old capital's region, less a lost fraction (1 % + 2 % per missing safeguards level + 3 % under delegated authority, doubled at war) that goes to the loose-warhead count for step 5 (`nd_loose_warheads` globally, `nd_ar_loose` per origin). It runs from `on_civil_war_won`, `on_secession_end`, a day after `on_wargoal_enforced`, `on_country_formed` and the mod's own `annex` sites, and monthly as the catch-all. At a civil war's **outbreak** both sides hold their own stockpile and record, so the revolution merge (the winner keeps its own value of every variable: `scripting_best_practices.md` § "What a Civil War's Winner Inherits") never counts a transfer twice. An armed original country is asked **"Who Holds the Button?"** (`nuclear_custody.1`): hold the line (the rebels seize a share following their population, raised by delegated authority and cut by safeguards), pull them back (Recessed, locked until every civil war is over: `nd_cw_withdrawn`), or dismantle them. A winning revolution is a **new regime**: it keeps the state's doctrine and investments (tenures satisfied), restarts at credibility 50, drops the old regime's pledges and inherited crises, and carries warhead progress over its fresh entry. A nuclear strike on one's **own civil war** costs −50 legitimacy decaying over 20 years and a radical surge the speech and security laws scale (`nd_struck_own_people`), and the AI does it only when losing badly under Outlawed Dissent or a Secret Police. The shared civil-war hooks the other systems' repairs will plug into are `common/on_actions/te_civil_war_on_actions.txt` and `te_civil_war_effects.txt`.

**Losing, the world watching, and secession** (since 2026-09-26, the rest of §0.10 steps 3–4). A government **losing a revolution** while armed (`nd_is_losing_war_to` its rebels; a secession is not asked) is asked once, monthly check, **"Deny Them the Bomb"** (`nuclear_custody.5`): take the warheads apart in haste (the outbreak's chaos loss rate goes missing), let an accepted custodian take them apart with nothing lost, or keep them. A week after an armed country's civil war breaks out, every power believed armed that is a major power or borders it gets **"A Nuclear Power Divided"** (`.6`): back either side (relations ±20, +5 war support in the civil war), offer to secure the arsenal (`.7` to the government; accepted, the helper is its custodian and its **custody is secured** for the war), or stay out. **Secured custody** (`nd_custody_is_secured`: a civil-war custodian, inspection after a loose warhead was traced to us, a `nuclear_security_assistance` article, or the UN physical-protection convention) halves both loss rates. A **secession that won holding warheads** is pressed by the great powers (`.10`): the first to press sends **"The Budapest Offer"** (`.11`), which trades the warheads for treaties in force — `nuclear_disarmament` + `nuclear_guarantee` with the largest-economy presser, a guarantee from each other — or keeps them (−20 relations with each, `nd_bp_refused` raises AI disarmament demands for ten years).

**Loose warheads** (`nuclear_loose_effects.txt`, `events/nuclear_loose_events.txt`, since 2026-09-26, §0.10 step 5). The pool the settlement fills is drawn on by **plots**: weight 3 in the social-tensions yearly terror pool (`nuclear_loose.1`), in the terrorism era, with the domestic terror attack's unrest gate. A device is real with 20 % per warhead in the world pool; its line is chosen weighted by `nd_ar_loose` among live owners other than the victim; the victim's services seize it first with 10 % + 5 per intelligence-ministry level + 15 with `nuclear_weapons`; otherwise `nuclear_industrial_strike` hits a populous state with no attacker. A world cooldown of a year follows. **Attribution** is rolled at once and reported a month later (`.4`): confirmed (10 + few candidates + the victim's expertise + an origin that tested openly + a covert network there + a device in hand, max 90), else a shortlist of the origin and decoys (which can name the innocent) or nothing. **Blame means negligence** (`.6`): a confirmed origin pays compensation (the reparations formula), opens its custody to inspection (secured for twenty years, −5 legitimacy decaying, a 50 % first search) or denies it (−40 relations, +5 infamy); a shortlisted suspect is only asked to cooperate. **Recovery** takes warheads out of the pool: inspection's search, the covert operation **Secure Loose Material** (`covert_secure_material_action`, § Covert Warfare), the `nuclear_security_assistance` treaty article (6 % a month for the helped country), and the UN's **Convention on the Physical Protection of Nuclear Material** (3 % a month for a party). The posture panel shows an "Unaccounted for" row while our line has warheads missing.
...
| `common/scripted_effects/te_monetary_civil_war_effects.txt` | `te_monetary_repair_after_civil_war` (the shared layer's plug-in) and `te_monetary_inherit_central_bank` — what a revolution's winner takes over from the loser's central bank (see **Civil wars** below) |
...
- **On Actions:** `common/on_actions/space_race_on_actions.txt` — yearly random events for all in-progress and cross-system events; the monthly country pulse (inactive-milestone cleanup, entry-modifier backstop, failure cooldown). A civil war's end runs `sr_repair_after_civil_war` (`space_race_effects.txt`) from the shared hook `te_civil_war_on_won` (`te_civil_war_on_actions.txt`): a revolution's winner gets its country rewards back
...
- **A revolution's winner continues the programme** (`docs/audits/civil_war_inheritance_audit.md` F5, #464). The winner inherits the loser's variables but none of its modifiers, and every entry it inherits active runs `immediate` again. So:
  ...
  - **Every reward is rebuilt from a variable.** Milestone rewards (`sr_first_<m>` / `sr_<m>`) from `sr_completed_<m>` / `sr_was_first_<m>`, and the probe result (`sr_probe_*_data`, recorded as `<modifier>_held` by `sr_grant_probe_data`), come back in `sr_repair_after_civil_war`, which the shared hook `te_civil_war_on_won` runs for the winner. It is stateless: it adds a reward only where the record is held and the modifier missing, reads none of the shared layer's side variables, and removes nothing. Entry modifiers come back from each entry's `immediate` (`sr_sync_<m>_entry`) and, as a backstop, the monthly country pulse (`sr_sync_space_race_entries`, only for a country that has run a milestone: `sr_space_race_participant`): the approach modifier from `sr_safe_<m>` / `sr_ambitious_<m>` (and a stale one with no approach selected is removed), the choice modifier from the choice variable, the 68 colony specialisations from `<modifier>_held` (`sr_grant_colony_modifier`), and `sr_solar_system_trade` from `sr_completed_solar_colonization`. The monthly pulse also records a probe result or colony modifier granted before the record existed, so old saves migrate themselves. A loyalist winner still holds every modifier, so all of it is a no-op there. Timed modifiers (the completion events' themed rewards, failure penalties, the safety review) are not restored.
  - **The rebels start no programme of their own mid-war.** Suborbital flight's `is_shown_when_inactive` requires `is_revolutionary = no` (every later entry already needs a completion the rebels lack). Their own run would otherwise beat the loser's progress in the variable merge.
...
**Civil wars** (the shared hooks in `te_civil_war_on_actions.txt`, #467). `te_civil_war_on_won` runs `te_civil_war_resolve_sides`, then nuclear custody and the #460–#465 repairs (`agdiff_repair_after_civil_war`, `un_repair_after_civil_war`, `te_monetary_repair_after_civil_war`, `cr_repair_after_civil_war`, `sr_repair_after_civil_war`), then `te_civil_war_clear`. The agricultural-diffusion ones:
...
| `covert_warfare_rule` | `covert_warfare_enabled` | enabled | Covert operations command-centre JE (`je_covert_warfare`), all 14 covert diplomatic actions (secure loose material also needs `nuclear_weapons_enabled`) |
...
- **Secure loose material** (`covert_secure_material_action`, code 13, mild, military defence; 2026-09-26, `nuclear_crisis_design.md` §0.10 step 5): needs the `nuclear_weapons_enabled` rule too, and a target whose arsenal line has warheads unaccounted for (`nd_loose_has_line`), which is also its `requirement_to_maintain` — it ends by itself when nothing is left to find. **No modifier on anyone.** Its own block in `covert_ops_apply_all_phase_effects`: once established, each month a `covert_secure_material_chance` (`covert_secure_material_base_chance` 3 × `covert_op_effect_mult`: 3 % established, 6 % fully operational, 9.6 % at priority 3) of `nd_loose_recover_from_line` on the target — the warhead is destroyed and leaves the pool — and `nuclear_loose.9` for the operator; the target is never told. `test_covert_op_registry.ACTS_WITHOUT_MODIFIER` lets it act without a marker. AI: a major power or above proposes it against any target with a loose line (score 5, +5 for a neighbour; evaluation 0.05).
...
- **All 12 peacetime ops** (election interference, financial subversion, industrial espionage, military espionage, influence campaign, ideological subversion, destabilization, slice 6's regime change, nuclear programme sabotage, space programme espionage and cultivate assets, and secure loose material) have truce checks in both `possible` and `requirement_to_maintain`, so they auto-cancel if a truce with the target begins. None of them has a war guard (the actions say "Allowed during war"): a war with the target does not end them.
- **Slice-6 operations also lapse** when their own condition goes: regime change when the rivalry ends; nuclear programme sabotage when the target's programme is paused by treaty, given up or gone (`nuclear_program_is_standing` — not when its funding merely drops to 0); space programme espionage once the target is no longer ahead; cultivate assets when the target becomes decentralized; secure loose material when the target's line has no warhead left unaccounted for.
