...
| File | Holds |
|---|---|
| `events/te_inflation_events.txt` | `te_inflation.1` "Not Worth the Paper" — the §9.2 hyperinflation crisis. Three options: currency reform (reset to 5% + investment-pool wipe + radicals + a ten-year risk premium), dollarise (−75% minting, no dial and no monetisation **until the country enacts a monetary law again** — the P9 exit, which clears the flag and pays the new-currency price), ride it out (default; 24-month cooldown) |
