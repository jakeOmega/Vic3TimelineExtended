...
- **Graduated exposure blowback (2026-09, covert slice 2):** what a detection costs is a function of the burned operation's **exposure tier** and **phase**, not a flat number.
  - **Tier table:** which code lands in which tier lives in exactly one place, the four `covert_code_tier_mild` / `_moderate` / `_severe` / `_war` triggers in `covert_warfare_triggers.txt` — each is an `OR` of `var:$VAR$ = <code>` guarded by `has_variable = $VAR$`. Adding a tenth operation type means adding its code to exactly one of those four OR lists; nothing else re-lists codes (the phase-mult/infamy/relations script values, the custom-loc tier-name blocks and the pre-launch tier note in each action's `_desc` all branch through these triggers rather than switching on the raw code). Today's split: mild = {4 industrial espionage, 5 military espionage} — "everyone spies"; moderate = {0 election interference, 1 financial subversion, 6 influence campaign} — meddling in how the target governs itself; severe = {7 ideological subversion, 8 destabilization} — attacking the state itself, the tier third parties hear about; war = {2 infrastructure sabotage, 3 comms disruption} — only sensible against someone you are already fighting.
  - **Base cost by tier** (`covert_exposure_infamy_base` / `covert_exposure_relations_base`, `covert_warfare_script_values.txt`): mild +1 infamy / −10 relations, moderate +2 / −20, severe +4 / −40, war +1 infamy / no relations term (the event also skips the relations effect outright for the war tier, so the option never previews a "-0"). An unrecognised code reads as moderate.
