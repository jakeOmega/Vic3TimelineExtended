...
**Open market operations, rewired.** `banking_open_market_ops` no longer touches interest at all (the dial owns the rate). It is gated on the policy rate sitting at its regime **floor** — `te_mon_policy_rate_at_floor`, `<= -2.99` under digital currency and `<= 0.01` otherwise — so it is the balance-sheet tool of last resort rather than an everyday easing lever, and gold-standard countries (clamped to the reference rate ±2 in phase 1) never reach it. Its `country_bubble_pressure_monthly_add` went 0.8 → **1.5**: buying assets with the rate already at zero is how speculative excess builds. Its `ai_chance` was retuned around the new gate (panic 70, downturn 45, bubble-risk-high −30) and its old "don't act while the rate hike is on" weight is gone as unreachable.
