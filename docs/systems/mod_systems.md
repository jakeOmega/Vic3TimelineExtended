...
**Phase 6 is planned, not built.** A review of the three 5a articles (`monetary_policy_design.md` §15B) found no leverage, no coercive variant, and three verified exploits — multi-provider swap-line stacking against a self-inflicted crisis, serial default farming against a guarantor whose odds never move, and uncapped per-pegger prestige on the anchor. The staged plan is §15C, with the owner's rulings taken (§15C.5): **6a** hardening (the draw becomes a repayable, capped loan from one provider, drawn only in a financial crisis; a ward-side `te_mon_lolr_calls` counter that escalates the honour cost, tapers the relief, shifts `te_lolr.1`'s odds, lengthens the cooldown and counts against the ward at signing; the AI's own post-signing withdrawal on the re-scored articles, with a scripted yearly walk on `is_equal_exchange_for` only as fallback; `non_fulfillment = withdraw` on war / expulsion — the per-pegger prestige fan-out stays as shipped by ruling H5), **6b** `country_treaty_leverage_generation_add` 200 / 150 / 300, **6c** two hostile enforceable articles, `imposed_currency_peg` and `debt_receivership`, demandable only against a country in default. Its checklist is P6-1…13. Do not treat any of it as shipped until a §0.10 exists.
...
- **Width.** The plot is a fixed-width `hbox`; each item is `size = { 0 100% }` + `layoutpolicy_horizontal = expanding` + `maximumsize = { 40 -1 }`, so the *visible* bars divide the width between them (vanilla's `levels_progressbar` idiom). Switching to 1 year widens the bars instead of leaving 228 empty slots. At the 20-year end the 484px plot is split 240 ways — about 2px a bar, so the series reads as a shape, not as countable months. An out-of-range item pays for its `visible` test and nothing below it, so the short ranges stay as cheap as they were before the cap doubled.
