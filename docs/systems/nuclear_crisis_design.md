...
| Custody: the arsenal ledger, the settlement, the civil-war split and new regime, the price of striking one's own side (§0.10) | `common/scripted_effects/nuclear_custody_effects.txt`, `common/scripted_triggers/nuclear_custody_triggers.txt`, `common/script_values/nuclear_custody_values.txt`, `common/on_actions/nuclear_custody_on_actions.txt` |
| Custody events (`nuclear_custody.*`) | `events/nuclear_custody_events.txt` |
| Shared civil-war hooks (every system's civil-war repair hangs off these) | `common/on_actions/te_civil_war_on_actions.txt`, `common/scripted_effects/te_civil_war_effects.txt` |
| Loose warheads: the plot, attribution, the reckoning, recovery (§0.10 step 5) | `common/scripted_effects/nuclear_loose_effects.txt`, `common/scripted_triggers/nuclear_loose_triggers.txt`, `common/script_values/nuclear_loose_values.txt`; the pool entry in `common/on_actions/social_tensions_on_actions.txt` |
| Loose-warhead events (`nuclear_loose.*`) | `events/nuclear_loose_events.txt` |
| Security assistance treaty article `nuclear_security_assistance` | `common/treaty_articles/116_nuclear_security_assistance.txt` |
| Covert operation "Secure Loose Material" | `covert_secure_material_action` in `common/diplomatic_actions/covert_operations.txt` (every covert site: `test_covert_op_registry.py`) |
| UN Convention on the Physical Protection of Nuclear Material | topic `physical_protection`, agency `un_agency_cppnm`, `un_events.23`, chamber op 17 (the UN files; `journal_entry_systems.md` § Chamber Widget) |
...
43. **Custody (§0.10).** Every armed country and every great power with a programme gets a ledger record within a week (`debug.log` is quiet; `TE_CUSTODY:` lines appear only at custody changes).
44. A revolution breaks out in an armed country: "Who Holds the Button?" (`nuclear_custody.1`) arrives with the rebels' name and our stockpile, and each option's tooltip shows the warheads the rebels would take, the ones lost and the ones we keep. Hold the line: the rebels' stockpile matches the tooltip, their "Nuclear Weapons" entry appears at once and `nuclear_power` within a week. Pull them back: readiness is Recessed at once, every raise is greyed with "No civil war holds our warheads in storage" failing, a strike on us offers no answer, and the lock is gone the month after the civil war ends. Dismantle: our stockpile is 0, `nuclear_power` is gone, relations +10 everywhere.
45. The revolutionaries win: `debug.log` has `TE_CIVIL_WAR:` lines saying who won and **whether the loser still resolves and its `te_cw_role` reads at `on_civil_war_won`** — record the answer in `scripting_best_practices.md` § "What a Civil War's Winner Inherits". The winner holds the old state's warheads less the lost ones and its own share, keeps the old doctrine and investments, has credibility 50, no pledges, and the old state's warhead progress back on the entry's first pulse; a human gets "The Old Regime's Arsenal" once.
46. The loyalists win: the rebels' warheads come back less the lost ones, and a human gets "The Arsenal Changes Hands".
47. A secession wins: the new country keeps its share. A secession is crushed: its share comes back. Check whether `on_civil_war_won` fires for a secession (the `TE_CIVIL_WAR:` lines say).
48. An armed country is annexed (war goal, diplomatic annexation, a formable): within a day, or at the latest the next month, the annexer holds its warheads less the lost ones, and `global_var:nd_loose_warheads` counts the lost ones.
49. Striking our own rebels (or, as rebels, the government): the strike's confirmation box shows "Nuclear Weapons Used on Our Own People" (−50 legitimacy) and the radicals, and they land. Check first whether the strike action can target a civil-war country at all. An AI does not strike its rebels unless 40 % of it is occupied under Outlawed Dissent or a Secret Police.
50. `error.log` stays quiet through all of the above; in particular the `add_journal_entry` for a rebel or successor does not duplicate an inactive entry.
51. **Pull-back lock (behaviour change, 2026-09-26).** It lifted the month after the outbreak before: `any_civil_war` iterates civil wars still brewing, not one under way. Now `nd_in_civil_war` reads the rebel country: the lock holds for the whole war and lifts the month after it ends.
52. **Deny Them the Bomb.** Losing a revolution while armed (25 % occupied, or losing its battles), "Deny Them the Bomb" (`nuclear_custody.5`) arrives once, with the rebels' name and our stockpile. Take them apart: the tooltip's missing count matches the loose count added, stockpile 0, `nuclear_power` gone. With a custodian accepted, option b takes them apart with nothing lost and relations +15 with it. The question comes again in a later civil war, not this one.
53. **The world watching.** A week after an armed country's civil war breaks out, an armed major power gets "A Nuclear Power Divided" (`.6`) with both names. Backing a side moves relations ±20 and that side's war support in the civil war +5 (check the war's support tooltip shows it). An offer reaches the government as `.7`; accepting it names the custodian, and the helper hears the answer (`.8`). A second offer while one is pending or accepted is not offered.
54. **Secured custody.** With a custodian, inspection, the article or the convention, a settlement or hasty dismantling loses half as many (the `TE_CUSTODY:` settlement line and the tooltips).
55. **Budapest.** A secession wins holding warheads: armed great powers get "A New Nuclear State" (`.10`) a week later; a month after the first presses, the new state gets "The Budapest Offer" (`.11`). Trade: the diplomacy panel shows a treaty with the lead (disarmament + guarantee, 10-year binding) and a guarantee from each other presser; stockpile 0. **Verify whether `create_treaty` with `is_draft = no` ran `nuclear_disarmament`'s `on_entry_into_force`** (`te_nuclear_disarmament_ended_program` set). Keep: −20 relations with each presser, who hear it (`.12`).
56. **Loose-warhead plot.** Force one: set `global_var:nd_loose_warheads` and an `nd_ar_loose` on some live country's record, then `event nuclear_loose.1` on a terrorism-era country with radicals. A seized device (`.3`) or a detonation (`.2`: devastation, casualties, no `nuked_by_country`), the world notice (`.5`) for other humans, and `.4` a month later. **Verify the weighted pick** (`random_in_global_list` with `weight = { value = var:nd_ar_loose }`) takes the weight rather than picking uniformly.
57. **Attribution.** Over repeated plots: confirmed names the true origin; a shortlist of two or three includes it in a random place; unknown names no one. Hold to account sends `.6`; paying moves the treasury sum shown; inspection sets the 20-year secured custody and the modifier; denying costs −40 relations and +5 infamy. Name them all sends `.7` to each suspect; asking sends each `.6`'s request variant.
58. **Recovery.** "Secure Loose Material" is listed only against a country with a loose line, runs as a mild operation, recovers a warhead now and then once established (`nuclear_loose.9` for the operator; the panel's row falls for the target), and ends by itself at zero. `nuclear_security_assistance` can be proposed to an armed country or one with a loose line and recovers at about 6 % a month. The convention appears on the docket only after warheads have gone loose, passes, and gives parties the 3 % monthly search; its chamber row is hidden under the disabled nuclear rule.
59. The posture panel's "Unaccounted for" row shows our line's count and the world count in its tooltip, and disappears at zero.

### 0.10 Civil wars, custody and loose warheads (agreed 2026-09-25; built 2026-09-26)

The order agreed with the owner for closing the §11 gap ("handle annexation, civil war … explicitly") and extending it. §0.7 says what the engine did before. Each step starts with its own design pass. Steps 2 and 3's first event were built together on 2026-09-26 (spec: `docs/superpowers/specs/2026-09-26-nuclear-custody-design.md`), and the rest of steps 3–5 later that day (spec: `docs/superpowers/specs/2026-09-26-nuclear-loose-warheads-design.md`, which lists the calls made without the owner); nothing of them has run in a game yet (§0.9 items 43–59).

1. **Engine test — done.** A save pair of a revolution the rebels won showed what the winner inherits (`scripting_best_practices.md` § "What a Civil War's Winner Inherits"). The arsenal passes whole to the rebels through the variable merge, and `nuclear_power` comes back on the fresh entry's first pulse. So custody is an explicit transfer, not a rescue. **Still open:** whether the dead-but-undeleted loser resolves through a stored pointer at `on_civil_war_won`. Nothing built depends on it; the `TE_CIVIL_WAR:` log lines answer it on the first civil war in a test game (§0.9 item 45).
2. **Custody transfer — built.**
   - **The ledger.** Every armed country, and every great power with a programme, has a ledger record: a freestanding script container (tag `nd_arsenal`, global list `nd_arsenal_records`, the owner's `nd_arsenal_record`) with its count, warhead progress, capital region, posture and at-war flag, refreshed by the entry's weekly pulse, the country pulse and every launch. A container outlives its country.
   - **The settlement** (`nd_custody_reconcile`): a record whose owner no longer resolves or is not alive passes to the owner of the most populous live state in its capital region, less a lost fraction (`nd_ar_loss_rate`: 1 % + 2 % per missing safeguards level + 3 % under delegated authority or launch on warning, doubled if the holder was at war; stochastically rounded). One helper for every way an owner can end: it runs at `on_civil_war_won` and `on_secession_end`, a day after `on_wargoal_enforced`, `on_country_formed` and each of the mod's own `annex` sites (`nuclear_custody.9`), and monthly from `on_monthly_pulse` for every other path. The successor gets the entry at once, the posture initialiser, a snapped public estimate and, if human, "The Arsenal Changes Hands" (`.3`).
   - **At the outbreak** both sides hold their own stockpile variable and record, so the merge never hands a winner the loser's arsenal and nothing is counted twice. The rebel's share is decided by the event in step 3.
   - **A winning revolution is a new regime** (owner ruling 2026-09-26): it keeps the state's doctrine and investments with both tenures satisfied; credibility restarts at 50; the old regime's pledges and defiance records lapse (lists are not inherited; the settlement removes the dead owner from its partners' lists while it still resolves); an inherited crisis the other side no longer mirrors is dropped as outcome 9; warhead progress is carried over the entry's fresh activation (`nd_custody_restore_progress`); the AI re-reviews its posture at once, and a human gets "The Old Regime's Arsenal" (`.4`). Read from the old state's record, never from the dead loser.
   - **`can_revolution_inherit` stays `no`** (`je_nuclear_program.txt` says why).
   - **Shared infrastructure — built:** `te_civil_war_on_actions.txt` / `te_civil_war_effects.txt`. The rebel gets `te_cw_origin` at the outbreak; the end hooks record the pair; `on_civil_war_won` works out who won (`te_cw_rebels_won`, `scope:te_cw_loser`), runs each system's repair between `te_civil_war_resolve_sides` and `te_civil_war_clear`, and logs whether the loser still resolves. The fixes for `docs/audits/civil_war_inheritance_audit.md` (#460–#465) plug in there.
   - **Striking one's own civil war (owner ruling) — built.** `nd_record_nuclear_use`, outside its `hidden_effect`, applies `nd_struck_own_people` (−50 `country_legitimacy_base_add`, decaying over 20 years; a second strike renews it) and `add_radicals` of 0.2 × the law factors (Outlawed Dissent 0.5, Censorship 0.75, Right of Assembly 1.25, Protected Speech 1.5; Secret Police or a variant 0.75, Guaranteed Liberties 1.25). The same for every regime, and for rebels striking the government. `nd_ai_nuclear_use_justified` refuses a civil-war counterpart, retaliation included, unless 40 % of the country is occupied under Outlawed Dissent or a Secret Police. Before this, `dp_revolution`'s mirrored `annex_country` goal let both sides strike under Existential Deterrence and past the Rules of War at no cost.
3. **Civil-war events.**
   - **"Who Holds the Button?" — built** (`nuclear_custody.1`, to an armed original country at `on_revolution_start` / `on_secession_start`). The split is rolled once in `immediate`, so each option shows exact numbers. **Hold the line** (the default): the rebels seize `stock × their population share × 1.5 under delegated authority × (1 − 0.2 × safeguards)`, and of those twice the loss rate goes missing. **Pull them back**: Recessed at once and locked until every civil war of ours is over (`nd_cw_withdrawn`: the panel, the AI, the incident and crisis paths and the retaliation path all respect it); the rebels still take half their share under delegated authority. **Dismantle** (South Africa before its transition): stockpile 0, `nuclear_power` off, relations +10 with everyone, nothing seized or lost — the choice for a government that expects to lose, since a winning revolution inherits whatever it still holds.
   - **"Deny Them the Bomb" — built** (`nuclear_custody.5`): a government losing a **revolution** (not a secession, whose winner does not take the government's arsenal) while armed is asked once per civil war, from a monthly check: take the warheads apart now (the outbreak's chaos rate goes missing, rolled in `immediate`), let an accepted custodian take them apart with nothing lost, or keep them (default).
   - **Foreign reaction — built** (`.6`–`.8`): a week after the outbreak, every power believed armed that is a major power or borders the origin backs the government or the rebels (relations ±20, +5 war support in the civil war), offers to secure the arsenal (accepted: a custodian, whose presence secures custody — half the loss rate — for the war and enables Deny's supervised option), or stays out.
4. **Secession and the Budapest path — built** (`.10`–`.12`). Winning secessionists keep their share (step 2), and every armed great power may press them; a month after the first does, "The Budapest Offer" trades the warheads for treaties in force: `nuclear_disarmament` beside a `nuclear_guarantee` with the largest-economy presser, a `nuclear_guarantee` from each other (10-year binding; the warheads are also zeroed directly, since whether an in-force treaty runs `on_entry_into_force` is unverified). Refusing costs relations with each presser and raises AI disarmament demands for ten years. The upkeep of an arsenal it may not be able to replace is the pressure.
5. **Loose warheads.**
   - **The pool.** Every custody change loses a fraction (step 2). The world count is `global_var:nd_loose_warheads`; the count per origin is `nd_ar_loose` on the origin's ledger record, and passes to the successor's record when the origin dies.
   - **Detonation — built** (`nuclear_loose.1`–`.3`, `.5`): weight 3 in the social-tensions yearly terror pool, in the `terrorism_and_anti_terrorism` era, with the domestic terror attack's unrest gate. A device is real with 20 % per warhead in the pool (max 80); its line is chosen weighted by `nd_ar_loose` among live owners other than the victim; it leaves the pool at once, and the world waits a year. The victim's services seize it first with 10 % + 5 per intelligence-ministry level + 15 with `nuclear_weapons` (max 75); otherwise `nuclear_industrial_strike` hits a populous incorporated state, with no attacker.
