<!-- Auto-generated from triggers.log @ 2026-05-20T22:04:03+00:00; effects.log @ 2026-05-20T22:04:03+00:00. Do not hand-edit. Run POST /reload after the engine regenerates the source. -->
...
- `any/every/ordered/random_civil_war` → civil_war — Iterate through all civil wars related to the scoped country
- `any/every/ordered/random_cobelligerent_in_diplo_play` → country — Iterate through all co-belligerents of scope country in all diplomatic plays (includes wars)
- `any/every/ordered/random_cobelligerent_in_war` → country — Iterate through all co-belligerents of scope country in all wars
- `any/every/ordered/random_company` → company — Iterate through all companies in a country
- `any/every/ordered/random_country_strategic_region` → strategic_region — Iterate through all strategic regions containing states owned by the scoped country
- `any/every/ordered/random_diplomatic_catalyst` → diplomatic_catalyst — Iterate through all diplomatic catalysts in the recent memory of a country
- `any/every/ordered/random_diplomatically_relevant_country` → country — Iterate through all diplomatically relevant countries of a country scope
- `any/every/ordered/random_direct_subject` → country — Any country directly below current in hierarchy
- `any/every/ordered/random_enemy_in_diplo_play` → country — Iterate through all enemies of scope country in all diplomatic plays (includes wars)
- `any/every/ordered/random_enemy_in_war` → country — Iterate through all enemies of scope country in all wars
- `any/every/ordered/random_harvest_condition` → harvest_condition (scopes: country, state, state_region, strategic_region) — Iterate through all harvest conditions affecting a state, state region, strategic region, or country
- `any/every/ordered/random_in_hierarchy` → country — Any country in current hierarchy, including current
- `any/every/ordered/random_interest_group` → interest_group — Iterate through all interest groups in a country
- `any/every/ordered/random_law` → law — Iterate through all laws in a country
- `any/every/ordered/random_military_formation` → military_formation (scopes: country, front, hq) — Iterate through all military formations currently present at input scope Supported scopes: country, front, hq
- `any/every/ordered/random_neighbouring_state` → state (scopes: country, state, state_region, strategic_region) — Iterate through all states neighbouring a state region
- `any/every/ordered/random_overlord_or_above` → country — Any country above current in hierarchy
- `any/every/ordered/random_political_lobby` → political_lobby (scopes: country, interest_group) — Iterate through all political lobbies in a country or interest group
...
- `any/every/ordered/random_scope_ally` → country — Iterate through all allies to a: country
- `any/every/ordered/random_scope_army` → military_formation (scopes: country, front, hq) — Iterate through all armies currently present at input scope Supported scopes: country, front, hq
- `any/every/ordered/random_scope_building` → building (scopes: country, state) — Iterate through all buildings in a: state, country
- `any/every/ordered/random_scope_character` → character (scopes: country, front, interest_group, military_formation) — Iterate through all characters in a: country, interestgroup, or front
- `any/every/ordered/random_scope_culture` → culture (scopes: country, state) — Iterate through all cultures in the scope
- `any/every/ordered/random_scope_diplomatic_pact` → diplomatic_pact — Any diplomatic pact of the country in a scope
- `any/every/ordered/random_scope_fleet` → military_formation (scopes: country, hq) — Iterate through all fleets currently present at input scope Supported scopes: country, hq
- `any/every/ordered/random_scope_general` → character (scopes: country, front, interest_group, military_formation) — Iterate through all generals in a: country, interestgroup, front, or military formation
- `any/every/ordered/random_scope_held_interest_marker` → interest_marker — Iterate through all interest markers held by a country
- `any/every/ordered/random_scope_homeland_state` → state (scopes: country, culture) — Iterate through all states that are a homeland for the scoped culture, or a homeland of any primary culture of the scoped country
...
- `any/every/ordered/random_scope_theater` → theater — Iterate through all theaters in a: country
- `any/every/ordered/random_scope_treaty` → treaty — Iterate through in force treaties binding the scoped country
- `any/every/ordered/random_scope_violate_sovereignty_interested_parties` → country — Iterate through all countries that would be interested if country in scope has their sovereignty violated
- `any/every/ordered/random_scope_violate_sovereignty_wars` → war — Iterate through all relevant wars if target country had their sovereignty violated by scoped country
- `any/every/ordered/random_scope_war` → war — Iterate through all wars related to the scope
- `any/every/ordered/random_strategic_objective` → state — Iterate through all Strategic Objective states from the scoped Country
- `any/every/ordered/random_subject_of_subject` → country — Any country below direct subjects in hierarchy
...
- `average_sol_for_slaves` — Compare average standard of living for enslaved pops
...
- `can_have_as_subject` — Checks if a country can have another country as a particular type of subject Where X = country and Y = subject type
...
- `can_lobbies_target` — Checks if target country is a valid target for lobbies in the scoped country → country
...
- `country_average_cultural_acceptance` — Compares the average acceptance of a culture that is present in a country against a value
...
- `country_average_religious_acceptance` — Compares the average acceptance of a religion that is present in a country against a value
...
- `country_has_country_monopoly` — Check if the scoped country has any country monopolies
- `country_has_primary_culture` — Checks if a culture is one of the primary cultures in the country → culture
...
- `country_ship_type_fraction` — Checks that a country has a certain fraction of a specific ship type
...
- `enactment_chance` — Compares the current enactment success chance in scope country (including values from enactment modifier)
- `enactment_chance_for_law` — Compares the enactment success chance in scope country for given law (including values from enactment modifier)
- `enactment_chance_for_law_without_enactment_modifier` — Compares the enactment success chance in scope country for given law but excludes values from enactment modifier
...
- `goods_production_rank` — Compare to a country's production leaderboard rank of a good "goods_production_rank(g:luxury_clothes)" < 10
...
- `has_claim` — Checks if country in scope has a claim on state/state region/country
...
- `has_diplomatic_pact` — Checks if two countries have an active diplomatic pact of type Where X = country and Y = diplomatic action type, is_initiator is optional parameter that checks to see if scope country is the origin...
- `has_diplomatic_relevance` — Checks if target country is diplomatically relevant for scope country → country
...
- `has_law` — Checks if a country has a certain law active → law_type
...
- `influence_usage` — Compares the consumed influence of the scoped country
...
- `is_adjacent_to_country` — Checks if country in scope is adjacent to a target country → country
- `is_adjacent_to_state` — Checks if country in scope is adjacent to a target state → state
...
- `is_home_country_for` — Checks if a country is the home country for target country → country
- `is_immune_to_revolutions` — Checks if the country has been set to be immune to revolutions via set_immune_to_revolutions Warning: This does not check if the country is naturally immune to revolutions due to for example being ...
...
- `is_revolutionary` — Check if the country, movement or interest group is revolutionary (scopes: country, interest_group, political_movement)
- `is_secessionist` — Check if the country, movement or interest group is secessionist (scopes: country, interest_group, political_movement)
...
- `nationalization_cost` — Compares the total cost of nationalizing all buildings in scope country owned by the government or pops of target country
...
- `num_diplomatic_pacts` — Compare to a country's number of diplomatic pacts of the specified type
...
- `num_subjects` — Compares the number of subjects of scoped country
...
- `potential_diplomatic_play_power_ratio` — Check the expected power ratio between scope country side and target country side in a potential diplomatic play Attempts to predict which sides involved participants will take (if any) so doesn't ...
...
- `power_bloc_share_power_projection_with` — Compare the share of Power Projection of the country in scope against all its Power Bloc members plus an additional country, returns -1 if not in a Power Bloc "power_bloc_share_power_projection_wit...
- `power_bloc_share_power_projection_without` — Compare the share of Power Projection of the country in scope against all its Power Bloc members minus one of the members, returns -1 if not in a Power Bloc "power_bloc_share_power_projection_witho...
...
- `radical_fraction` — Compares radical fraction in pops in state or country, all parameters except value are optional (scopes: country, state)
- `radical_population_fraction` — Compares the fraction of the population that are radicals in a given country
- `region_score` — Compares the aggregated AI strategy region score for the specified strategic region in country scope Returns 0 in case the country is player controled "region_score(sr:region_key)" >= 300
- `relative_authority` — Compares the unused fraction of authority for the scoped country
- `relative_bureaucracy` — Compares the unused fraction of bureaucracy for the scoped country
- `relative_influence` — Compares the unused fraction of influence for the scoped country
...
- `stall_chance_for_law_without_enactment_modifier` — Compares the enactment stall chance in scope country for given law but excludes values from enactment modifier
- `stall_chance_without_enactment_modifier` — Compares the current enactment stall chance in scope country but excludes values from enactment modifier
...
- `add_banned_goods` — Adds a total ban of a good to a country → goods
- `add_change_relations_progress` — Add progress towards changing relations between two countries
...
- `add_involvement` — Adds involvement for the scoped country in the given strategic region
...
- `add_primary_culture` — Adds a culture to the primary cultures of a country Where X is a culture scope → culture
...
- `add_taxed_goods` — Adds consumption taxes on a good to a country → goods
- `add_technology_progress` — Add technology progress Where X is a fixed point and Y is an technology
...
- `call_election` — Sets the next election date for country in N months
- `cancel_enactment` — Stops enacting the country's currently enacting law
- `change_infamy` — Change infamy of scope country
- `change_institution_investment_level` — Add/remove the investment level for the institution
- `change_relations` — Change relations between two countries
...
- `change_tension` — Change tension between two countries
...
- `complete_objective_subgoal` — Completes an objective subgoal
- `copy_laws` — Copies the constitution of the target country scope Warning: This stops any current enactment. → country
- `create_bidirectional_truce` — Create a bidirectional truce between two countries
- `create_character` — Creates a character, any option can be omitted.
- `create_diplomatic_catalyst` — Creates a new diplomatic catalyst
- `create_diplomatic_pact` — Create a diplomatic pact between two countries, with scope country as initiator
- `create_diplomatic_play` — Create a diplomatic play with the scoped object as target
- `create_incident` — Creates a diplomatic incident that generates infamy, with target country as the victim
- `create_military_formation` — Creates a military formation 	# Optional name
- `create_political_lobby` — Creates a new political lobby
- `create_political_movement` — Creates a political movement of the specified type in the country, culture/religion optional
- `create_power_bloc` — Creates a power bloc with the scoped object as leader
- `create_ship` — Creates a ship in a scoped country
- `create_unidirectional_truce` — Create a unidirectional truce for one country towards another
...
- `disable_temporary_hostilities` — Disable temporary hostilities of a given type between two countries
- `enable_temporary_hostilities` — Enable temporary hostilities between two countries for a duration
...
- `play_as` — Change which country scoped country's player will play as → country
...
- `remove_active_objective_subgoal` — Removes an active objective subgoal
- `remove_banned_goods` — Removes a total ban of a good from a country → goods
- `remove_company` — Removes company type from a country's companies → company_type
- `remove_diplomatic_pact` — Removes a diplomatic pact between two countries, with scope country as initiator
...
- `set_export_tariff_level` — Sets export tariff level for a good in scoped country
...
- `set_import_tariff_level` — Sets import tariff level for a good in scoped country
- `set_institution_investment_level` — Sets the investment level for an institution
- `set_market_capital` — Set market capital in a country scope Where X is a state region
- `set_military_wage_level` — Sets the military wage level of scoped country
- `set_mutual_secret_goal` — Set mutual secret AI goal for scope country and target country
...
- `set_relations` — Set relations between two countries
- `set_ruling_interest_groups` — Creates a government for the country in scope from a set of interest groups
- `set_secret_goal` — Set a secret AI goal for scope country towards another country
- `set_social_hierarchy` — Sets the social hierarchy the scoped country
- `set_state_religion` — Changes the state religion of the country to the specified religion Where X is a religion scope → religion
- `set_strategy` — Set AI strategy for scope country
- `set_tax_level` — Sets the overall tax level of scoped country
- `set_tension` — Set tension between two countries
- `start_enactment` — Starts enacting the specified law type for the country in scope → law_type
- `start_national_awakening` — Starts a cultural awakening
- `start_research_random_technology` — Scoped country starts research of any random technology they can
- `take_on_scaled_debt` — Transfers an amount of debt equal to the defined multiplier on target's max credit 	who
...
- `any/every/ordered/random_sea_node_adjacent_state` → state — Iterate through all states that share a sea node with a state
...
- `can_construct_building` — Checks if 1 level of <building_type> can be constructed in a scoped state
- `controls_treaty_port_province` — Checks if the scoped state controls the treaty port province in the state region
- `cultural_acceptance_delta` — Compare the current local delta (modification) to a culture's acceptance in the scoped state
...
- `is_strategic_objective` — Checks if the scoped State is a Strategic Objective of a Country → country
...
- `state_exports` — Compare total units of exports from scope to target state (if set) or world state in general (if no state specified)
...
- `state_imports` — Compare total units of imports from target to scope state
...
- `state_trade` — Compare total units of goods traded between scope state and target state (if set) or world state in general (if no state specified)
...
- `world_market_access` — Checks the world market access of the scoped state Worldmarket_access > 0.8
- `years_to_incorporate` — Checks how many years it would take for target country to incorporate scope state

### Effects (34)

- `activate_building` — Activate a building in a state
- `add_acceptance` — Adds an acceptance delta for a culture in a state
...
- `create_mass_migration` — Initiates mass migration of a specific culture from a origin country to a scoped state
- `create_pop` — Creates a pop in the scoped state
- `deactivate_building` — Deactivate a building in a state
...
- `set_state_owner` — Set State Owner → country
...
- `add_state_trait` — add state trait in a scoped state region
- `create_state` — creates a state in a state region
- `finish_harvest_condition` — Finish a harvest condition of type in the scoped state region
...
- `remove_state_trait` — remove state trait in a scoped state region
- `set_owner_of_provinces` — Gives a set of provinces in a state region to a specific country
- `spawn_entity_effect` — Spawns a temporary entity at the center of the scoped state region for a number of seconds
- `start_earthquake_effect` — Starts an earthquake camera shake centered on the scoped state region
- `start_harvest_condition` — Start a harvest condition of type (or refresh it if already exists) in the scoped state region
- `start_harvest_condition_with_params` — Start a harvest condition of type in scoped state region where intensity and duration(in days) are optionally provided
...
- `has_social_class` — Checks if the scoped pop belongs to a specific social class
...
- `add_pop_wealth` — Adds the wealth of the pop Where the distribution adding to wealth of the pop
- `change_pop_culture` — Changes the culture of the scoped pop to a specified culture by a specified percentage
- `change_pop_religion` — Changes the religion of the scoped pop to a specified religion by a specified percentage
...
- `move_pop` — Moves the scoped pop in its entirety to the specified state (they become unemployed) NOTE: VERY IMPORTANT! This effect _may_ change the pop type of the moved pop. → state
- `set_pop_literacy` — Sets the literacy of the pop Where the ratio is a script value computing the percentage of (workforce) pops that will be literate
- `set_pop_qualifications` — Sets the pop qualifications of the pop for the given type Where the qualifications is a script value computing the percentage of (workforce) pops that will have the qualifications
- `set_pop_wealth` — Sets the wealth of the pop Where wealth is a script values
...
- `any/every/ordered/random_character_role` → character_role — Iterate through all roles of the scoped character

### Triggers (49)

- `age` — Compares the character age
- `amendment_stance` — Compares the stance of the scoped character, movement or interest group has on the specified amendment type, assuming the amendment type has a parent law 	amendment (scopes: character, interest_group, political_movement)
- `can_agitate` — Check if the scope character can agitate the target country → country
- `character_acceptance` — Compares the acceptance of the scoped character in the target country against an acceptence value
...
- `could_support_political_movement` — Check if a character could potentially support a political movement
...
- `has_ideology` — Check if scoped character or interest group has ideology (scopes: character, interest_group, political_movement)
...
- `is_interest_group_leader` — Checks if character is the leader of their interest group is_interest_group_Leader = bool
...
- `law_enactment_stance` — Compares the stance of the scoped character, movement or interest group has about enactment of the specified law compared to the current active law in the same group (scopes: character, interest_group, political_movement)
- `law_stance` — Compares the base stance of the scoped character, movement or interest group has about the specified law, ignoring the current active law in the same group (scopes: character, interest_group, political_movement)
...
- `kill_character` — Kill a character kill_character
...
- `retire_character` — Retire a character retire_character
- `retire_character_if_should_be_culled` — Conditionally retire a character if it should be culled retire_character_if_should_be_culled
...
- `set_character_as_executive` — Makes scope character the executive of target company → company
...
- `set_home_state` — Changes the home state of the scoped character → state
...
- `any/every/ordered/random_market_goods` → market_goods — Iterate through all active (market) goods in a market
- `any/every/ordered/random_scope_country` → country (scopes: market, state_region, strategic_region) — Iterate through all countries with a presence in the supported scope (currently: market, strategic region)
...
- `is_adjacent_to_market` — Checks if market in scope is adjacent to a target market → market
- `market_consumption_share` — Compare fraction of scope market consumption coming from target country
- `market_exports` — Compare total units of exports from scope to target market (if set) or world market in general (if no market specified)
- `market_exports_reliance` — Compare fraction of buy orders in scope market that are due to exports to target market (if set) or world market in general (if no market specified)
- `market_has_goods_shortage` — Check if market has a shortage on any of its building inputs
- `market_imports` — Compare total units of imports from target to scope market
- `market_imports_reliance` — Compare fraction of buy orders in scope market that are due to imports from target market (if set) or world market in general (if no market specified)
- `market_number_goods_shortages` — Check how many shortages a market has on any of its building inputs
- `market_number_goods_shortages_with` — Check how many shortages a market has on any of its building inputs, plus the ones from the target country "market_number_goods_shortages_with(scope:target) >= 2"
- `market_number_goods_shortages_without` — Check how many shortages a market has on any of its building inputs, subtracting the ones from the target country "market_number_goods_shortages_without(scope:target) >= 2"
- `market_production_share` — Compare fraction of scope market production coming from target country
- `market_trade` — Compare total units of goods traded between scope market and target market (if set) or world market in general (if no market specified)
- `market_trade_reliance` — Compare fraction of buy and sell orders in scope market that are due to trade with target market (if set) or world market in general (if no market specified)
...
- `has_potential_supply` — Check if the market goods or state goods has a potential supply, either though local production or theoretical import (scopes: market_goods, state_goods)
...
- `market_goods_buy_orders` — Checks if market goods has the specified number of buy orders Where X = fixed point
- `market_goods_cheaper` — Checks if market goods is at least the specified percentage cheaper than base price Where X = fixed point
- `market_goods_consumption` — Checks if market goods has the specified number of total consumption Where X = fixed point
- `market_goods_delta` — Checks if market has the specified goods delta (production + imports) - (consumption + exports) Where X = fixed point
- `market_goods_export_share` — Checks if market goods exports are the specified fraction of world market exports Where X = fixed point
- `market_goods_exports` — Checks if market goods has the specified number of exports Where X = fixed point
- `market_goods_has_goods_shortage` — Check if market goods has a shortage in the market
- `market_goods_import_share` — Checks if market goods imports are the specified fraction of world market imports Where X = fixed point
- `market_goods_imports` — Checks if market goods has the specified number of imports Where X = fixed point
- `market_goods_pricier` — Checks if market goods is at least the specified percentage more expensive than base price Where X = fixed point
- `market_goods_production` — Checks if market goods has the specified number of total production Where X = fixed point
- `market_goods_sell_orders` — Checks if market goods has the specified number of sell orders Where X = fixed point
- `market_goods_shortage_ratio` — Compares the shortage ratio of a market goods in its market
- `market_prestige_goods_buy_orders` — Compare the buy orders of a specific prestige good type in the market
- `market_prestige_goods_consumption` — Compare the consumption value of a specific prestige good type in the market
- `market_prestige_goods_delta` — Compare the delta (sell orders - buy orders) of a specific prestige good type in the market
- `market_prestige_goods_export_share` — Compare the world market expore share of a specific prestige good type in the market
- `market_prestige_goods_exports` — Compare the export value of a specific prestige good type in the market
- `market_prestige_goods_import_share` — Compare the world market export share of a specific prestige good type in the market
- `market_prestige_goods_imports` — Compare the import value of a specific prestige good type in the market
- `market_prestige_goods_production` — Compare the production value of a specific prestige good type in the market
- `market_prestige_goods_sell_orders` — Compare the sell orders of a specific prestige good type in the market
...
- `state_goods_cheaper` — Checks if state goods is at least the specified percentage cheaper than base price Where X = fixed point
- `state_goods_consumption` — Checks if state goods has the specified number of total consumption Where X = fixed point
- `state_goods_delta` — Checks if state has the specified goods delta (production - consumption) Where X = fixed point
- `state_goods_has_local_goods_shortage` — Check if state goods has a shortage in a state, but NOT in the whole market
- `state_goods_pricier` — Checks if state goods is at least the specified percentage more expensive than base price Where X = fixed point
- `state_goods_production` — Checks if state goods has the specified number of total production Where X = fixed point
...
- `any/every/ordered/random_preferred_law` → law — Iterate through all active and possible laws in an interest group's country, ordered by how much they prefer that law
...
- `is_member_of_party` — Checks if Interest Group is a member of target party → party
...
- `prefers_law` — Checks if the scoped interest group prefers the specified law to the comparison law
- `would_sponsor_amendment` — Check if scoped IG would sponsor an amendment of a given type → amendment_type
...
- `is_being_suppressed` — Check if scoped movement is being suppressed
- `is_political_movement_type` — Check if a political movement is a particular type
- `movement_can_cause_obstinance` — Check if the political movement can generate obstinance in states
- `movement_is_causing_obstinance` — Check if the political movement is causing obstinance in any state
- `movement_pressure` — Compares the pressure of the scoped movement on the target IG movement_pressure( ig:intelligentsia ) > 0.1
...
- `any/every/ordered/random_lobby_member` → interest_group — Iterate through all interest group members of a lobby
...
- `change_appeasement` — Change appeasement of scope political lobby
...
- `any/every/ordered/random_member` → interest_group — Iterate through all interest group members of a party

### Triggers (5)

- `election_momentum` — Compare election momentum of the scoped party against a value
- `has_party_member` — Checks if the target interest group is a member of scope party → interest_group
...
- `is_primary_culture_of` — Checks if culture is any of a country's primary cultures → country
- `shares_heritage_trait_group_with_any_primary_culture` — Checks if culture shares any trait group marked as 'heritage' with any of a country's primary cultures → country
- `shares_heritage_trait_group_with_culture` — Checks if culture shares any trait group marked as 'heritage' with another culture → culture
- `shares_heritage_trait_with_any_primary_culture` — Checks if culture shares any trait marked as 'heritage' with any of a country's primary cultures → country
- `shares_heritage_trait_with_culture` — Checks if culture shares any trait marked as 'heritage' with another culture → culture
- `shares_language_trait_group_with_any_primary_culture` — Checks if culture shares any trait group marked as 'language' with any of a country's primary cultures → country
- `shares_language_trait_group_with_culture` — Checks if culture shares any trait group marked as 'language' with another culture → culture
- `shares_language_trait_with_any_primary_culture` — Checks if culture shares any trait marked as 'language' with any of a country's primary cultures → country
- `shares_language_trait_with_culture` — Checks if culture shares any trait marked as 'language' with another culture → culture
- `shares_tradition_trait_with_any_primary_culture` — Checks if culture shares any trait marked as 'tradition' with any of a country's primary cultures → country
- `shares_tradition_trait_with_culture` — Checks if culture shares any trait marked as 'tradition' with another culture → culture

### Effects (10)

- `add_cultural_community_in_state` — Adds a cultural community for the scoped culture in the target state. → state
- `add_cultural_obsession` — Adds a new obsession to the culture in scope Where X is a goods
- `add_cultural_taboo` — Adds a new taboo to the culture in scope Where X is a goods
- `add_fervor` — Adds fervor to a scoped culture
- `add_tradition_trait` — Adds a new tradition trait to the culture in scope Where X is a tradition trait
- `remove_cultural_obsession` — Removes a new obsession to the culture in scope Where X is a goods
- `remove_cultural_taboo` — Removes a taboo from the culture in scope Where X is a goods
- `remove_tradition_trait` — Removes an existing tradition trait from the culture in scope Where X is a tradition trait
...
- `has_religious_taboo` — Checks if a religion has a certain goods as taboo
...
- `shares_heritage_trait_group_with_state_religion` — Checks if the religion shares any heritage trait group with a country's state religion → country
- `shares_heritage_trait_with_religion` — Checks if the religion shares any heritage trait with another religion → religion
- `shares_heritage_trait_with_state_religion` — Checks if the religion shares any heritage trait with a country's religion → country
...
- `any/every/ordered/random_scope_initiator_ally` → country — Iterate through all allies to an initiator in a: diplomatic play
- `any/every/ordered/random_scope_play_involved` → country — Iterate through all involved in a: diplomatic play
- `any/every/ordered/random_scope_target_ally` → country — Iterate through all allies to a target in a: diplomatic play
...
- `remove_initiator_backers` — Remove a tag/scope country from the initiator side of a diplomatic play
- `remove_target_backers` — Remove a tag/scope country to the target side of a diplomatic play
...
- `any/every/ordered/random_participant` → country — Any of two participants of the diplomatic pact in a scope
...
- `is_forced_pact` — Check if a diplomatic pact has a forced duration due to reasons such as a sway or obligation
...
- `formation_ship_type_fraction` — Checks that a formation has a certain fraction of a specific ship type
...
- `deploy_to_front` — Deploys the scope formation to the target front Can also take a base_camp parameter, will use given province to search for the nearest connected base camp deploy_to_front
- `fully_mobilize_army` — Fully mobilizes scope army
- `mobilize_army` — Mobilizes scope army
- `teleport_to_front` — Teleports the scope formation to the target front Can also take a base_camp parameter, will use given province to search for the nearest connected base camp teleport_to_front
...
- `num_front_casualties` — Checks the number of casualties for the target country in the scoped front
- `num_front_dead` — Checks the number of dead for the target country in the scoped front
- `num_front_wounded` — Checks the number of wounded for the target country in the scoped front
...
- `any/every/ordered/random_combat_unit` → new_combat_unit (scopes: battle, building, front, hq, military_formation) — Iterate through all combat units of input scope Supported scopes: building, military formation, front, battle
...
- `any/every/ordered/random_power_bloc_member` → country — Iterate through all members of the scoped power bloc including the leader
...
- `free_principle_slots` — Compare a power bloc's free principle slot count power_bloc
...
- `leverage_advantage` — Checks that the leverage advantage between power bloc and target country meets condition power_bloc
...
- `power_bloc_rank` — Compare a power bloc's rank power_bloc
...
- `total_used_principle_levels` — Compare a power bloc's total number of active principle levels power_bloc
- `used_principle_slots` — Compare a power bloc's used principle slot count power_bloc
...
- `add_principle` — Adds principle to powerbloc power_bloc
- `remove_principle` — Removes principle from powerbloc power_bloc
...
- `any/every/ordered/random_owned_country` → country — Iterate through all countries owned by the scoped company
- `any/every/ordered/random_scope_regional_hqs` → building — Iterate through regional HQs of the scoped company
...
- `company_is_prosperous` — Check if a company is considered to be prosperous
...
- `any/every/ordered/random_possible_amendment_type` → amendment_type — Iterate through amendment types that could be added to the scoped law
- `any/every/ordered/random_scope_amendment` → amendment — Iterate through amendments of the scoped law
...
- `has_amendment` — Check if scoped law has an amendment of the given type → amendment_type
...
- `is_same_law_group_as` — Checks if scope law type is in the same group as the target law type scope → law_type
- `law_progressiveness_difference` — Compares the progressiveness of the scoped law type to the target law type, higher value means greater difference "law_progressiveness_difference( scope:target_law )" > 50
- `progressiveness` — Compare the progressiveness of the law type in scope law_type:law_isolationism.progressiveness > 50
...
- `any/every/ordered/random_scope_je_involved` → country — Iterate through all involved in a journal entry
...
- `scripted_bar_progress` — Determines the progress of a scripted progress bar

### Effects (5)

- `add_involved_country` — Makes target country an involved country in scope journal entry → country
- `add_progress` — Adds progress to a journal entry progressbar
- `remove_involved_country` — Removes target country from involved countries in scope journal entry → country
- `set_bar_progress` — Sets progress for a journal entry scripted progressbar
- `set_target_technology` — Sets a (new) target technology scope for a journal entry
...
- `any/every/ordered/random_scope_article` → treaty_article — Iterate through articles of the scoped treaty
- `any/every/ordered/random_scope_article_option` → treaty_article_options (scopes: treaty_options, treaty) — Iterate through article_options of the scoped treaty or treaty_options
...
- `world_market_delta` — Checks if goods has the specified number of exports minus imports in the world market Where X = fixed point
- `world_market_exports` — Checks if goods has the specified number of exports in the world market Where X = fixed point
- `world_market_imports` — Checks if goods has the specified number of imports in the world market Where X = fixed point
...
- `unit_formation_has_commander` — Checks if ther formation of the scoped combat unit has the provided character as one of its commanders → character
...
- `any/every/ordered/random_province` → state — Iterate through all Provinces in the scoped State
...
- `country_definition_has_culture` — Checks if a culture is one of the cultures of the country definition → culture
...
- `any/every/ordered/random_power_bloc` → power_bloc — Iterate through all power blocs
- `any/every/ordered/random_state` → state — Iterate through all states globally
- `any/every/ordered/random_state_region` → state_region — Iterate through all state regions
...
- `is_target_in_variable_list` — Checks if a target is in a variable list Where X is the name of the variable Where Y is an event target
- `is_template_used` — Checks if character template has already been used
- `is_theme_selected` — Does the player have the theme selected in settings
...
- `change_variable` — Changes the value or a numeric variable Where X is the name of the numeric variable to modify Where the valid operations are add, subtract, multiply, divide, modulo, min and max Where Y is a fixed ...
...
- `execute_event_option` — Execute effects of the specified event option in the specified event
- `hidden_effect` — Enclosed effects are not shown in tooltips
- `if` — Executes enclosed effects if limit criteria are met
- `post_audio_event` — Runs an audio even on a "persistent" audio object
...
- `remove_list_variable` — Removes the target from a variable list Where X is the name of the variable Where Y is an event target
...
- `round_variable` — Rounds a variable to the nearest specified value Where X is the name of the variable Where Y is a script value
...
- `save_temporary_scope_value_as` — Saves a numerical or bool value as an arbitrarily-named temporary target to be referenced later in the same effect
- `set_global_variable` — Sets a variable set_variable = { name = X value = Y days = Z } Where X is the name of the variable used to then access it Where Y is any event target, bool, value, script value or flag (flag:W) An ...
- `set_local_variable` — Sets a variable set_variable = { name = X value = Y days = Z } Where X is the name of the variable used to then access it Where Y is any event target, bool, value, script value or flag (flag:W) An ...
- `set_variable` — Sets a variable Where X is the name of the variable used to then access it Where Y is any event target, bool, value, script value or flag (flag:W) An optional days where Z is the number of days or ...
- `show_as_tooltip` — Enclosed effects are only shown in tooltips (but are not actually executed)
...
- `sort_variable_list` — Sorts a variable list Where X is the name of the variable Where Y is some math script to determine sort order
- `start_tutorial_lesson` — Starts the tutorial lesson with the given key.
- `switch` — Switch on a trigger for the evaluation of another trigger with an optional fallback trigger.
- `trigger_event` — Triggers an event for the current scope trigger_event
