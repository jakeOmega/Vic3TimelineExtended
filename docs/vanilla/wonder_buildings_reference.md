# Wonder Buildings — Reference & Design
...
- **Current state of this tech:** Previously unlocked an orphan `pm_base_building_space_power` PM (never wired into a building). The PM has been removed; the wonder is the sole consumer of this tech.
...
The legacy `pm_base_building_space_power` PM has been removed. The solar collector wonder is the sole consumer of `space_based_solar_power`; players who research that tech build the wonder instead of switching a PM. The old PM was orphaned (defined but never attached to renewable power plants) and absurdly profitable, so removing it is a net cleanup.
