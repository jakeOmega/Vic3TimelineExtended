...
- **Trade Advantage on the global market** for the producer (see § 14). Since 1.13.9, Trade Centers also gain Trade Advantage scaled by the prestige-good share of their inputs (prestige Merchant Marine).
...
- A **modest combat bonus** to military formations consuming prestige goods (small arms, ammunition, artillery, etc.) as supply — since 1.13.6 this actually applies to units in combat.
...
This **mod** turns Construction into a market good — built and consumed like any other. The sections below describe vanilla's behavior; for the mod's variant see `docs/systems/mod_systems.md`.
...
Since 1.13.9, drains have a dedicated modifier shape: most vanilla flat *negative* modifiers were converted to capacity-cost modifiers (`country_authority_cost_add`, `country_bureaucracy_cost_add`, `country_influence_cost_add`) that render as costs in the capacity breakdown rather than as negative production.
...
- Since 1.13.9, a bonus scaling with the prestige-good share of the Trade Center's inputs (prestige Merchant Marine).
...
When the mod adds new `ship_type_*` entities, register the relevant `ship_battle_against_ship_type_<ship>_<axis>_<add|mult>` combinations explicitly per the rule in `docs/guides/scripting_best_practices.md` — vanilla auto-registration is patchy.
...
- **Engine syntax & validation rules:** `docs/guides/scripting_best_practices.md`.
- **Mod systems that hook these vanilla mechanics:** `docs/systems/mod_systems.md`, `docs/systems/journal_entry_systems.md`. Banking cycle (IP/dividend pressure), construction-as-a-good (mod's market-mediated construction layer over the vanilla two-FCFS queues), tourism (state-scope dynamic modifiers), wonder buildings (long-build construction-good drains), migration crowding (per-state pull modifiers), Bulk Transportation (Merchant Marine relocalization + broadened production).
- **Vanilla company / charter implementations:** `docs/vanilla/vanilla_company_buildings_reference.md`.
- **What changed at the last vanilla bump:** `docs/guides/vanilla_patch_runbook.md` § "Engine-doc diff" and § "Known vanilla renames".
...
4. **Don't duplicate:** modifier validation rules, scope chain quirks, and engine-syntax gotchas belong in `docs/guides/scripting_best_practices.md`. This file is *concepts*, not *syntax*.
