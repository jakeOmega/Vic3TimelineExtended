...
Each condition fires per-state with a randomized intensity (a discrete level inside the type's defined `range`) and duration. Each state in a region gets its own roll, so a state whose history diverges from siblings (different ownership, different traits) may keep the condition longer or shorter than the others. The full list of effects per condition is in the type definition's `modifier` block; conditions can be positive (optimal sunlight, moderate rainfall, pollinator surge) or negative (the rest). Since 1.13.9 condition types are restricted to applicable geographic regions — frosts/ice, tropical storms, and heatwaves no longer fire outside their climates, and the engine gained region-scoped iterators (e.g. `any_state_in_tropical_seas`, `any_state_in_polar_seas`) to support this. Modding note: 1.13.9 also deregistered some per-condition `state_harvest_condition_*` modifier types (hailstorm, torrential_rains) while keeping the condition types themselves — mods referencing them must re-register in `common/modifier_type_definitions/`.
