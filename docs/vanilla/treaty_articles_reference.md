# Treaty Articles — Reference & Design
...
|---|---|---|
| `currency_peg` | pegger → anchor | the pegger becomes *anchored* (kind 1): no policy rate of its own, the anchor's rate + 0.5, the anchor's exchange rate, imported credibility, −0.5pp credit standing, and a Peg Confidence that overvaluation drains (crisis `te_peg.2`). The anchor gains a reserve-currency standing cut and prestige, and no obligation. → 200 leverage |
| `imposed_currency_peg` *(hostile)* | pegger → anchor (**source concedes**) | the same anchored state on worse terms: spread **1.0**, **no** −0.5pp standing cut, −10 legitimacy on the pegger, no prestige for the anchor — and *Break the peg* is unavailable, so the way out is the treaty UI the enforce play answers. Demandable only against a country **in default**, by a major power or better that holds a dial. Infamy base 8. → 400 leverage |
| `debt_receivership` *(hostile)* | debtor → receiver (**source concedes**) | externally supervised finances, not a guarantee at gunpoint: the debtor gets the guarantee's relief and pays **0.1% of its GDP a month** to the receiver, −10 legitimacy; its default fires no `te_lolr.1`, because a receiver is a creditor. **Ends itself** twelve months after the debtor holds `scaled_debt < 0.25` out of default. Demandable only against a country **in default**. Infamy base 10. → 500 leverage |
| `swap_line` | provider → recipient | recipient −1pp risk premium, +2 peg confidence a month, orderly hot-money exit in a panic; **drawn on as a loan** (0.1% of recipient GDP a month, provider → recipient) in a *financial* crisis — not merely a war — capped at 2% of recipient GDP, repaid at the same rate out of the crisis and settled in a lump when the line ends. **One provider per recipient.** → 150 leverage |
| `lender_of_last_resort` | guarantor → ward | the ward's debt-load premium halved, imported crashes start shallower; the ward's default fires `te_lolr.1` on the guarantor (honour / renege). **The ward's call counter** then raises the honour cost (to 15%), tapers the relief to nothing by the third default, walks the odds toward Renege and lengthens the cooldown — and counts −20 a call against it at signing. → 300 leverage |
...
**Gating.** Symmetric conditions in `possible`; everything directional in `requirement_to_maintain` (the anchor keeps a dial; the provider keeps a national bank; the guarantor is not in default) or `can_ratify` (rank / GDP tests, one peg at a time, one guarantor per ward, one swap-line provider per recipient — the existing-treaty checks use the vanilla `is_renegotiation` / `amended_treaty` guard and only resolve there). **The two hostile articles' pretext obeys the same rule and is therefore tested twice**: `possible` asks whether a country in default exists in *either* direction, and `can_ratify` holds the imposer to the side that actually has the pretext and carries the tooltip. `possible` runs before the direction is fixed, so `scope:source_country` / `scope:target_country` are not handles there.
