...
**Missing leverage, no coercive variant, and a gameable AI/exploit surface — reviewed, then planned, not built:** the findings are `docs/systems/monetary_policy_design.md` §15B; the staged phase-6 plan is §15C, with the owner's rulings on every decision (H1–H7, L1–L2, C1–C4) tabled in §15C.5 — the swap line as a repayable, capped, single-provider loan drawn only in a financial crisis; a guarantee that remembers its calls; the AI's own post-signing withdrawal on the re-scored articles; `non_fulfillment = withdraw` on war / expulsion; `country_treaty_leverage_generation_add` 200 / 150 / 300; and two new hostile, enforceable articles, `imposed_currency_peg` (113) and `debt_receivership` (114), demandable only against a country in default, with no hostile swap line. The per-pegger prestige fan-out stays as shipped by ruling (H5).
