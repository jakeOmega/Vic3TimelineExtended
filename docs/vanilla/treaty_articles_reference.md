...
**Missing leverage, no coercive variant, and a gameable AI/exploit surface — reviewed, then planned, not built:** the findings are `docs/systems/monetary_policy_design.md` §15B; the staged phase-6 plan is §15C — the swap line as a repayable, capped, single-provider loan; a guarantee that remembers its calls; one managed, capped prestige term replacing the three articles' flat `country_prestige_mult` lines; `non_fulfillment = withdraw` on war / expulsion; `country_treaty_leverage_generation_add` 200 / 150 / 300; and two new hostile, enforceable articles, `imposed_currency_peg` (113) and `debt_receivership` (114), with no hostile swap line. Owner decisions H1–H7, L1–L2, C1–C4 are tabled in §15C.5.
