...
| [`audits/civil_war_inheritance_audit.md`](audits/civil_war_inheritance_audit.md) | What a revolution's winner inherits (read from saves) and the 16 places the mod loses state because of it: JE modifiers wiped, `immediate` re-run, completed entries re-armed, lists and containers dropped, the loser's stocks discarded. Findings open unless marked | Touching a system that keeps country state across a civil war; fixing one of its findings; adding an `on_civil_war_won` repair |
