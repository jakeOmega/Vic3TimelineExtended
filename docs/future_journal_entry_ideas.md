...
**Implementation status:** All 8 JEs below have MVP implementations (JE definition, 2 monthly pulse events, 1 fail-state event, modifiers_while_active, completion/fail/timeout modifiers, full localization). See `docs/mod_systems.md` § "Social Movement Journal Entries (MVP)" for the file index and summary table. Planned expansion includes additional events (5-6 per JE), scripted toggle buttons, and cross-JE interactions.
