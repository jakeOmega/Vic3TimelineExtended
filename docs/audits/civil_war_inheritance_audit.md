...
- **FIXED (#460).** At `on_revolution_start` / `on_secession_start` the rebels (`scope:target`) get a copy of each diffusion modifier the original holds (`agdiff_copy_diffusion_from_root`), and nothing the nation lacked. They are also marked `agdiff_cw_copied`. At `on_civil_war_won` the winner gets the backfill as a backstop only if it carries no such marker, which means the war began before this build (a rebel winner holds the marker, a loyalist inherits it), and only if it is recognized (the broadcast's filter). The marker is then removed. The remaining exposure: a war begun before the build in a recognized laggard (a country the `decolonization` tech recognized after the broadcasts) hands its winner the diffusion. The overlord- and company-subject release hooks now run the backfill too; the release and formation hooks stay unfiltered, as before. The first-mover reward is restored from a new grant-month record (`agdiff_first_mover_month`), not from the permanent `is_world_first_<tech>`. It comes back at the strength it had decayed to, for the whole years it had left (to the nearest year, off a ladder of literal durations). Rewards granted before the fix have no record and are not restored. `first_<tech>_country` is re-pointed from the dead loser.
