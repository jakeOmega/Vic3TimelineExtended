...
- **GitHub auth for the vanilla clone**: the Routines GitHub proxy only auto-authenticates the routine's primary repo (`Vic3TimelineExtended`). The cross-repo clone of the private `jakeOmega/vic3` requires `GH_TOKEN` (or `GITHUB_TOKEN`) set in the routine's Environment Variables UI — a PAT with `repo:read` scope on `jakeOmega/vic3`. `cloud_setup.sh` embeds the token in the clone URL and runs a preflight `git ls-remote` probe that fails fast with an actionable error if the token is missing/wrong (see issue #99). In local dev, leaving the env var unset falls back to whatever credentials git is already configured with.
