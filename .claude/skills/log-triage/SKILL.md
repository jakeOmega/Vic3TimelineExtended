...
- 2026-05-24: **Ad-hoc `/logs/error?...` queries need `mod_noise=hide` too, not just `vanilla_bugs=hide`.** The canonical *debug* command sets both, but when you drop to error.log with a hand-built query (e.g. `mod_only=unknown&vanilla_bugs=hide`) and omit `mod_noise=hide`, already-registered mod noise re-surfaces and looks like a fresh finding. This session the harvest-condition sound-entity warnings (`Couldn't find any animation state for harvest condition type` — already in `mod_known_noise.md`, tracked `open_issues.md#L7`) appeared "new" purely because the error.log query lacked `mod_noise=hide`. Mirror the canonical filter set on every family.
- 2026-05-24: **`GetCustom` in a JE `name` loc has no vanilla precedent and emits a per-render `jomini_custom_text.h:91 "Object of type 'country' is not valid for 'X'"` burst** — even when the title renders correctly. A JE name like `[GetPlayer.GetCustom('te_banking_cycle_title')]` (custom loc `type = country`) renders fine but the JE name appears to evaluate in a `journal_entry` data context that validates the country-typed `GetCustom` against the call-site rather than `GetPlayer`'s return. Measured ~83 error.log writes in a ~1s burst per panel render (0 in debug.log); cosmetic, only cost is synchronous-write hitch. Registered as tracked mod noise (`mod_known_noise.md`, `open_issues.md#L11`). Candidate fix (resolve the country from the JE scope chain instead of `GetPlayer`) deferred. Discriminator: `grep -rln "GetCustom" "<vanilla>/localization/english/"*journal*` returns nothing — vanilla never does this.
