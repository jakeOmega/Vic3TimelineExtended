...
| 5 | `common/script_values/st_res_script_values.txt` | One full per-good section (10 script values: decay_rate, weekly_decay, actual_rate, actual_rate_base_applied, weekly_delta, capacity, good_mult, max_withdrawable, max_storable, sale_profit) appended after the existing `--- TANKS ---` section, plus **two** widget accessors at the end with their siblings: `st_res_<GOOD>_fill_pct` and `st_res_<GOOD>_last_net`, plus **nine** reserve-policy values (`_price_up`, `_price_down`, `_price_rel`, `_price_signal`, `_unit_price`, and the four `_policy_*_limit` stepper guards). The two accessors and `_price_signal` **must be `has_variable`-guarded** — the widget evaluates them every frame. See template. |
