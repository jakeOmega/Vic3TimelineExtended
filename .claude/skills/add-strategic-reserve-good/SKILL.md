...
The Strategic Reserve is a country-scoped stockpile system anchored on `je_strategic_reserve` and the `building_strategic_reserve_hub` building. Each covered good has its own stored stockpile, configurable weekly storage/withdraw rate, decay rate, an optional **price-triggered policy** that re-decides that rate weekly, and one collapsible row in the **reserve inventory widget** (`gui/journal_entry_widgets/strategic_reserve_widget.gui`) carrying its fill bar, stored / capacity, net weekly movement, annual decay rate, status label, decrease/stop/increase controls, policy summary and a collapsible settings panel. The system already covers `grain`, `ammunition`, `oil`, `small_arms`, `artillery`, `aeroplanes`, `tanks`, `fertilizer` (displayed as Chemicals). Adding any new good touches ~14 files with a strict copy-paste-modify pattern — miss one and the good silently does nothing in-game.
...
3. **Base annual decay rate.** Fraction of stockpile lost per year. Existing reference: grain=0.25 (perishable food), aeroplanes=0.04 (fabric/engines age), fertilizer (Chemicals)=0.04, tanks=0.025, ammunition=0.02, small_arms=0.015, artillery=0.01, oil=0.005 (stable). Pick by analogy. This also fixes how many decimals the row's *Decay: X%/yr* cell needs — enough to show the good's finest tech step (file 14).
...
| 5 | `common/script_values/st_res_script_values.txt` | One full per-good section (11 script values: annual_decay_rate, decay_rate, weekly_decay, actual_rate, actual_rate_base_applied, weekly_delta, capacity, good_mult, max_withdrawable, max_storable, sale_profit) appended after the existing `--- TANKS ---` section, plus **two** widget accessors at the end with their siblings: `st_res_<GOOD>_fill_pct` and `st_res_<GOOD>_last_net`, plus **ten** reserve-policy values (`_price_up`, `_price_down`, `_price_rel`, `_price_signal`, `_unit_price`, the four `_policy_*_limit` stepper guards, and `_policy_budget_max`, the budget stepper's ceiling). The two accessors and `_price_signal` **must be `has_variable`-guarded** — the widget evaluates them every frame. See template. |
...
| 10 | `gui/journal_entry_widgets/strategic_reserve_widget.gui` | 1 `widget_je_st_res_inventory_row` instance in the root flowcontainer: datacontext + visible + tooltip + 12 blockoverrides (the first two, `row_collapse_toggle` and `row_details_visible`, carry the good's `st_res_row_collapsed_<GOOD>` GUI key), the last of which nests a `widget_je_st_res_policy_panel` with its datacontext, visibility key and 8 value cells. **No `type` needs to change** — the policy panel's op codes are identical for every good. |
...
| 14 | `localization/english/te_miscellaneous_l_english.yml` | 10 keys: `st_res_<GOOD>_store_flow`, `st_res_<GOOD>_withdraw_flow`, `st_res_row_<GOOD>_name`, `st_res_row_<GOOD>_amount`, `st_res_row_<GOOD>_decay` (pick its `%N` decimals per decision 3), `st_res_row_<GOOD>_status`, `st_res_row_<GOOD>_flow`, `st_res_row_<GOOD>_tooltip`, `st_res_row_<GOOD>_policy`, `st_res_row_<GOOD>_policy_reason`. Every other policy string (policy names, presets, settings labels, tooltips, all ten explanations) is shared and already exists. |
| 15 | `localization/english/te_concepts_l_english.yml` | 2 keys: `st_res_<GOOD>_store_flow_desc`, `st_res_<GOOD>_withdraw_flow_desc`. (`organize_loc.py` files the `_desc` pair here and the row tooltip in misc.) |
