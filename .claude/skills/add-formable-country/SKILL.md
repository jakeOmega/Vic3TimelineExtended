...
- **`on_country_formed` extends, doesn't replace**: Vic3 merges `on_xxx` definitions across files. The mod already extends it in `common/on_actions/extra_on_actions.txt` (search `on_country_formed = {`). The cleanest way to wire a custom flavor event for a new tag is to add it to that block's `events = { … }` list with the event self-guarding on its tag (`trigger = { c:TAG ?= this }`) — you can't partial-edit vanilla's giant if/else chain anyway. Worked example: `events/te_formable_formation_events.txt` + `common/scripted_effects/te_formable_claims_effects.txt` (the India/Indonesia core-shrink + claims-on-formation pass).
- **Generic `formation.17` auto-fires** for new tags not on its NOR-list — it's triggered from the **final `else`** of vanilla `on_country_formed` (`00_code_on_actions.txt`), so the `NOR` inside `events/misc_unifications.txt:1042` is a self-guard. For MVP, leave it (it grants `unification_prestige` + `unification_claims_effect`, i.e. claims on all cultural-homeland states). To replace it with bespoke flavor, redeclare `formation.17` in a mod events file (e.g. `events/te_formation_overrides.txt`) with your tag added to its `NOR`. **Verify in-game**: grep `debug.log` for `Duplicated key formation.17` — if present, the redeclaration was dropped and you must fall back to a whole-file override of `events/misc_unifications.txt` (re-sync on vanilla bumps).
...
- **Candidacy is gated on CULTURE, not capital/region** (major formations). The engine offers a country as a unification candidate iff it has ≥1 of the formable's primary `cultures` AND passes `can_be_formation_candidate` (vanilla checks only `country_rank` there). It does **not** check the candidate's capital location — `geographic_region`/`states` only sets the territorial % needed to *form*, not who's eligible. Vanilla avoids leaks via tight, region-coherent culture lists; the mod's broad pan-region lists **overlap across regions** (e.g. `misri` ∈ both AFU and DAR, `malay` ∈ both NUS and DAR, `bengali/panjabi/sindi/baluchi` ∈ both BHA and DAR), so a Misri-primary Arabia leaks in as an African Union candidate. **Fix**: bind `can_be_formation_candidate` to a capital-in-region check — `is_major_<region>_power = yes` for the pan-region unions (these triggers already exist in `common/scripted_triggers/formable_gating_triggers.txt` and combine `country_rank >= major_power` + `capital = { is_in_geographic_region = ... }`), or an explicit `capital = { is_in_geographic_region = geographic_region_<x> }` line for state-list/`use_culture_states` formables (vanilla `geographic_region_india`/`_indonesia`/`_china` work here — `is_in_geographic_region` traverses their `strategic_regions`, unlike the formation `geographic_region` field). Always confirm the formable's OWN capital resolves in the chosen region first, or you break its candidacy. UNE (global) intentionally stays rank-only. When adding any new pan-region formable, add the capital-in-region gate from the start.
- **A culture's heritage can switch on vanilla content.** Forming adds every listed culture to the country's primary cultures, and vanilla content keyed on `any_primary_culture = { has_discrimination_trait_group = … }` then treats the formable as that kind of country. Métis in UNA's list put vanilla's Turtle Island entry in front of UNA; its `change_tag = TIS` button and UNA's re-formation made the AI alternate between the two tags. For each culture, grep the vanilla clone for its heritage group (`git grep -n "heritage_group_<x>" common events`) before settling the list. Details: `docs/guides/scripting_best_practices.md` § "Forming a Country Can Switch On Vanilla Content Meant for Someone Else".
