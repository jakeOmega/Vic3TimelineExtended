...
| `covert_warfare_rule` | enabled | Covert operations command-centre JE (`je_covert_warfare`), 14 covert diplomatic actions |
...
- **United Nations (`je_united_nations`)** — full UN simulation: founding, membership, an authority bar (0–100) that follows the state of the world, five tiers whose enforcement scales resolutions, charter reforms that raise the ceiling, a crisis that can end in dissolution and a later refounding, Security Council with veto power, expulsion votes, 17 vote topics, 8 specialized agencies (WHO, UNESCO, ICJ, UNHRC, IAEA, UNEP, UNHCR, UNOOSA), graduated-binding-resolution model, and a `join_united_nations` treaty article.
  - Files: `common/journal_entries/je_united_nations.txt`, `common/scripted_buttons/un_buttons.txt`, `common/scripted_effects/un_vote_effects.txt`, `common/scripted_progress_bars/un_progress_bars.txt`, `common/script_values/un_script_values.txt`, `common/scripted_triggers/un_permanent_member_triggers.txt`, `common/scripted_triggers/un_resolution_triggers.txt`, `common/on_actions/un_on_actions.txt`, `events/un_events.txt`, `events/un_vote_events.txt`. Generator `gen_un_button_descs.py` keeps loc in sync with effect bodies.
...
- **Nuclear weapons (`je_nuclear_program`)** — one entry for the programme, posture and crises: a Great Power-only warhead programme (progress bar, first-mover bonuses, stockpile, world-first detection), plus doctrine, readiness, launch authority, upkeep, incidents and nuclear crises for anyone armed or party to a crisis. Diplomatic actions: `nuke_diplo_action`, `tactical_nuke_diplo_action`. Treaty articles: `nuclear_disarmament`, `nuclear_program_aid`, `nuclear_program_pause`. Files: `common/journal_entries/je_nuclear_program.txt`, `common/scripted_buttons/nuclear_program_buttons.txt`, `common/scripted_effects/nuclear_weapon_effects.txt`, `common/scripted_triggers/nuke_triggers.txt`, `common/diplomatic_actions/nuke.txt`, `events/nuclear_weapon_events.txt`.
...
- **Global warming (`je_global_warming`)** — persistent JE tracking `temperature_anomaly_display` against a 4°C goal. 6 status tiers (negligible → catastrophic). Dynamic `global_warming` modifier scaled by current anomaly. 16 climate-policy buttons (carbon tax, renewables, climate adaptation, emission standards, reforestation, public transit, fossil fuel divestment, green building codes). Threshold + reversal events at 0.5°C / 1.0°C / 2.0°C / 3.0°C and (cooling) recovery events at the same milestones.
  - Files: `common/journal_entries/je_global_warming.txt`, `common/scripted_buttons/global_warming_buttons.txt`, `events/environmentalism_events.txt`. The `enforce_emissions_reduction` treaty article (`common/treaty_articles/109_enforce_emissions_reduction.txt`) forces a market leader to keep all major mitigation policies active.
...
- **Dynamic homelands** — monthly progress bars that create / remove homelands based on primary-culture status and population thresholds, with live blocker explanations in the state panel.
...
- `common/scripted_buttons/` — per-system button files (banking, colonial empire, covert warfare, cultural hegemony, global warming, heir education, nuclear program, space race, strategic reserve, UN, world war) plus a catch-all `timeline_extended_scripted_buttons.txt` (central-bank `cb_*` tools and the custom-religion wizard).
...
| `scripts/generators/add_tech_modifiers.py` | Registers tech-gate booleans (`country_*_pb_principles_bool`, `country_can_use_*_bool`) and rewrites the matching `has_technology_researched` triggers into `modifier:X = yes` checks, so the tech tree shows what each tech unlocks. **Additive and idempotent** — a run on a clean tree leaves no diff. Its outputs, notably `common/modifier_type_definitions/tech_gate_modifier_types.txt`, are hand-maintained; the script only appends what is missing and never prunes, so *deleting* a gate is a manual edit in both the script's tables and the files. Pinned by `test_add_tech_modifiers.py`. Takes `--root` to target another checkout. |
