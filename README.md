...
Several **social-movement journal entries** model major modern movements. Each has a `modifiers_while_active` modifier, monthly pulse events, AI-tuned three-option choices, and a fail-state event. Documented at `docs/systems/mod_systems.md` § Social Movement Journal Entries; the JE files themselves are the authority — `ls common/journal_entries/` is the live roster.

| JE | Trigger tech | Lawgroup / win condition | Files |
|---|---|---|---|
| Human augmentation | `biohacking_and_human_augmentation` / `brain_computer_interfaces` | `law_regulated_augmentation_market` / `law_mandatory_augmentation` | `common/journal_entries/je_human_augmentation.txt`, `events/augmentation_events.txt` |
| Digital rights | `automated_surveillance` / `cybersecurity` | `law_strong_privacy_rights` | `common/journal_entries/je_digital_rights.txt`, `events/surveillance_events.txt` |
...
| Mental health | `mental_health_awareness` | `law_rehabilitation_focused_criminal_justice` + social_security ≥ 4 | `common/journal_entries/je_mental_health.txt`, `events/mental_health_events.txt` |
| Civil rights | minority-rights lawgroup tiers | `law_racial_segregation` → `law_multicultural` ladder | `common/journal_entries/je_civil_rights.txt`, `events/movement_events_te.txt` |
...
- **Civil rights (`je_civil_rights`)** — minority-rights movement, status tiers from minority-rights law, monthly events. Also `common/scripted_effects/civil_rights_effects.txt`.
...
- `common/on_actions/` — 20 files; the spine is `extra_on_actions.txt`, which wires monthly/yearly pulses, immediate triggers (on_company_disbanded, on_building_built, on_law_activated, …), and FMC market hooks. Pulse routing for every system is enumerated in `docs/systems/mod_systems.md` § On-Actions Reference.
